...
class EpochModel:
    """Bound speculation by ordered confirmation, including early completions."""
    def __init__(self, count, window):
        assert count > 0 and window > 0
        self.states = ["queued"] * count
        self.window, self.frontier, self.aborted = window, 0, False

    def dispatch(self, sequence):
        if (self.aborted or self.states[sequence] != "queued"
                or sequence >= self.frontier + self.window):
            return False
        self.states[sequence] = "started"
        return True

    def finish(self, sequence, *, unexpected_failure=False):
        assert self.states[sequence] == "started"
        self.states[sequence] = "failure" if unexpected_failure else "completed"
        if unexpected_failure:
            self.aborted = True
            self.states = ["cancelled_before_start" if state == "queued" else state
                           for state in self.states]
        if not self.aborted:
            while self.frontier < len(self.states) and self.states[self.frontier] == "completed":
                self.frontier += 1
...
def run_checks():
    ...
    # The original proposal order is independent of both ID sorting and delivery.
    score_rows = [("x", 10, 0), ("a", 10, 1), ("b", 12, 2)]
    decide = lambda rows: min(rows, key=lambda item: (item[1], item[2]))[0]
    assert decide(score_rows) == decide(list(reversed(score_rows))) == "x"
    current_epoch = 2
    ...
    epoch = EpochModel(10, window=2)
    assert epoch.dispatch(0) and epoch.dispatch(1)
    epoch.finish(1)
    assert epoch.frontier == 0 and not epoch.dispatch(2)
    epoch.finish(0, unexpected_failure=True)
    assert epoch.aborted and epoch.states[1] == "completed"
    assert epoch.states[2:] == ["cancelled_before_start"] * 8
    assert not epoch.dispatch(2)
    passed("completed_ahead_of_slow_failure_still_occupies_speculation_window")

    other = EpochModel(4, window=2)
    other.dispatch(0)
    other.dispatch(1)
    other.finish(1, unexpected_failure=True)
    assert other.states[0] == "started"  # cannot claim a queued/free cancellation
    other.finish(0)
    assert other.aborted and other.frontier == 0
    strict = EpochModel(3, window=1)
    assert strict.dispatch(0) and not strict.dispatch(1)
    strict.finish(0)
    assert strict.dispatch(1)
    passed("failure_preserves_started_accounting_and_strict_mode_stays_serial")
