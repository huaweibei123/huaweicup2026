...
\begin{document}
\hypersetup{pdfsubject={Paper review v2}}
\maketitle
\begin{center}\small 审阅稿 v2\end{center}
\begin{abstract}
...
\keywords{计算图切分；多核调度；微架构仿真；数据搬运；缓存复用}
\end{abstract}
\section{问题重述与分析}
...
\tightlist
\item
  \textbf{子图间数据依赖边集 \(E_{\mathcal{S}}\)}：若子图 \(s\) 内存在算子生成的数据张量被子图 \(t\) 内的算子读取作为输入（\(s \ne t\)），则在子图之间引入一条有向数据依赖边 \((s, t) \in E_{\mathcal{S}}\)；
\item
...
\tightlist
\item
  \textbf{计算算子子集与周期定义}：设 \(V_{\mathrm{comp}} \subseteq V\) 为具有确定执行时长的计算算子集合，它由绑定在矩阵流水线 \texttt{PIPE\_M}（Cube 单元）与向量流水线 \texttt{PIPE\_V}（Vector 单元）上的计算操作构成，其算子固有计算耗时严格定义为 \(d_v = \max(1, \text{cycles}_v)\)（单位为 cycle）。集合 \(V \setminus V_{\mathrm{comp}}\) 中的数据搬运与内存访问操作耗时取决于底层总线仲裁、动态带宽争用与缓存状态，不具有静态固定常数周期；
\item
...
\item
  \textbf{关键路径依赖下界}：在由全体计算算子 \(V_{\mathrm{comp}}\) 构成的依赖关系中，若算子 \(u\) 的输出张量直接或通过中间数据搬运操作传递给算子 \(v\) 读取，则存在从 \(u\) 到 \(v\) 的计算依赖路径（保留经搬运操作传递的依赖关系，不因忽略搬运耗时而断开依赖路径）。定义从源点算子到汇点算子沿任意计算依赖路径累加各算子计算时长 \(d_v\) 的最大值为\textbf{关键路径长度} \(L_{\mathrm{cp}}\)（cycle），该累加严格计入路径终点算子的计算时长 \(d_v\)。由于同一依赖链上的算子在逻辑上必须依次串行执行，任何合法方案的全局执行时间必然不低于 \(L_{\mathrm{cp}}\)。
\end{enumerate}
...
\item
  \textbf{离散事件模拟评测开销}：单次离散事件评测耗时 \(T_j\) 取决于评测内核在推进离散事件时的队列操作与硬件模拟规模。在实际求解测试中，若采取盲目迭代搜索，随着评估次数累加，总时间将迅速失控。本文各求解器将单用例的在线评测调用次数 \(b\) 严格冻结在极小常数范围（问题一至多 10 次，问题二与问题三至多 3 次），大幅削减了在线评测的时间占比；结合轻量级结构校验，实测端到端运行时间整体落在题面所建议的高效区间内。
\end{enumerate}
...
针对该计算图结构，算法继承并拓展了团队成员 Fang 在上游工作（\texttt{0bf12cf}）中建立的任务结构装箱与分叉启发式排序框架，在统一求解入口（\texttt{unified.py}）中设计了六类互补的基础结构候选生成器。针对给定用例与核心数 \(K\)，求解器至多构造六种不同侧重点的基础方案：
...
\item
  \textbf{前沿子树划分与并行展开（\texttt{\seqsplit{fork-frontier}}）}：源自 Fang（\texttt{0bf12cf}）的分叉阶段与前沿子树划分策略，并推广至无分叉链及内向树（in-trees）结构，沿拓扑阶段以前沿粒度（\texttt{grain=4}）将不相交的子树打包分配至各核心，挖掘拓扑天然存在的横向并行性；
\item
...
\item
  \textbf{通信流量与多核权衡}：全量 500 个用例的额外 DDR 搬运总量从基准版本的 3,043,860,416 字节上升至 4,159,738,560 字节（增幅为 36.66\%）。深入分析 500 个用例的性能变动分布：共有 55 个用例实现了执行时间与搬运量双重下降；199 个用例执行时间显著缩短但搬运量有所增加；14 个用例执行时间缩短而搬运量持平；其余 232 个用例两项指标均保持不变。该数据佐证了算法以主目标（Makespan）优先的选优策略，表明算法接受了通过合理增加通信换取核心并发度的有效决策；
\item
...
\item
  \textbf{先入先出替换策略（FIFO）}：当 Cache 剩余可用空间不足以容纳新插入的张量时，Cache 依据各数据块的首次插入时间先后顺序，依次逐出最早存入的数据块，直至释放出足够的连续可用空间。\textbf{缓存命中操作不会更新或刷新该数据块的入队时间戳，亦不改变其在 FIFO 队列中的淘汰顺序}；
\item
...
对于由相邻分隔算子 \(u_{i-1}\) 与 \(u_i\) 划定的局部时间区间，考察落在该区间内部且与前后边界均存在依赖路径的计算操作子集 \(S \subseteq V_{\mathrm{comp}}\)（不含分隔操作自身）。此处必须严格定义局部的内部前后依赖耗时下界 \(a, b \ge 0\)：

\begin{itemize}
\tightlist
\item
  前端内部计算下界 \(a\)：以左边界分隔操作 \(u_{i-1}\) 完成时刻为时间零点，定义为从 \(u_{i-1}\) 到集合 \(S\) 中操作的最长内部计算依赖路径时长（不含 \(u_{i-1}\) 自身的耗时）；
\item
  后端内部计算下界 \(b\)：止于右边界分隔操作 \(u_i\) 开始时刻，定义为从集合 \(S\) 中操作到 \(u_i\) 的最长内部计算依赖路径时长（不含 \(u_i\) 自身的耗时）。
\end{itemize}

不能将附录 B.2 中相对于全图源汇点全局定义的 \(h_v, t_v\) 直接代入局部区间，必须按当前区间的边界重新计算内部局部链长。集合 \(S\) 中的每个受约束操作，在局部拓扑中必须同时存在来自左边界 \(u_{i-1}\) 的前驱路径以及通往右边界 \(u_i\) 的后继路径。若考察的区间处于计算图最前端（无前驱分隔操作），则令 \(\ell = 0\) 且 \(a = 0\)；若处于计算图最末端（无后继分隔操作），则令 \(r = 0\) 且 \(b = 0\)；当区间两端均存在确定分隔操作时，令 \(\ell = 1, r = 1\)。

对于被分配至不同核心的计算操作，跨核心硬件依赖等待常数记为 \(\tau_q\)（依据附录 B.1 经官方验证的硬件语义：问题一跨 Task 硬件同步屏障为 \(\tau_1 = 1000\) cycles；问题二与问题三跨核心搬运等待下界为 \(\tau_2 = \tau_3 = 500\) cycles；该等待下界仅在硬件规则明确支持的范围内生效）。扣除区间内部前后计算已自然覆盖的重叠时间后，跨核传输仍需额外占据的纯等待时间下界为：
...
设该局部时间区间的物理跨度为 \(x\)，扣除前后内部计算链长后，可供集合 \(S\) 中操作并发执行的有效时间窗口为 \(z = x - a - b\)，对应流水线上的总计算工作量为 \(W(S) = \sum_{v \in S} d_v\)。若前后两个分隔操作指派在同一核心，该核心在窗口内最多可提供 \(z\) 个周期，其他核心受跨核传输等待约束，各至多提供 \((z - c)_+\) 个有效计算周期，故有：
...
仅当各个区间之间严格互不重叠且由拓扑依赖链串联时，各区间的下界与分隔算子自身的执行周期方可直接线性求和累加。若仅考察局部相互重叠的滑动窗口，则只能取各窗口下界的最大值，绝不可重复累加。此外，问题一中同一核心内部不同 Task 切换所引入的 100 cycles 切换开销，由于具体方案未必在所选算子边界处切分子图，不能作为必要下界无条件计入。
...
为了形式化分析状态信息的充分性，首先固定输入的原始计算图 \(G\)、目标计算核心数 \(K\) 以及官方硬件配置参数 \(\text{config}\)。设 \(\mathcal{P}\) 为声明的合法调度方案域，定义一组允许对方案施加的确定性原子修改动作集合 \(\mathcal{A}\)（例如在声明的结构范围内移动、合并或拆分子图；每项动作的参数含义与执行逻辑严格确定）。对于任意动作 \(a \in \mathcal{A}\)，记能够合法执行该动作的方案子集为 \(D_a \subseteq \mathcal{P}\)。我们假设动作转移具有确定性，且声明的方案域 \(\mathcal{P}\) 对允许动作封闭，即：
...
需要明确的是，命题 B.1 仅给出了状态信息足以支撑无损预测的充分条件，并未证明该状态表示在信息论或状态复杂度意义下达到了最小表示（极小商空间）。若要证明状态的最小性，必须进一步证明任意两个不同的抽象状态均存在某条测试动作序列能够将其观测区分开。本文求解器在实现上并未构建此类通用行为商空间，而是针对具体的搜索算子实施轻量级局部校验与字节级哈希去重；本命题旨在为未来探索状态折叠与受控搜索空间压缩提供严谨的判定基准。
