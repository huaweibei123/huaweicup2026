...
\begin{document}
\hypersetup{pdfsubject={Paper review v7}}
\maketitle
\begin{center}\small 审阅稿 v7\end{center}
\begin{abstract}
...
\textbf{针对问题二：} 在同一核心内子图合并为单一任务、允许私有缓存跨子图复用数据的场景下，本文联合优化计算位置与数据搬运。针对容量受限引发的溢出开销，利用空闲区间编排计算时序，并建立\textbf{超图连接度模型}刻画多算子共享张量在核心边界的预阶段原始复制量；在二核心局部调整中设计\textbf{受流水线工作量上限保护的最小割方法}，防止算力向单核倾斜。5 核心场景下，100 张基准图相对官方单核基准的逐用例\textbf{平均加速比达到 4.5498}。

\textbf{针对问题三：} 在同一核心内子图合并并引入共享只读缓存的场景下，本文协同优化算子顺序与缓存利用。针对多分支树状归约结构，在子树不交错且内部开销固定的受限模型下，证明\textbf{按子树内部峰值与保留输出之差（\(h_i - r_i\)）降序排列}可使模型内临时结果存储峰值达到最小；构建\textbf{基于硬件流水线必要不等式的时间下界}快速剪枝，在至多 3 次在线评测预算内按 Makespan 单调改善选优。5 核心场景下，100 张基准图相对官方单核基准的逐用例\textbf{平均加速比达到 4.7576}；针对同一方案开展同配置成对仿真，开启只读缓存相比关闭缓存的\textbf{逐用例成对平均加速比达到 1.0829}，平均字节命中率为 \textbf{31.36\%}。
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=.84\textheight,keepaspectratio]{figures/team-v5/p2-lifetimes.pdf}
\caption{单核心张量生命周期与片上私有缓存容量占用示例。上部展示 9 个物理张量在闭区间内的驻留条带；中部与下部展示静态编译操作序号下的 L1 Buffer（容量 512 KiB）与 Unified Buffer（容量 128 KiB）存储占用阶梯曲线，说明输入与输出张量在操作边界共同驻留对局部容量峰值的影响。}
...
\tightlist
\item
  \textbf{例 A（固定核心集合为空，\(F = \emptyset\)）}：超边连接两条局部可移动链 \(u, v\)，超边权重为 \(w\)。基准偏移为 \(\text{offset} = -w\)。若 \(u, v\) 同归属于某一核心（例如均分在核心 \(a\) 或均分在核心 \(b\)），网络割容量为 \(w\)，加上偏移后局部复制代价为 \(C = w - w = 0\)；若 \(u, v\) 分跨两个核心，两侧辅助节点的容量边均被割断，割容量为 \(2w\)，加上偏移后的局部复制代价为 \(C = 2w - w = w\)；
\item
  \textbf{例 B（已固定触及核心 \(a\) 与另一核心 \(c\)，\(F = \{a, c\}\)）}：由于核心 \(a\) 已在固定集合中，系统不再为核心 \(a\) 建立容量边，仅当有链移入新增核心 \(b\) 时才产生新增复制，基准偏移为 \(\text{offset} = +w\)。若链全部保留在核心 \(a\)，割容量为 0，总代价为 \(C = 0 + w = w\)；若有链被分配至核心 \(b\)，割断指向核心 \(b\) 的容量边（权重 \(w\)），总代价为 \(C = w + w = 2w\)。两类情形求得的割容量与偏移之和均与真实超图连接度严格相等。
\end{enumerate}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=.84\textheight,keepaspectratio]{figures/team-v5/p2-local-cut.pdf}
\caption{二核心局部超图割辅助网络构建与工作量保护流程。上部展示超边在固定核心集合为空集与非空集合时的有向辅助网络映射，说明割容量加上固定偏移等于局部超边复制代价；下部展示结合流水线工作量上限保护的迭代锚定求解流程。}
...
\item
  \textbf{空闲区间方案 P1}：尝试构造 P1，仅在 P1 构造成功后，才以原始 P1 为基准启动局部超图调整；若 P1 与 P0 相同，则不重复入列；
\item
  \textbf{局部改进方案 P2 的门槛控制}：仅当局部调整后得到的预 Step 2 原始复制字节数（pre-Step 2 original COPY bytes）严格减少且重排成功时，方生成完整的 P2 方案（\texttt{hypergap}）；若字节数未严格减少或调整不支持，则跳过 P2；
\item
  \textbf{按序去重与受限评测}：完整方案按 P0 \(\to\) P1 \(\to\) P2 的构造顺序进行去重。若去重后仅有 1 份有效方案，直接输出 P0 而不发起任何在线评测；若存在 2 份或 3 份互不相同的方案，向在线评分接口提交请求（总调用次数严格不超过 3 次）。在当前论文的固定求解器版本中，该评分接口为原生离散仿真 E2；独立的外部 E0 复验作为离线验证单独报告，不计入在线求解调用账；
\item
  \textbf{字典序决选}：依评测返回的目标向量 \((M_2, \text{added\_copy\_bytes})\) 按严格字典序择优，平局时优先保留构造序列靠前的方案。需要强调的是，局部原始 COPY 字节数门槛仅为触发 P2 生成的内部过滤条件，并不构成官方最终 Makespan 必定改善的理论保证。
\end{enumerate}

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=.84\textheight,keepaspectratio]{figures/team-v5/p2-three-plans.pdf}
...
\item
  \textbf{完整候选的评测决策链}：在求解流程中，所有候选方案共享至多 3 次在线 E0 评测预算。基础方案与注意力感知候选先行生成并确立当前最优解；针对新构造的树状排序方案，系统依次实施严格的前置检验：若已用评测次数达到 3 次，或无法构造合法的树状排序方案，或与当前最优方案编码完全相同，则直接跳过评测；
\item
  \textbf{理论下界过滤与严格改善}：若该方案满足下界构建条件且计算所得下界 \(L(\text{proposal}) \ge U\)，则断定其完成时间绝不可能超越当前最优值，直接作为 \texttt{\seqsplit{bound\_pruned}} 剪除；若由于拓扑或算子类型导致下界不支持（\texttt{\seqsplit{UnsupportedBound}}），算法记录 \texttt{\seqsplit{bound\_unavailable}} 并继续推进评测，绝不将无法证明的下界当作剪枝条件；仅当通过全部前置检验时，方调用官方 E0 进行最终评测，且仅在 Makespan 严格缩短时采纳该方案。
\end{enumerate}

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=.84\textheight,keepaspectratio]{figures/team-v5/p3-forest-decision.pdf}
\caption{树状分支排序准则与完整候选方案的评测决策流程。上部展示基于指标差值 $h_i - r_i$ 降序排定非交错子树执行顺序的判定条件与局部峰值最小化范围；下部展示共享至多 3 次在线 E0 评测预算下的结构合法性校验、去重检查、理论下界剪枝与严格 Makespan 改善择优决策链。}
...
\caption{固定实现与复现入口}\label{tab:appendix-1}\small
\begin{tabular}{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}
