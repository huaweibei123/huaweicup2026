...
snapshot={str(f.relative_to(P)): f.read_bytes() for folder in ['chapters','appendix-code','data','ai-disclosure']
...
def figure(file,caption,label,trim=None):
    ...
    if file == 'fig-p1-cuts-xy-corrected.png' and trim is None:
        ...
        trim = '0 235 0 0'
    files=[file]
    if file == 'team-v8/p2-three-plans-a.pdf':
        files.append('team-v8/p2-three-plans-b.pdf')
    parts=[]
...
def number_tables(x, chapter, captions=None):
    ...
    def add(m):
        ...
        title = (captions or {}).get(index, headings[-1])
        caption = r'\caption{' + title + r'}\label{tab:' + chapter + '-' + str(index) + '}'
...
def clean_md(s, chapter=None):
    ...
    def render_algorithm(m):
        ...
        if code:
            ...
            long_algorithm = len(code_text.splitlines()) > 48
            code_style = 'paperpseudolong' if long_algorithm else 'paperpseudo'
            source_note = pandoc(content[code.end():].strip())
            if long_algorithm:
                after_float = '\n' + r'{\small ' + source_note + r'\par}' + '\n'
                source_note = ''
            rendered = (pandoc(content[:code.start()].strip()) +
                        '\n' + r'\begin{lstlisting}[style='+code_style+']' + '\n' +
                        code_text + '\n' + r'\end{lstlisting}' + '\n' +
                        source_note)
        else:
            rendered = pandoc(content.strip())
        # Flush the immediately preceding illustration before its local algorithm.
        # Text and mathematical logic remain authored in Markdown; only layout changes here.
        return ('\n\n'+r'\FloatBarrier'+'\n'+r'\begin{algorithm}[!htbp]'+'\n'+
                r'\caption{'+title+'}'+r'\label{'+label+'}\n'+r'\small'+'\n'+
                rendered+'\n'+r'\end{algorithm}'+'\n'+r'\FloatBarrier'+after_float+'\n\n')
...
for f in sorted(CHAPTERS.glob('*.md')):
    ...
    s=render_images(s)
    body.append(number_tables(pandoc(s), f.stem)+'\n')
...
appendix=number_tables(pandoc(render_images(clean_md(app))), 'appendix', appendix_captions)
appendix=appendix.replace(r'\section{',r'\FloatBarrier'+'\n'+r'\section{')
...
for item in code_manifest['files']:
    ...
    appendix+=r'\Needspace{14\baselineskip}'+'\n'
...
def convert_citations(x):
    # A numeric subscript in real/pseudo code is not a bibliography citation.
    return ''.join(part if part.startswith(r'\begin{lstlisting}') else
                   re.sub(r'\{\[\}(\d+)\{\]\}|\[(\d+)\]',cite_match,part)
                   for part in re.split(r'(\\begin\{lstlisting\}[\s\S]*?\\end\{lstlisting\})', x))
...
bib=r'\clearpage\begin{thebibliography}{'+str(len(references))+'}\n'+r'\raggedright'+'\n'
...
\lstdefinestyle{gmcmappendix}{
  basicstyle=\small\ttfamily\songti\color{gmcmcodetext},
...
\lstdefinestyle{paperpseudo}{style=gmcmappendix,language=Python,
  basicstyle=\fontsize{10}{12.8}\selectfont\ttfamily\songti\color{gmcmcodetext},
...
\lstdefinestyle{paperpseudolong}{style=paperpseudo,
  basicstyle=\fontsize{9.7}{12}\selectfont\ttfamily\songti\color{gmcmcodetext}}
...
for name,source,output in [('main',tex,'anonymous-paper-'+VERSION+'.pdf'),('result-tables',supplement,'result-tables.pdf')]:
    ...
    for run in range(4):
        ...
        rerun = ('Label(s) may have changed' in log or
                 ...
                 'Rerun to get' in log)
        if run >= 1 and not rerun:
            break
    if rerun:
        raise RuntimeError(name + ': cross-references did not stabilize after four passes')
