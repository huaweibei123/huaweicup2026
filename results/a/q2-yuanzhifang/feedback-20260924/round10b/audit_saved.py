...
def main():
    ...
    assert len(spec['methods']) == 1 and spec['methods'][0]['variant'] == 'capacity_window'
    n = len(spec['cases'])
    ...
    for name in ledger['attempts']:
        ...
        assert run['method']['variant'] == rec['variant'] == 'capacity_window'
        ...
        assert report['selected'] in {'capacity_window', 'capacity_window_guard_unchanged',
                                      'heavy_component_packet_override'}
        certified = False
        if report['selected'] == 'capacity_window':
            assert len(report['core_details']) == spec['cores']
            active = [detail for detail in report['core_details'] if detail.get('reason') != 'empty core']
            certified = bool(active) and all(detail['window'] >= 1 and
                all(detail['bucket_footprint_bytes'][pool] <= detail['capacity_bytes'][pool]
                    for pool in ('L1', 'UB')) for detail in active)
        ...
        rows.append(dict(case_id=case, baseline_cycles=base['makespan_cycles'], makespan_cycles=cycles,
                         ...
                         E0_seconds=run['stages']['E0']['wall_seconds'], route=report['selected'],
                         all_nonempty_cores_capacity_certified=certified,
