...
NAME = 'p2-rcx-069-e0-s8ee-20260925'
ARCHIVE = OUT / 'p2-rcx-069-k5-e0-s8ee-20260925.zip'
RUNNER = ROOT / 'scripts/q2_rcx_fixed_e0_probe.py'
...
def verify_launch():
    ...
    if hashlib.sha256(RUNNER.read_bytes()).hexdigest() != manifest['files']['scripts/q2_rcx_fixed_e0_probe.py']:
...
def main():
    ...
    try:
        ...
        live_call(deadline, 'download', ['download', '-s', NAME, '/content/q2-rcx-e0-results.zip',
        ...
        if record['exec_attempted'] and not (OUT / 'results.zip').exists():
            try:
                call('failure-download', ['download', '-s', NAME, '/content/q2-rcx-e0-results.zip',
