"""Three frozen P2 gap→latency-hyperrefine cells; partial mechanism pilot.
...
from src.q2_nikolastarx.latency_hyperrefine import build as latency_build
...
def old_cell(args, case, e2_identity):
    ...
    old_receipt = json.loads((folder / 'cell.json').read_bytes())
    frozen = json.loads((ROOT / 'results/a/q2-nikolastarx/hypergap-full500-20260925/manifest-workers1.json').read_bytes())
    if frozen['solver_commit'] != 'c66559a6f8a31ef7b4720e1f7c3c28d61f8dff3f':
        raise ValueError('old solver is not the pinned c665 release')
    expected_sources = {Path(name).name: digest for name, digest in frozen['solver_sources'].items()}
    old_runner_commit = 'ff47cbca4a953602dec7e4b959bbb10a016eca9a'
    expected_sources['hypergap_full500.py'] = sha(subprocess.check_output(
        ['git', 'show', old_runner_commit + ':src/q2_nikolastarx/hypergap_full500.py'], cwd=ROOT))
    plan, result = json.loads(plan_raw), json.loads(result_raw)
    calls, attempts = ledger.get('calls', {}), ledger.get('attempts', [])
    old_e0_process = json.loads((folder / 'e0-process/process.json').read_bytes())
    canonical = sha(json.dumps(plan, sort_keys=True, allow_nan=False).encode())
    matched = [a for a in attempts if a.get('plan_sha256') == canonical]
    if (ledger.get('status') != 'ok' or ledger.get('request_in_flight')
            or old_receipt.get('status') != 'accepted'
            or old_receipt.get('case') != case or old_receipt.get('cores') != CORES
            or old_receipt.get('graph_sha256') != sha(graph_raw)
            or old_receipt.get('config_sha256') != sha(config_raw)
            or old_receipt.get('plan_sha256') != sha(plan_raw)
            or old_receipt.get('solver_ledger_sha256') != sha((folder / 'online/solver.json').read_bytes())
            or old_receipt.get('official', {}).get('result_sha256') != sha(result_raw)
            or old_receipt.get('official', {}).get('makespan') != result.get('makespan')
            or ledger.get('solver_checkout_commit') != old_runner_commit
            or ledger.get('solver_source_sha256') != expected_sources
            or ledger.get('source_checked') is not True
            or any(ledger.get('source', {}).get(k) != e2_identity[k] for k in
                   ('commit', 'manifest_sha256', 'native_binary_sha256'))
            or old_e0_process.get('status') != 'ok' or old_e0_process.get('surviving_pids')
            or calls.get('E2_api_attempted') != len(attempts)
            or calls.get('native_returns') != len(attempts)
            or calls.get('E0_fallback') or ledger.get('possible_E0_fallback_calls')
            or len(matched) != 1 or matched[0].get('status') != 'native'
            or matched[0].get('record', {}).get('route') != 'native'
            or matched[0]['record'].get('status') != 'ok'
            or matched[0]['record'].get('problem') != 2
    ...
    native = matched[0]['record']
    if any(native.get(key) != result.get(key) for key in
           ('makespan', 'cross_task_traffic', 'data_movement_bytes')):
        raise ValueError('old selected native score differs from independent E0: ' + case)
...
def worker(args):
    ...
    at = time.perf_counter()
    selected, latency_meta = latency_build(graph, seed, config, region_width=16)
    stages['latency_hyperrefine_retime_seconds'] = time.perf_counter() - at
    selected_bytes = mandatory_copy_work(graph, selected, config['bandwidth'])['transfer_bytes']
    proxy = latency_meta['placement']
    if proxy['after_isolated_transfer_proxy_cycles'] > proxy['before_isolated_transfer_proxy_cycles']:
        raise AssertionError('latency proxy worsened')
    ...
    def checked_native(p):
        ...
        if remaining <= 0:
            raise TimeoutError('candidate-plus-E2 wall exhausted before request')
        try:
    ...
    if (len(ledger['attempts']) != 1 or attempt['status'] != 'native'
            or attempt['plan_sha256'] != sha(json.dumps(selected, sort_keys=True, allow_nan=False).encode())
            ...
            or ledger['calls']['E0_fallback'] or ledger['possible_E0_fallback_calls']
            or ledger['request_in_flight']):
        raise ValueError('native P2 E2 evidence unavailable')
    ledger['status'] = 'ok'
    save(ledger_path, ledger)
    result = {'case': cell, 'cores': CORES, 'source_commit': args.runner_commit,
              'old': row, 'seed_original_copy_bytes': seed_bytes,
              'selected_original_copy_bytes': selected_bytes,
              'seed_meta': seed_meta, 'latency_meta': latency_meta, 'stages': stages,
...
def main():
    ...
    if (args.raw_root / 'data/config.txt').is_file():
        args.raw_root = args.raw_root / 'data'
    ...
    if len(args.runner_commit) != 40:
        raise ValueError('full runner commit required')
    ...
    source_names = tuple(sorted(p.name for p in (ROOT / 'src/q2_nikolastarx').glob('*.py')))
    if len(source_names) < 47 or 'latency_hyperrefine.py' not in source_names:
        raise ValueError('incomplete solver source set')
    source_hashes = {}
    ...
    old = {case: old_cell(args, case, identities['e2']) for case in CASES}
