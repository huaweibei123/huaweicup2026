"""Frozen three-cell Colab CPU mechanism probe; E0 only, never a full score."""
...
def construct(case, out):
    ...
    from src.q2_nikolastarx.gap_candidate import build as gap
    from src.q2_nikolastarx.latency_hyperrefine import build as refine
    ...
    seed, seed_info = gap(graph, 5, cfg)
    plan, detail = refine(graph, seed, cfg, region_width=16)
    derive_multicore_plan(graph, plan)
    save(out/'plan.json', plan)
    save(out/'candidate.json', {'case':case, 'cores':5, 'seed_info':seed_info,
