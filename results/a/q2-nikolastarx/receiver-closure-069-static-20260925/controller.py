...
NAME = 'p2-rcx-069-s8ee-20260925'
ARCHIVE = OUT / 'p2-rcx-069-k5-s8ee-20260925.zip'
RUNNER = ROOT / 'scripts/q2_receiver_contract_colab_probe.py'
...
def verify_launch():
    ...
    if hashlib.sha256(RUNNER.read_bytes()).hexdigest() != manifest['files']['scripts/q2_receiver_contract_colab_probe.py']:
...
def main():
    ...
    try:
        ...
        live_call(deadline, 'exec', ['exec', '-s', NAME, '-f', str(OUT / 'bootstrap.py'), '--timeout', '160',
                     '--env', 'P2_RCX_CAPSULE_FILENAME=' + ARCHIVE.name,
                     '--env', 'P2_RCX_CAPSULE_SHA256=' + freeze['capsule_sha256']], 170)
        live_call(deadline, 'download', ['download', '-s', NAME, '/content/q2-rcx-069-k5-results.zip',
        ...
        if record['exec_attempted'] and not (OUT / 'results.zip').exists():
            try:
                call('failure-download', ['download', '-s', NAME, '/content/q2-rcx-069-k5-results.zip',
