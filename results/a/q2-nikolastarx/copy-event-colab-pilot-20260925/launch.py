...
archive=Path('/content/p2-copyevents-s8ee-20260925.zip')
assert hashlib.sha256(archive.read_bytes()).hexdigest()=='8df9e8410e8316243c8ac8897e63705034d2c1d67ef14005c02153d2608ea332'
root=Path('/content/p2-copyevents-s8ee-20260925'); root.mkdir(exist_ok=False)
with zipfile.ZipFile(archive) as z:
 for name in z.namelist():
  assert (root/name).resolve().is_relative_to(root)
 z.extractall(root)
output=root/'output/probe'; output.parent.mkdir()
command=[sys.executable,'-B',str(root/'results/a/q2-nikolastarx/copy-event-colab-pilot-20260925/runner.py'),'--output',str(output)]
...
result_zip=Path('/content/p2-copyevents-results-s8ee.zip')
