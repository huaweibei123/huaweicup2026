"""One manual follow-up after a zero-call manifest newline preflight failure; original analysis cap remains one."""
...
RUN = OUT / "run.json"
PRIOR_RUN = OUT.parent / "tail-response-static-20260925" / "run.json"
PRIOR_RUN_SHA256 = '850d007458713800ac7f7ffc0dd3c05acc0ed5c7d1de6590d56c30913fcd217d'
...
started = now()
prior_raw = PRIOR_RUN.read_bytes()
prior = json.loads(prior_raw)
if sha(prior_raw) != PRIOR_RUN_SHA256 or any(prior["calls"].values()):
    raise SystemExit("prior sealed run changed or consumed an analysis call; no dispatch")
...
for path, rev in PINNED.items():
    ...
    source_checks[path] = {"commit": rev, "bytes": len(current),
        "working_sha256": sha(current), "frozen_sha256": sha(frozen), "raw_bytes_equal": current == frozen,
        "matches": (current.replace(b"\r\n", b"\n") == frozen.replace(b"\r\n", b"\n")
            if path == "docs/a/source-manifest.json" else current == frozen),
        "comparison": "CRLF/LF-only normalization" if path == "docs/a/source-manifest.json" else "raw bytes"}
...
record = {"schema": "q3-tail-response-static-run-v1", "started_at_utc": started,
    ...
    "workflow_supervisor_attempt": 2,
    "prior_zero_call_run": {"path": "../tail-response-static-20260925/run.json", "sha256": PRIOR_RUN_SHA256},
    "retry_note": "Manual preflight correction; one previous supervisor attempt stopped before dispatch. Total real-graph analysis cap is still one; no automatic retry.",
