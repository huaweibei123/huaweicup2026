...
def run_process(argv, cwd, folder, limit, roots):
    ...
    with (folder / "stdout.txt").open("wb") as stdout, (folder / "stderr.txt").open("wb") as stderr:
        ...
        try:
            ...
            # Only the process created immediately above and its own tree.
            if os.name == "nt":
            ...
            try:
                ...
                receipt["cleanup_unconfirmed"] = True
                raise RuntimeError("child process cleanup could not be confirmed")
...
def main():
    ...
    if protocol_path.exists() and read(protocol_path) != protocol:
        raise ValueError("protocol changed: use a new run, do not silently mix results")
    if not protocol_path.exists():
        save(protocol_path, protocol)
    singles = {}
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        futures = {pool.submit(baseline, case, out, roots): case for case in cases}
        for future in as_completed(futures):
            case = futures[future]
            singles[case] = future.result()
    ...
    jobs = [(p, c, k) for c in cases for p in problems
            for k in ([1, *cores] if p == "P3" and args.include_p3_one else cores)]
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        futures = {pool.submit(cell, p, c, k, out, roots, singles[c]): (p, c, k) for p, c, k in jobs}
    ...
    print(json.dumps({"finished_at": utc(), "counts": aggregate(out)}), flush=True)
