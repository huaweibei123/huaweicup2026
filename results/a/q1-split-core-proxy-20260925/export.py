"""Export the existing P1 split-core case 084 probe; zero scoring calls."""
...
SOURCE = ROOT/'output/p1-split-core/20260924T1856Z-split084'
DEST = ROOT/'results/a/q1-split-core-proxy-20260925/20260924T1856Z-split084'
BASE_COMMIT = '6fcec11ccc472a1a652b21feb6fccf85a4555598'
BASE_PATH = 'results/benchmark-board/official-singlecore-20260924/084'
SOURCE_COMMIT = '57574a7c7bebf48a1823b13e576677b5f30bea64'
...
def main():
    ...
    assert result['scene']=='A' and result['num_cores']==5 and result['makespan']==run['makespan']==426656
    ...
    safe['derivation']={'original_sha256':sha(run_raw),'change':'Absolute interpreter paths in solver/evaluation argv replaced with <local-python>. All other receipt fields unchanged.','original_local_path':'output/p1-split-core/20260924T1856Z-split084/run.json'}
    ...
    record={
      'attempt_id':'nikolastarx-20260924T1856Z-split084-P1-084-k5-r0','revision':1,'run_id':run['run_id'],
      'algorithm_id':diagnostics['algorithm_id'],'algorithm_name':'P1 分核容量代理机制探针','variant':diagnostics['variant'],
      'solver_commit':SOURCE_COMMIT,'problem':'P1','case_id':'084','cores':5,'status':'ok',
      'parameters':{'cores':5,'whole_cores':diagnostics['whole_cores'],'cut_cores':diagnostics['cut_cores'],'whole_packet':diagnostics['whole_packet'],'cut_packet':diagnostics['cut_packet'],'cut_chains':diagnostics['cut_chains'],'policy':'Graph-derived split-core single candidate; proxy only, no online E0/E1 scoring or search','solver_timeout_seconds':15,'external_E0_timeout_seconds':60,'batch_timeout_seconds':90,'workers':1,'retries':0},
      ...
      'provenance':{'producer_session':'nikolastarx/s-6607cb2735304751b36662035723372b','task_url':'https://github.com/huaweibei123/huaweicup2026/issues/33',
        'solver':{'source':source('src/q1/capacity_split_cores.py','main'),'authors':['nikolastarx'],'method':'One deterministic split-core proxy candidate for private M/V/M chains; direct construction, no online scoring.','references':[],'upstream':[source('AI chats/P1多Pipe链构造证明/附件/r1-p1_s6607/p1_phase_cut.py','recognize/encode')],'selected_algorithm_id':None,'selected_solver_commit':None},
        'runner':{'source':source('src/q1_benchmarks/split_core_probe.py','run'),'argv':['<local-python>','-B','src/q1_benchmarks/split_core_probe.py',run['run_id'],'--execute'],'working_directory':'.'},
      ...
      'notes':['One-cell negative mechanism probe, not a unified full-500 or v2 three-cell result. Makespan 426656 exceeds full-return 399121 despite lower added DDR 7252992 versus 9925632 bytes.','Original run receipt contains local absolute interpreter paths; committed run-derived.json is a path-redacted derivative. Original SHA256='+sha(run_raw),
    ...
    feed=DEST/'board-feed-20260924T1856Z-split084.json'
