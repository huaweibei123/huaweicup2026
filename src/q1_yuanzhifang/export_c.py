"""Export the separately budgeted Stage C; never execute a solver/evaluator."""
...
from benchmark_c import ROOT, SESSION, BASELINE, OUTPUT, VARIANTS, NEW_SCRIPT, BASELINE_SCRIPT
...
def export(output):
    ...
    for row in rows:
        ...
        baseline_method = variant == VARIANTS[0]
        ...
        provenance = {"producer_session": SESSION, "task_url": protocol["task_url"],
                      "solver": {"source": source(row["solver_commit"], BASELINE_SCRIPT if baseline_method else NEW_SCRIPT, "construct / main"),
                                 "authors": ["NikolaStarx"] if baseline_method else ["yuanzhifang30-sudo"],
                                 "method": "Factor-4 in-tree frontier then split oversized Tasks only between independent components" if baseline_method else "Fork-rank stages, atomic serial chains, in-tree antichain frontiers and whole light reduction tail Task per stage",
                                 "references": [protocol["task_url"]], "upstream": [], "selected_algorithm_id": None, "selected_solver_commit": None},
                      "runner": {"source": source(row["runner_commit"], "src/q1_yuanzhifang/benchmark_c.py", "main"),
        ...
        record = {"attempt_id": f"fang-q1-stage-c-20260924-{variant}-{case}-k{cores}-r0", "revision": 1,
                  "run_id": "fang-q1-stage-c-20260924-" + variant,
                  "algorithm_id": "q1-bounded-component-tasks" if baseline_method else "q1-fork-stage-frontier",
                  "algorithm_name": "独立分量分块的树前沿构造" if baseline_method else "分叉阶段前沿构造",
                  ...
                  "artifacts": artifacts, "runtime_id": "fang-windows-q1-stage-c-20260924", "observed_at": row["finished_at"],
                  ...
                  "provenance": provenance, "notes": ["Public development panel: case051 k2/3/4/5 plus eight graphs k4; not a 100-case mean or held-out test.",
