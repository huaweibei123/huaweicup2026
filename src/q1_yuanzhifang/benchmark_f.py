"""Stage F: guarded split-chain star; only one new 051/k5 candidate attempt.

Default mode only checks inputs/source. --execute is required to consume the
separate Stage F budget, after a fixed solver SHA and scheduling window exist.
"""
...
SOLVER = "916a19e57c041ca5dc4aa1f3748e23726464b762"
NEW_SCRIPT = "src/q1_yuanzhifang/star_frontier.py"
OUTPUT = "results/a/q1-yuanzhifang/stage-f-20260925/run"
SCENARIOS = (("051", 5),)
VARIANTS = ("twelve-four-fixed-tail-v1",)
...
def preflight(args):
    ...
    if args.solver_commit != SOLVER:
        raise ValueError("Stage F solver identity differs from frozen task card")
    if (ROOT / OUTPUT).exists():
        raise ValueError("Stage F output directory already exists; no overwrite/retry")
    ...
    code = verify_source(ROOT, runner, ["src/q1_yuanzhifang/benchmark_f.py", "src/q1_yuanzhifang/benchmark.py",
                                       "src/q1_yuanzhifang/export_f.py", "src/q1_yuanzhifang/trace_c.py"])
    new_sources = None
...
def main():
    ...
    protocol.update(producer_session=SESSION, task_url="https://github.com/huaweibei123/huaweicup2026/issues/98",
                    ...
                    parameters={VARIANTS[0]: {"guard": "12 chains x 4 PIPE_V nodes, 11 binary reductions per round", "tail_core": 2, "fallback": "fork-frontier grain4 core tasks"}},
                    preparation="Reuse Stage A/B/C uv sync --locked environment; initial 14-package installation reported 43.79s before A. Parent reports synthetic structure/model tests and one real051 guard-only check before Stage F freeze (0 complete solvers/E0; full development wall not recorded here). Pro theoretical model and exposed Stage C evidence informed this fixed candidate; no held-out claim. No training or precomputed graph products reused by cold solver processes. Existing Stage C successful results are reused only for post-run comparison, with 0 baseline solver/E0 calls.",
                ...
                try:
                    ...
                    if run["solver"]["status"] == "ok":
                        ...
                        if (run["diagnostics"].get("selected"), run["diagnostics"].get("model_r_cycles"),
                            run["diagnostics"].get("task_count")) != ("split-chain-star", 166540, 263):
                            raise ValueError("Frozen 051 star identity/model differs; no E0 launch")
