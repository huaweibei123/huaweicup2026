"""Export the separately budgeted Stage H; never execute a solver/evaluator."""
...
def export(output, graphs):
    ...
    for commit, feed, variant, cycles in ((OLD_COMMIT, OLD_FEED, "chain-atomic-grain4", 253856),
                                           (G_COMMIT, G_FEED, "fixed-root-four-two-v1", 234536)):
        check_reused(commit, feed, variant, cycles, protocol["input_sha256"], protocol["official_code_hash"])
    g_feed = json.loads(git("show", G_COMMIT + ":" + G_FEED))
    prior_g = next(r for r in g_feed["records"] if
                   (r["case_id"], r["cores"], r["variant"]) == ("051", 5, "fixed-root-four-two-v1"))
    g_plan = json.loads(git("show", G_COMMIT + ":" + prior_g["artifacts"]["plan"]["path"]))
    g_result = json.loads(gzip.decompress(git("show", G_COMMIT + ":" + prior_g["artifacts"]["result"]["path"])))
    ...
    for row in rows:
        ...
        missing = {"provenance.environment.peak_rss_bytes": "Child peak RSS not instrumented; physical RAM and worker/thread counts recorded.",
                   "provenance.measurement.seed": "All compared methods are deterministic graph constructions with no RNG or seed parameter."}
        if row["failure"]:
        ...
        provenance = {"producer_session": H_SESSION, "task_url": protocol["task_url"],
                      "solver": {"source": source(row["solver_commit"], NEW_SCRIPT, "construct / main"),
                                 ...
                                 "method": "Guarded intact four-node chains: first round 3/3/3/3/0, later 4/2/2/2/2, remaining reduction tail on core 0; " +
                                           ("closed reduction subtrees fused into each frontier Task" if protocol["parameters"][variant]["fuse_reductions"] else "independent full reduction tail") +
                      ...
                      "runner": {"source": source(row["runner_commit"], "src/q1_yuanzhifang/benchmark_h.py", "main"),
        ...
        record = {"attempt_id": f"fang-q1-stage-h-20260925-{variant}-{case}-k{cores}-r0", "revision": 1,
                  "run_id": "fang-q1-stage-h-20260925-" + variant,
                  ...
                  "artifacts": artifacts, "runtime_id": "fang-windows-q1-stage-h-20260925", "observed_at": row["finished_at"],
                  ...
                  "provenance": provenance, "notes": ["One exposed development case: 051/k5, two separately frozen parameter variants; not a 100-case mean, unified full benchmark, or held-out test. Model R is not an official result. Existing Stage C/G evidence reused for comparisons; no new baseline scoring.",
        ...
        comparisons.append({"case_id": case, "cores": cores, "variant": variant, "status": row["status"], "makespan_cycles": cycles,
                            ...
                            "stage_g_cycles": prior_g["metrics"]["makespan_cycles"],
                            "stage_g_solver_wall_seconds": prior_g["metrics"]["solver_wall_seconds"],
                            "stage_g_extra_ddr_bytes": prior_g["metrics"]["extra_ddr_bytes"],
                            "stage_g_attempt_id": prior_g["attempt_id"],
                            "stage_g_cycle_difference": cycles - prior_g["metrics"]["makespan_cycles"] if cycles else None,
        ...
        if row["status"] == "ok":
            ...
            ddr_analyses.append({"case_id": case, "cores": cores, "variant": variant,
                                 ...
                                 "stage_g_commit": G_COMMIT, "stage_g_result_sha256": prior_g["artifacts"]["result"]["sha256"],
                                 "stage_g": static_ddr(graph, g_plan, g_result),
                                 "stage_h": static_ddr(graph, json.loads((ROOT / plan_path).read_bytes()), result)})
