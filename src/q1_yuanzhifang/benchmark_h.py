"""Stage H: guarded intact-chain prefetch; two new 051/k5 candidate attempts.

Default mode only checks inputs/source. --execute is required to consume the
separate Stage H budget, after a fixed solver SHA and scheduling window exist.
"""
...
OUTPUT = "results/a/q1-yuanzhifang/stage-h-20260925/run"
SCENARIOS = (("051", 5),)
VARIANTS = ("four-core-start-v1", "four-core-start-subtrees-v1")
PARAMETERS = {v: {"guard": "12 chains x 4 PIPE_V nodes, 11 binary reductions per round",
                  "startup_cores": 4, "fuse_reductions": i == 1, "tail_core": 0,
                  "first_round_chain_counts": [3, 3, 3, 3, 0],
                  "later_round_chain_counts": [4, 2, 2, 2, 2],
                  "expected_task_count": 143, "expected_model_r_cycles": (208152, 206917)[i],
                  "fallback": "fork-frontier grain4 core tasks"}
              for i, v in enumerate(VARIANTS)}
BUDGET = {"solver": 2, "E0": 2, "E1": 0, "E2": 0, "workers": 1,
          "wall_seconds": 360, "solver_timeout_seconds": 30,
...
G_COMMIT = "45fde88569b4ce877bda397ae32bc9a1b4abf082"
G_FEED = "results/a/q1-yuanzhifang/stage-g-20260925/board-feed-20260924T175725Z-stage-g.json"


def check_reused(commit, feed, variant, cycles, inputs, official_hash):
    raw = git("show", commit + ":" + feed)
    assert raw == (ROOT / feed).read_bytes()
    old = next(r for r in json.loads(raw)["records"] if
               (r["case_id"], r["cores"], r["variant"]) == ("051", 5, variant))
    assert old["status"] == "ok" and old["metrics"]["makespan_cycles"] == cycles
    assert old["identity"]["graph_sha256"] == inputs["case_051.json"]
    assert old["identity"]["config_sha256"] == inputs["config.txt"]
    assert old["identity"]["official_sha256"] == official_hash
    for item in old["artifacts"].values():
        stored = (ROOT / item["path"]).read_bytes()
        assert stored == git("show", commit + ":" + item["path"])
        assert sha(stored) == item["sha256"]
    result = json.loads(gzip.decompress((ROOT / old["artifacts"]["result"]["path"]).read_bytes()))
    assert result["makespan"] == cycles and result["num_cores"] == 5 and result["scene"] == "A"
    return {"commit": commit, "feed": feed, "feed_sha256": sha(raw),
            "attempt_id": old["attempt_id"], "artifacts": old["artifacts"], "makespan_cycles": cycles}


def preflight(args):
    if not re.fullmatch("[0-9a-f]{40}", SOLVER or ""):
        raise ValueError("Stage H is preparation-only until parent freezes the solver SHA")
    ...
    if args.solver_commit != SOLVER:
        raise ValueError("Stage H solver identity differs from frozen task card")
    if (ROOT / OUTPUT).exists():
        raise ValueError("Stage H output directory already exists; no overwrite/retry")
    ...
    code = verify_source(ROOT, runner, ["src/q1_yuanzhifang/benchmark_h.py", "src/q1_yuanzhifang/benchmark.py",
                                       "src/q1_yuanzhifang/export_h.py", "src/q1_yuanzhifang/trace_c.py"])
    new_sources = None
    ...
    old = check_reused(OLD_COMMIT, OLD_FEED, "chain-atomic-grain4", 253856, inputs, manifest["official_code_hash"])
    stage_g = check_reused(G_COMMIT, G_FEED, "fixed-root-four-two-v1", 234536, inputs, manifest["official_code_hash"])
    return {"runner_commit": runner, "solver_commit": args.solver_commit,
            ...
            "reused_stage_c": old, "reused_stage_g": stage_g}
...
def command(args, variant, case, cores, folder):
    ...
    argv = [*python, NEW_SCRIPT, graph, plan, "--cores", str(cores),
            "--config", relative(args.graphs / "config.txt"), "--diagnostics", diagnostics,
            "--startup-cores", str(parameters["startup_cores"])]
    if parameters["fuse_reductions"]:
        argv.append("--fuse-reductions")
    return argv
...
def main():
    ...
    protocol.update(producer_session=H_SESSION, task_url="https://github.com/huaweibei123/huaweicup2026/issues/98",
                    ...
                    preparation="Reuse the Stage A uv sync --locked environment; initial 14-package installation reported 43.79s before A. Parent reports 3 synthetic structural/model tests in 0.600s before source freeze; no real-graph solver/E0. Full development wall not recorded here. Exposed Stage C/F/G evidence and second Pro response informed these two finite structural mechanisms; no held-out claim. No training or precomputed graph products reused by cold solver processes. Existing Stage C/G successes reused only for post-run comparison, with 0 baseline solver/E0 calls.",
                ...
                try:
                    ...
                    if run["solver"]["status"] == "ok":
                        ...
                        if (run["diagnostics"].get("selected"), run["diagnostics"].get("model_r_cycles"),
                            run["diagnostics"].get("task_count")) != ("intact-prefetch-frontier", PARAMETERS[variant]["expected_model_r_cycles"], 143):
                            raise ValueError("Frozen 051 intact-prefetch identity/model differs; no E0 launch")
                        info = run["diagnostics"]
                        if (info.get("algorithm_id") != "q1-guarded-intact-prefetch" or
                            info.get("variant") != variant or
                            info.get("fuse_reductions") is not PARAMETERS[variant]["fuse_reductions"] or
                            info.get("first_chain_counts") != [3, 3, 3, 3, 0] or
                            info.get("later_chain_counts") != [4, 2, 2, 2, 2] or info.get("tail_core") != 0):
                            raise ValueError("Frozen variant flags/diagnostics mismatch; no E0 launch")
