"""Export the separately budgeted Stage F; never execute a solver/evaluator."""
...
from benchmark_f import ROOT, SESSION, SOLVER, OUTPUT, VARIANTS, NEW_SCRIPT, OLD_COMMIT, OLD_FEED
    ...
    for row in rows:
        ...
        provenance = {"producer_session": SESSION, "task_url": protocol["task_url"],
                      "solver": {"source": source(row["solver_commit"], NEW_SCRIPT, "construct / main"),
                                 ...
                                 "method": "Guarded twelve four-node chain split template: first round 2+2 cuts, later rounds 3+1 and 1+3 cuts, shared reduction tail on core 2; deterministic structural fallback",
                                 "references": [protocol["task_url"], "https://github.com/" + REPO + "/blob/" + SOLVER + "/docs/a/q1-yuanzhifang/STAR_FRONTIER.md"],
                                 "upstream": [], "selected_algorithm_id": None, "selected_solver_commit": None},
                      "runner": {"source": source(row["runner_commit"], "src/q1_yuanzhifang/benchmark_f.py", "main"),
        ...
        record = {"attempt_id": f"fang-q1-stage-f-20260925-{variant}-{case}-k{cores}-r0", "revision": 1,
                  "run_id": "fang-q1-stage-f-20260925-" + variant,
                  "algorithm_id": "q1-guarded-split-chain-star",
                  "algorithm_name": "结构守卫切链星式构造",
                  ...
                  "artifacts": artifacts, "runtime_id": "fang-windows-q1-stage-f-20260925", "observed_at": row["finished_at"],
                  ...
                  "provenance": provenance, "notes": ["One exposed development case: 051/k5; not a 100-case mean or held-out test. Model R 166540 is not an official result. Existing Stage C evidence reused for quality comparison; no new baseline scoring.",
        ...
        if row["status"] == "ok":
            ...
            ddr_analyses.append({"case_id": case, "cores": cores,
                                 ...
                                 "stage_f": static_ddr(graph, json.loads((ROOT / plan_path).read_bytes()), result)})
