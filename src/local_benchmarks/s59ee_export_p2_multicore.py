"""Export existing P2 k=2..5 receipts to board-submission-v1; never run a solver."""
...
RUNNER_COMMIT = "04be5d8b200cf65dec459dfa8e167bde565e3afc"
...
def main():
    ...
    if not batch.is_relative_to(ROOT / "results/a/local-p2-multicore-20260924"):
    ...
    runner_bytes = subprocess.check_output(["git", "show", f"{RUNNER_COMMIT}:src/local_benchmarks/s59ee_p2_multicore.py"], cwd=ROOT)
    ...
    for case, core_text in requested:
        k = int(core_text)
        if case not in {f"{i:03d}" for i in range(1, 101)} or k not in (2, 3, 4, 5):
        ...
        record = {
            ...
            "provenance": {
                ...
                "runner": {"source": {"repo": "huaweibei123/huaweicup2026", "commit": RUNNER_COMMIT,
                                      "path": "src/local_benchmarks/s59ee_p2_multicore.py", "entrypoint": "main"},
                           "argv": ["python", "-B", "src/local_benchmarks/s59ee_p2_multicore.py",
            ...
            "notes": ["True P2 solver at k=2..5, independently evaluated by frozen official E0; paired scene-A singlecore is a separate baseline.",
