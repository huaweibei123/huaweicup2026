"""Export six fixed P1 overload continuation receipts; never run a solver."""
...
RUNNER_COMMIT = "718d105b8f98ecc2c35ed36863c8a4421e9feda3"
...
def main():
    ...
    if not batch.is_relative_to(ROOT / "results/a/local-q1-overload-tail6-20260925"):
    ...
    runner_bytes = subprocess.check_output(["git", "show", f"{RUNNER_COMMIT}:src/local_benchmarks/s59ee_p1_overload_tail6.py"], cwd=ROOT)
    ...
    for case, core_text in requested:
        ...
        record = {
            "attempt_id": f"nikolastarx-s59ee-p1-overload-tail6-{case}-k{k}-r0",
            "revision": 1,
            ...
            "parameters": {"cores": k, "max_rounds": 64, "max_sinks": 64,
                           ...
                           "evaluation_timeout_seconds": 180, "batch_deadline_seconds": 900},
            ...
            "provenance": {
                ...
                "runner": {"source": {"repo": "huaweibei123/huaweicup2026", "commit": RUNNER_COMMIT,
                                      "path": "src/local_benchmarks/s59ee_p1_overload_tail6.py", "entrypoint": "main"},
                           "argv": ["python", "-B", "src/local_benchmarks/s59ee_p1_overload_tail6.py",
            ...
            "notes": ["P1 component-overload-list k5 explicit six-cell continuation: 098/099/100 were not dispatched in prior batch; 014/041/079 previously timed out at 60 seconds. Old attempts remain immutable.",
                      "Successful cells received an independent frozen official P1 E0 final check under a 180-second limit; this is a new run and does not alter prior timeout receipts.",
