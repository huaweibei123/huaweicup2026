"""Bounded, concurrent P2 k=1 benchmark of a pinned contiguous solver.
...
def cell(case, source, batch, deadline):
    folder = batch / "cells" / case
    ...
    record = {"case_id": case, "problem": "P2", "cores": 1, "algorithm_id": "q2-contiguous-baseline",
    ...
    try:
        if time.monotonic() >= deadline:
            ...
            return record
        solver = [sys.executable, "-B", source / "src/q2/construct.py", graph, "--cores", "1", "--output", plan]
        ...
        if record["evaluation"]["status"] != "ok" or not result.exists():
            ...
            return record
        full = read(result)
        if full.get("scene") != "B" or full.get("num_cores") != 1 or not isinstance(full.get("makespan"), (int, float)):
...
def main():
    ...
    if meta_path.exists():
        ...
        meta = {"schema": "local-p2-k1-v1", "batch_id": batch.name, "started_at": utc(),
                "started_monotonic_note": "deadline is 120 minutes from first invocation start; no restart extension",
                "solver_commit": SOLVER_COMMIT, "solver_entrypoint": "src/q2/construct.py main",
    ...
    existing = [c for c in cases if (batch / "cells" / c).exists()]
    if existing:
    ...
    invocation = {"utc": utc(), "cases": cases, "workers": args.workers, "status": "running"}
    meta["invocations"].append(invocation)
    write(meta_path, meta)
    results = []
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        futures = {pool.submit(cell, c, source, batch, deadline): c for c in cases}
        for future in as_completed(futures):
            item = future.result()
            results.append(item)
            print(json.dumps({"case": item["case_id"], "status": item["status"], "makespan": item.get("makespan_cycles")}), flush=True)
