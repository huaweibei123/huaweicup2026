"""Run the fixed P2 semantic constructor on all 100 cases and 1-5 cores.
...
MANIFEST = ROOT / "src/local_benchmarks/s59ee_p2_semantic_manifest.json"
RESULT_ROOT = ROOT / "results/a/q2-nikolastarx/semantic-benchmark-s59ee-20260925"
...
def verify_sources(source, declaration):
    ...
    if declaration["cases"] != [f"{i:03d}" for i in range(1, 101)] or declaration["cores"] != [1, 2, 3, 4, 5]:
        raise RuntimeError("manifest does not cover exactly 100 x 1-5")
    if declaration["maximum_calls"] != {"solver": 500, "E0": 500, "E1": 0, "E2": 0} or declaration["retries"] != 0:
        raise RuntimeError("manifest budget changed")
    if declaration["sampled_process_tree_rss_limit_bytes"] != MAX_GROUP_RSS:
        raise RuntimeError("process-tree RSS cap changed")
    pilot = [tuple(x) for x in declaration["pilot"]]
    if len(pilot) != len(set(pilot)) or any(c not in declaration["cases"] or k not in declaration["cores"] for c, k in pilot):
        raise RuntimeError("invalid pilot coordinates")
    return manifest
...
def run_child(argv, cwd, timeout, public_roots, source):
    ...
    with tempfile.TemporaryFile() as stdout, tempfile.TemporaryFile() as stderr:
        ...
        try:
            while proc.poll() is None:
                ...
                if peak > MAX_GROUP_RSS or time.monotonic() - t0 > timeout:
                    status = "failed" if peak > MAX_GROUP_RSS else "timeout"
                    limit_reason = (f"sampled process-tree RSS exceeded {MAX_GROUP_RSS} bytes"
                                    if peak > MAX_GROUP_RSS else f"wall exceeded {timeout} seconds")
    ...
    return {"argv": [scrub(str(x)) for x in argv], "cwd": scrub(str(cwd)),
            ...
            "status": status if status != "ok" else ("ok" if proc.returncode == 0 else "failed"),
...
def cell(case, cores, source, batch, deadline, stopped, limits):
    ...
    record = {"case_id": case, "problem": "P2", "cores": cores, "algorithm_id": "q2-adaptive-semantic",
    ...
    try:
        ...
        solver = [sys.executable, "-B", "-m", "src.q2_nikolastarx.adaptive_semantic", graph,
                  "--config", OFFICIAL / "data/config.txt", "--cores", str(cores),
                  "--output", plan, "--evidence", online, "--wall", str(limits["solver"])]
        ...
        if record["solver"]["status"] != "ok" or not plan.exists():
            record.update(status=record["solver"]["status"] if record["solver"]["status"] != "ok" else "failed",
                          failure={"stage": "solver", "reason": record["solver"].get("limit_reason") or "solver did not publish plan"})
            stopped.set()
            return record
        plan_obj = read(plan)
        if set(plan_obj) != {"node_to_subgraph", "core_schedules"} or len(plan_obj["core_schedules"]) != cores:
            raise RuntimeError("plan top-level keys differ from official contract")
        ledger_path = online / "solver.json"
        ledger = read(ledger_path)
        if ledger.get("status") != "ok" or ledger.get("calls") != {"E0": 0, "E1": 0, "E2": 0}:
            raise RuntimeError("online evaluator ledger is not zero-call success")
        if len(ledger.get("attempts", [])) != 1 or ledger["attempts"][0].get("status") != "constructed":
            raise RuntimeError("solver must record exactly one constructed route")
        if ledger.get("plan_sha256") != sha(plan) or ledger["attempts"][0].get("plan_sha256") != sha(plan):
            raise RuntimeError("online ledger and published plan hash disagree")
        if (online / "adaptive_semantic/plan.json").read_bytes() != plan.read_bytes():
            raise RuntimeError("online candidate and published plan bytes disagree")
        record["solver_evidence"] = {"status": ledger["status"], "calls": ledger["calls"],
                                     "route": ledger["attempts"][0]["detail"].get("adaptive_route"),
                                     "selected": ledger.get("selected")}
        record["artifacts"]["online_ledger"] = {"path": ledger_path.relative_to(ROOT).as_posix(), "sha256": sha(ledger_path)}
        record["artifacts"]["online_plan"] = {"path": (online / "adaptive_semantic/plan.json").relative_to(ROOT).as_posix(),
                                                "sha256": sha(online / "adaptive_semantic/plan.json")}
        ...
        if time.monotonic() >= deadline:
            ...
            stopped.set()
            return record
        official = [sys.executable, "-B", OFFICIAL / "code/multicore_cut_evaluate_problem_2.py", graph,
        ...
        if record["evaluation"]["status"] != "ok" or not result.exists():
            record.update(status=record["evaluation"]["status"] if record["evaluation"]["status"] != "ok" else "failed",
                          failure={"stage": "E0", "reason": record["evaluation"].get("limit_reason") or "official CLI did not publish result"})
            stopped.set()
            return record
        full = read(result)
        if full.get("scene") != "B" or full.get("num_cores") != cores or full.get("problem") == 3 or full.get("cache_mode") is not None or not isinstance(full.get("makespan"), (int, float)):
...
def main():
    ...
    if subprocess.check_output(["git", "show", f"{runner_commit}:src/local_benchmarks/s59ee_p2_semantic.py"], cwd=ROOT) != Path(__file__).read_bytes():
        raise RuntimeError("runner bytes differ from fixed commit")
    if subprocess.check_output(["git", "show", f"{runner_commit}:src/local_benchmarks/s59ee_p2_semantic_manifest.json"], cwd=ROOT) != MANIFEST.read_bytes():
        raise RuntimeError("manifest bytes differ from fixed commit")
    all_jobs = [(c, k) for c in declaration["cases"] for k in declaration["cores"]]
    pilot = [tuple(x) for x in declaration["pilot"]]
    jobs = pilot + [job for job in all_jobs if job not in set(pilot)]
    if args.preflight:
    ...
    meta = {"schema": declaration["schema"], "batch_id": batch.name,
            ...
            "runner_path": "src/local_benchmarks/s59ee_p2_semantic.py",
    ...
    results = []
    phases = ((jobs[:4], 2), (jobs[4:16], 4), (jobs[16:], declaration["max_workers"]))
    for phase_jobs, workers in phases:
        if not phase_jobs:
            continue
        invocation = {"utc": utc(), "coordinates": [list(job) for job in phase_jobs],
                      "cases": sorted(set(c for c, _ in phase_jobs)),
                      "cores": sorted(set(k for _, k in phase_jobs)),
