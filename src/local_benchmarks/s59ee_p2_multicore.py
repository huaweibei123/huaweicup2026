"""Bounded, concurrent P2 k=2..5 benchmark of a pinned contiguous solver.
...
def cell(case, cores, source, batch, deadline):
    ...
    record = {"case_id": case, "problem": "P2", "cores": cores, "algorithm_id": "q2-contiguous-baseline",
    ...
    try:
        if time.monotonic() >= deadline:
            ...
            return record
        solver = [sys.executable, "-B", source / "src/q2/construct.py", graph, "--cores", str(cores), "--output", plan]
        ...
        if record["evaluation"]["status"] != "ok" or not result.exists():
            ...
            return record
        full = read(result)
        if full.get("scene") != "B" or full.get("num_cores") != cores or not isinstance(full.get("makespan"), (int, float)):
...
def main():
    ...
    if len(set(cores)) != len(cores) or any(k not in (2, 3, 4, 5) for k in cores):
        parser.error("cores must be unique 2..5")
    ...
    if meta_path.exists():
        ...
        meta = {"schema": "local-p2-multicore-v1", "batch_id": batch.name, "started_at": utc(),
