"""Export existing P2 k=1 receipts to board-submission-v1; never run a solver."""
...
SOLVER_COMMIT = "0b58c123cccf02fc993b741d79dcd8511e4dd38f"
RUNNER_COMMIT = "c44640cc30f3e8af3a4e59b90db3048f3c811cfe"
...
def main():
    ...
    if not batch.is_relative_to(ROOT / "results/a/local-p2-k1-20260924"):
    ...
    runner_bytes = subprocess.check_output(["git", "show", f"{RUNNER_COMMIT}:src/local_benchmarks/s59ee_p2_k1.py"], cwd=ROOT)
    ...
    for case in requested:
        if case not in {f"{i:03d}" for i in range(1, 101)}:
            p.error("invalid case")
        run_path = batch / "cells" / case / "run.json"
        run = read(run_path)
        if run["status"] != "ok" or run["case_id"] != case or run["calls"] != {"solver": 1, "E0": 1, "E1": 0, "E2": 0}:
            raise RuntimeError(f"case {case} has no complete successful receipt")
        invocation = next(x for x in meta["invocations"] if case in x["cases"])
        ...
        record = {
            "attempt_id": f"nikolastarx-s59ee-p2-{case}-k1-r0",
            "revision": 1,
            "run_id": meta["batch_id"],
            "algorithm_id": "q2-contiguous-baseline",
            "algorithm_name": "P2 拓扑连续分块基线",
            "variant": "one-core-deterministic-direct",
            "solver_commit": SOLVER_COMMIT,
            "parameters": {"cores": 1, "partition": "min-id Kahn topological contiguous workload block",
                           "seed": "none", "candidate_limit": 1, "solver_timeout_seconds": 120,
                           "evaluation_timeout_seconds": 180, "batch_deadline_seconds": 7200},
            "problem": "P2", "case_id": case, "cores": 1, "status": "ok",
            ...
            "provenance": {
                ...
                "solver": {"source": {"repo": "huaweibei123/huaweicup2026", "commit": SOLVER_COMMIT,
                                      "path": "src/q2/construct.py", "entrypoint": "main -> contiguous_plan"},
                           "authors": ["yuanzhifang30-sudo"],
                           "method": "min-ID Kahn topological order, contiguous cumulative-work partition; no search or online scoring",
                           "references": ["https://github.com/huaweibei123/huaweicup2026/blob/0b58c123cccf02fc993b741d79dcd8511e4dd38f/src/q2/construct.py"],
                ...
                "runner": {"source": {"repo": "huaweibei123/huaweicup2026", "commit": RUNNER_COMMIT,
                                      "path": "src/local_benchmarks/s59ee_p2_k1.py", "entrypoint": "main"},
                           "argv": ["python", "-B", "src/local_benchmarks/s59ee_p2_k1.py",
                                    "--source", "../local-p2-fixed-0b58", "--batch", batch.relative_to(ROOT).as_posix(),
                                    "--cases", ",".join(invocation["cases"]), "--workers", str(invocation["workers"])],
                           "working_directory": "."},
            ...
            "notes": ["True P2 solver at k=1, independently evaluated by frozen official E0; this is not the official scene-A singlecore anchor.",
