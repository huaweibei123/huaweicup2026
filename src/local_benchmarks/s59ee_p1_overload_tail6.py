"""Complete six P1 overload-list k5 gaps with a separate bounded batch.
...
MANIFEST = ROOT / "src/local_benchmarks/s59ee_p1_overload_tail6_manifest.json"
RESULT_ROOT = ROOT / "results/a/local-q1-overload-tail6-20260925"
...
def verify_sources(source, declaration):
    ...
    if declaration["cases"] != ["098", "099", "100", "014", "041", "079"]:
        raise RuntimeError("manifest does not exactly cover six prior gaps")
    if declaration["cores"] != [5] or len(declaration["cases"]) != 6:
        raise RuntimeError("manifest scope differs from six k5 cells")
    if declaration["maximum_calls"] != {"solver": 6, "E0": 6, "E1": 0, "E2": 0} or declaration["retries"] != 0:
        raise RuntimeError("manifest budget changed")
    if declaration["timeouts_seconds"] != {"solver": 30, "E0": 180, "batch": 900} or declaration["workers"] != 2:
        raise RuntimeError("manifest execution limits changed")
    return manifest
...
def main():
    ...
    if subprocess.check_output(["git", "show", f"{runner_commit}:src/local_benchmarks/s59ee_p1_overload_tail6.py"], cwd=ROOT) != Path(__file__).read_bytes():
        raise RuntimeError("runner bytes differ from fixed commit")
    if subprocess.check_output(["git", "show", f"{runner_commit}:src/local_benchmarks/s59ee_p1_overload_tail6_manifest.json"], cwd=ROOT) != MANIFEST.read_bytes():
    ...
    meta = {"schema": declaration["schema"], "batch_id": batch.name,
            ...
            "runner_path": "src/local_benchmarks/s59ee_p1_overload_tail6.py",
    ...
    results = []
    for phase_jobs, workers in ((jobs[:3], 2), (jobs[3:], declaration["workers"])):
        if not phase_jobs:
            continue
