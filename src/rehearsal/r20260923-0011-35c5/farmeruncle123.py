...
Independent implementation for actor farmeruncle123. Mirrors the team
convention (yuanzhifang30-sudo) so the leader can rerun and compare, but the
coefficients are computed from the CSV, never hardcoded.
"""
...
def main() -> None:
    ...
    uv_lock_sha256 = sha256(uv_lock) if uv_lock.exists() else "not present in worktree"
    result = {
        ...
        "command": f"python {script_relative} --input {args.input} --output {args.output}",
