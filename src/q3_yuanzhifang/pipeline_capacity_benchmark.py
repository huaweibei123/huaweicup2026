"""Frozen capacity-constrained pipeline: 044 k4, one cold plan, at most two E0s."""
...
SOLVER_COMMIT = "2df4a5fa70d7a59492a7477e9ecd706501e64ad6"
HELPER_COMMIT = "39e9c8d8a78e384855ffbfbf41a2dec4d4a7e5a0"
OUT = "results/a/q3-yuanzhifang/pipeline-capacity-20260925"
CASES = ("044",)
VARIANT = "pipeline_capacity"
...
def preflight(graph_dir):
    ...
    for name in ("src/q3_yuanzhifang/pipeline_capacity.py", "src/q3_yuanzhifang/pipeline_setup.py",
                 "src/q3_yuanzhifang/pipeline_stages.py",
                 "src/q3_yuanzhifang/active_stages.py", "src/q3_yuanzhifang/construct.py",
                 "src/q3_yuanzhifang/baseline.py", "docs/a/q3-yuanzhifang/PIPELINE_CAPACITY.md"):
        if Path(name).read_bytes() != git("show", SOLVER_COMMIT + ":" + name):
            raise ValueError("capacity pipeline source/spec differs from frozen commit: " + name)
    ...
    for name in ("src/q3_yuanzhifang/pipeline_capacity_benchmark.py", "src/q3_yuanzhifang/pipeline_capacity_export.py",
                 "docs/a/q3-yuanzhifang/PIPELINE_CAPACITY_MEASUREMENT.md"):
        if Path(name).read_bytes() != git("show", head + ":" + name):
            raise ValueError("pipeline-capacity runner/exporter must be committed: " + name)
...
class PipelineCapacityPilot(Pilot):
    def __init__(self, args):
        super().__init__(args)
        (self.out / ".gitattributes").write_bytes(b"# Preserve exact process output and receipt bytes.\n* -text\n")
        self.info.update(schema="q3-pipeline-capacity-v1", run_id="yuanzhifang-q3-pipeline-capacity-20260925",
                         budget=BUDGET,
                         ...
                         "Parent reports seven synthetic capacity tests as development cost, not official results. "
                         "Cold solver includes graph/chain indexing, one O(k L^2) capacity-constrained "
                         "cold-setup flowshop DP, singleton plan construction, "
    ...
    def invoke(self, kind, call_id, argv, folder):
        ...
        if elapsed >= BUDGET["dispatch_until_seconds"]:
            raise Stopped("pipeline-capacity dispatch cutoff reached; no new call")
        if sum(call["kind"] == kind for call in self.calls) >= limit:
            raise Stopped("pipeline-capacity call cap reached; no new call")
        memory = memory_gate()
    ...
    def run(self):
        try:
            ...
            for case in CASES:
                ...
                cid = f"{case}-{VARIANT}"
                argv = [relative(sys.executable), "-B", "-m", "src.q3_yuanzhifang.pipeline_capacity",
                ...
                if not detail.get("guard") or detail.get("selected") != VARIANT:
                    raise Stopped("pipeline-capacity guard failed; no E0 on fallback")
            ...
            reason = ("one cold construction; plan bytes equal frozen pipeline control, reused with zero new E0"
                      ...
                      else "one guarded capacity pipeline construction and two external E0s completed")
...
def main():
    ...
    return PipelineCapacityPilot(args).run()
