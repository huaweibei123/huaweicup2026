"""Export sealed 067 single-tail artifacts; no constructor or evaluator calls."""
...
from tail_benchmark import SOLVER_COMMIT, OUT, CASES, TEMPLATE
...
def export(folder):
    ...
    if m.get("schema") != "q3-wave-tail-v1" or m.get("status") not in ("ok", "stopped"):
        raise ValueError("requires an ended single-tail batch")
    ...
    for failure_index, failure in enumerate(m.get("failures", []), 1):
        ...
        attempted = [x for x in calls if x["call_id"].startswith(f"{case}-wave_tail")]
        ...
        if last is None or last["status"] == "ok":
            ...
            construction_id = f"{case}-wave_tail"
    ...
    for e in work:
        ...
        if cid != f"{case}-wave_tail":
            raise ValueError("unexpected tail construction ID")
        ...
        row.update(attempt_id=m["run_id"] + "-" + e["evaluation_id"], revision=1,
            run_id=m["run_id"], algorithm_id="q3-wave-tail", algorithm_name="单余量作业切分波次构造",
            variant="wave_tail", solver_commit=SOLVER_COMMIT, status="ok" if success else call["status"],
            ...
            parameters=dict(cores=5, requested_cores=5,
                ...
                candidate_policy="one fixed single-tail wave construction; no E0 selection",
                guard="strict homogeneous jobs and private tensor signatures; J=qk+1, k=5, one split tail",
            ...
            notes=["067 k5 research pilot: one cold tail plan and up to two external E0s.",
                   "P2/P3 share the same construction ID and plan; new calls counted once in sealed ledger.",
                   "One worker; only the final job is split across five cores; no online E0 selection.",
        ...
        receipt = dict(schema="q3-wave-tail-attempt-v1", run_id=m["run_id"],
        ...
        destination = folder / case / "wave_tail" / (problem if success else "failed-" + call["call_id"])
        ...
        p["solver"].update(source=source(SOLVER_COMMIT,
            "src/q3_yuanzhifang/wave_tail.py", "main"),
            method="Strict homogeneous job guard; distribute 70 complete jobs evenly and split only the final job into five consecutive segments. "
                   "Two-pass bounded DP minimizes the maximum per-core M/V compute workload, then a boundary-byte proxy. "
                   "Singleton plan and no internal E0; workload/capacity proxies are not official score bounds.",
            references=["https://github.com/huaweibei123/huaweicup2026/blob/" +
                        m["identity"]["runner_commit"] + "/docs/a/q3-yuanzhifang/TAIL_MEASUREMENT.md"],
            upstream=[source(x["commit"], x["path"], "dependency")
                      for x in m["identity"]["implementation"] if x["path"] !=
                      "src/q3_yuanzhifang/wave_tail.py"])
        p["runner"] = dict(source=source(m["identity"]["runner_commit"],
            m.get("runner_source_path", "src/q3_yuanzhifang/tail_benchmark.py"), "main"),
        ...
        rows.append(row)
        summary.append(dict(problem=problem, case_id=case, cores=5, variant="wave_tail",
                            status=row["status"], **row["metrics"]))
    stamp = utc().replace("-", "").replace(":", "").split(".")[0] + "Z"
    feed_path = folder / f"board-feed-{stamp}-tail.json"
    if feed_path.exists():
