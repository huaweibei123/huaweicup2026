...
from active_benchmark import SOLVER_COMMIT, OUT
...
def export(folder):
    ...
    if m["status"] != "ok" and not any(call["status"] != "ok" for call in calls):
        work.append(dict(evaluation_id="044-active_stages-preflight", problem="P3",
                         call=dict(call_id="044-active_stages-preflight", kind="preflight", status="failed",
    ...
    for e in work:
        ...
        row.update(attempt_id=m["run_id"] + "-" + e["evaluation_id"], revision=1,
                   run_id=m["run_id"], algorithm_id="q3-active-stages", algorithm_name="共享链参与核心数构造",
                   variant="active_stages", solver_commit=SOLVER_COMMIT, status=call["status"],
                   observed_at=call["started_at"], source_url=TASK,
                   parameters=dict(cores=4, requested_cores=4,
                                   active_cores=c["detail"].get("active_cores") if c else None,
                                   resource_choices=c["detail"].get("choices") if c else None,
                                   deterministic=True, candidate_policy="one resource-selected active-core plan with same-stage order",
        ...
        run_path = folder / "044/active_stages" / problem / "run.json"
        ...
        p = row["provenance"]
        p["solver"].update(source=source(SOLVER_COMMIT, "src/q3_yuanzhifang/active_stages.py", "main"),
                           method="Strictly guard serial homogeneous jobs and common-weight consumer signatures. "
                                  "For at most requested_cores arithmetic resource formulas, select active cores by the minimum "
                                  "max(compute pipe load, all-miss zero-spill COPY service). Build one same-stage schedule and "
                                  "pad unused cores with empty schedules; requested_cores remains 4. These arithmetic models are "
                                  "inside measured solver wall; no timeline trial-scoring or online E0. The model is not a P3 lower bound.",
                           references=["https://github.com/huaweibei123/huaweicup2026/blob/" + SOLVER_COMMIT +
                                       "/docs/a/q3-yuanzhifang/CONTINUOUS_GOAL.md"],
        ...
        p["runner"] = dict(source=source(m["identity"]["runner_commit"],
                                         "src/q3_yuanzhifang/active_benchmark.py", "main"),
        ...
        rows.append(row)
        summary.append(dict(problem=problem, case_id="044", cores=4, variant="active_stages",
                            status=call["status"], **row["metrics"]))
    stamp = utc().replace("-", "").replace(":", "").split(".")[0] + "Z"
    feed = folder / f"board-feed-{stamp}-active.json"
    if feed.exists():
