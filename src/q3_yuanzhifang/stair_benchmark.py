"""Frozen 067 k5 first-wave capacity stair pilot; no online selection."""
...
DEPENDENCIES = {
    **{f"src/q3_yuanzhifang/{name}.py": SOLVER_COMMIT for name in (
        "wave_stair", "tail_stair_model", "tail_phase_model", "tail_fifo_bound",
        "wave_tail", "wave_capacity", "active_stages", "construct", "baseline")},
    "docs/a/q3-yuanzhifang/WAVE_STAIR.md": SOLVER_COMMIT,
}
...
def preflight(graph_dir):
    ...
    for name in ("src/q3_yuanzhifang/stair_benchmark.py",
                 "src/q3_yuanzhifang/stair_export.py",
                 "docs/a/q3-yuanzhifang/STAIR_MEASUREMENT.md"):
...
def audit_p3(graph, result, detail):
    ...
    checks = dict(
        shared_read_is_part_of_total=measured <= all_read_bytes,
    ...
    audit = dict(shared_tensor_count=len(shared), shared_original_total_bytes=sum(tensors[t]["size"] for t in shared),
                 shared_read_bytes=measured,
...
class StairPilot(Pilot):
    def __init__(self, args):
        ...
        self.info.update(schema="q3-wave-stair-v1", run_id="yuanzhifang-q3-wave-stair-20260925",
                         ...
                         "The fresh Python solver call includes graph read/indexing, strict guard, two-pass tail DP, beta/U/h stair, FIFO bound, construction, validation and plan write. "
    ...
    def case(self, case):
        cid = f"{case}-wave_stair"
        folder = self.out / case / "wave_stair"
        graph = self.args.graph_dir / f"case_{case}.json"
        plan = folder / f"case_{case}_multicore_res.json"
        python = relative(sys.executable)
        solver_argv = [python, "-B", "-m", "src.q3_yuanzhifang.wave_stair", graph.as_posix(),
        ...
        construction = dict(construction_id=cid, case_id=case, variant="wave_stair",
        ...
        with self.lock:
            ...
            self.save()
        if not detail.get("guard") or detail.get("selected") != "wave_stair":
            raise Stopped("stair structural guard failed; no E0 on fallback: " + cid)
        owners = {}
        ...
        if set(value["node_to_subgraph"]) != {str(u) for u in index.ops}:
            raise Stopped("eligible compute node identity mismatch")
        ...
        h, first, phase, beta, upper = (detail.get(name) for name in
                                        ("h", "first_wave_sizes", "phase", "beta", "U"))
        if (type(h) is not int or h < 1 or first != [h+c for c in range(5)] or
                not isinstance(phase, list) or len(phase) != 5 or
                not isinstance(beta, list) or len(beta) != len(jobs[0]) or
                not isinstance(upper, list) or len(upper) != 5):
            raise Stopped("stair metadata shape mismatch")
        expected_upper = [min(14, min(beta[p] - int(cuts[c] <= p < cuts[c+1])
                                      for p in range(len(jobs[0])))) for c in range(5)]
        if upper != expected_upper or h != min(upper[c] - c for c in range(5)):
            raise Stopped("stair U/h formula mismatch")
        inverse = {sg: int(u) for u, sg in value["node_to_subgraph"].items()}
        for core in range(5):
            a, b = cuts[core:core+2]
            n = first[core]
            if (not 1 <= n <= 14 or n > upper[core] or
                    any(n + int(a <= p < b) > beta[p] for p in range(len(jobs[0])))):
                raise Stopped("stair width/upper mismatch")
            sizes = phase[core].get("full_job_wave_sizes")
            if (phase[core].get("core") != core or not isinstance(sizes, list) or
                    not sizes or sizes[0] != n or sum(sizes) != 14 or
                    any(type(x) is not int or x < 1 or x > min(beta) for x in sizes[1:])):
                raise Stopped("stair wave metadata mismatch")
            remaining = 14 - n
            wave_count = (remaining + min(beta) - 1) // min(beta)
            balanced = ([remaining // wave_count + int(i < remaining % wave_count)
                         for i in range(wave_count)] if wave_count else [])
            if sizes[1:] != balanced:
                raise Stopped("stair later waves are not minimum balanced waves")
            group = [jobs[j] for j in range(core, 70, 5)]
            expected = []
            for p in range(len(jobs[0])):
                if a <= p < b:
                    expected.append(jobs[-1][p])
                expected.extend(job[p] for job in group[:n])
            offset = n
            for size in sizes[1:]:
                wave = group[offset:offset+size]
                offset += size
                for p in range(len(jobs[0])):
                    expected.extend(job[p] for job in wave)
            actual = [inverse[s] for s in value["core_schedules"][core]]
            if offset != 14 or actual != expected:
                raise Stopped("stair tail-first/position-major order mismatch")
        ...
        for problem in (2, 3):
            ...
            evaluation = dict(evaluation_id=eval_call["call_id"], construction_id=cid, case_id=case,
                variant="wave_stair", problem=f"P{problem}", makespan_cycles=result["makespan"],
    ...
    def run(self):
        ...
        try:
            ...
            self.save()
            threads = [threading.Thread(target=self.worker, name="stair-worker")]
            ...
            if self.info.get("failures") or self.queue or len(self.constructions) != 1 or len(self.evaluations) != 2:
                ...
                self.info.update(status="ok", stop_reason="one guarded stair plan and two external E0s completed")
