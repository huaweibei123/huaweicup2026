...
from gap_benchmark import SOLVER_COMMIT, OUT, VARIANT
...
def export(folder):
    ...
    if m["status"] != "ok" and not any(call["status"] != "ok" for call in calls):
        ...
        work.append(dict(evaluation_id=case + "-gap-preflight", case_id=case, problem="P3",
                         call=dict(call_id=case + "-gap-preflight", kind="preflight", status="failed",
    ...
    for e in work:
        ...
        row.update(attempt_id=m["run_id"] + "-" + e["evaluation_id"], revision=1,
                   ...
                   notes=["Independent 069 k5 gap-list development pilot, one cold construction and two E0s at most.",
        ...
        p["runner"] = dict(source=source(m["identity"]["runner_commit"],
                                         "src/q3_yuanzhifang/gap_benchmark.py", "main"),
    ...
    feed = folder / f"board-feed-{stamp}-gap-list.json"
    if feed.exists():
    ...
    aliases = [dict(case_id=c["case_id"], construction_id=c["construction_id"], plan=c["plan"],
                    ...
                    comparisons=c["control_comparisons"], reused_evaluations=c["reused_evaluations"])
               for c in m["constructions"] if c.get("alias_of")]
    write_json(folder / "alias-reuse.json", dict(schema="q3-gap-alias-v1", aliases=aliases,
