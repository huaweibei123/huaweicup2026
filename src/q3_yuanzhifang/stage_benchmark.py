"""One guarded 044/k4 stage candidate: one cold construction, two external E0s."""
...
SOLVER_COMMIT = "b4f7f67673bce89d3032042795c08a149bb68410"
HELPER_COMMIT = "39e9c8d8a78e384855ffbfbf41a2dec4d4a7e5a0"
OUT = "results/a/q3-yuanzhifang/stages-20260924"
...
def preflight(graph_dir):
    ...
    name = "src/q3_yuanzhifang/stages.py"
    ...
    for name in ("src/q3_yuanzhifang/stage_benchmark.py", "src/q3_yuanzhifang/stage_export.py"):
        if Path(name).read_bytes() != git("show", head + ":" + name):
            raise ValueError("stage runner/exporter must be committed")
    result["implementation"].append(dict(path="src/q3_yuanzhifang/stages.py",
                                        sha256=sha("src/q3_yuanzhifang/stages.py")))
...
class StagePilot(Pilot):
    def __init__(self, args):
        super().__init__(args)
        (self.out / ".gitattributes").write_bytes(b"# Preserve exact process output and receipt bytes.\n* -text\n")
        self.info.update(schema="q3-shared-stages-v1", run_id="yuanzhifang-q3-stages-20260924",
                         budget=BUDGET,
                         ...
                         "Parent performed static chain/signature/live-interval checks and synthetic structure tests "
    ...
    def run(self):
        try:
            ...
            self.save()
            folder = self.out / "044" / "shared_stages"
            graph = self.args.graph_dir / "case_044.json"
            plan = folder / "case_044_multicore_res.json"
            argv = [relative(sys.executable), "-B", "-m", "src.q3_yuanzhifang.stages",
                    graph.as_posix(), "--cores", "4", "--output", plan.as_posix()]
            call = self.invoke("solver", "044-shared_stages", argv, folder / "solver")
            ...
            c = dict(construction_id="044-shared_stages", case_id="044", variant="shared_stages", cores=4,
            ...
            self.save()
            if not detail.get("guard") or detail.get("selected") != "shared_stages":
            ...
            for problem in (2, 3):
                ...
                call = self.invoke("E0", f"044-shared_stages-P{problem}", argv, destination)
                ...
                e = dict(evaluation_id=call["call_id"], construction_id=c["construction_id"],
                         case_id="044", variant="shared_stages", problem=f"P{problem}",
