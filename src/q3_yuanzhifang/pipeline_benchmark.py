"""Frozen 044 k4 pipeline pilot: one cold construction, two external E0s."""
...
OUT = "results/a/q3-yuanzhifang/pipeline-20260924"
CASES = ("044",)
...
def preflight(graph_dir):
    ...
    for name in ("src/q3_yuanzhifang/pipeline_stages.py", "src/q3_yuanzhifang/active_stages.py",
                 "docs/a/q3-yuanzhifang/PIPELINE_STAGES.md"):
        if Path(name).read_bytes() != git("show", SOLVER_COMMIT + ":" + name):
            raise ValueError("pipeline source/spec differs from frozen commit: " + name)
    ...
    for name in ("src/q3_yuanzhifang/pipeline_benchmark.py", "src/q3_yuanzhifang/pipeline_export.py"):
        if Path(name).read_bytes() != git("show", head + ":" + name):
            raise ValueError("pipeline runner/exporter must be committed: " + name)
...
class PipelinePilot(Pilot):
    def __init__(self, args):
        super().__init__(args)
        (self.out / ".gitattributes").write_bytes(b"# Preserve exact process output and receipt bytes.\n* -text\n")
        self.info.update(schema="q3-pipeline-v1", run_id="yuanzhifang-q3-pipeline-20260924",
                         budget=BUDGET,
                         argv=[relative(sys.executable), "-B", relative(__file__),
                               "--graph-dir", args.graph_dir.as_posix(), "--output", args.output.as_posix()],
                         offline_costs="No training, compilation or precomputed plans. Parent reports two synthetic DP/structure "
                         "tests before freeze. Earlier 100-graph static structure study and prior 044 official trials informed "
                         "development; they are separately preserved development costs, not part of this new solver timing. "
                         "The new DP and input indexing run inside this measured cold solver. uv sync --locked preceded "
                         "all batches; preparation wall was unrecorded.")
    ...
    def invoke(self, kind, call_id, argv, folder):
        ...
        if elapsed >= BUDGET["dispatch_until_seconds"]:
            raise Stopped("pipeline dispatch cutoff reached; no new call")
        if sum(call["kind"] == kind for call in self.calls) >= limit:
            raise Stopped("pipeline call cap reached; no new call")
    ...
    def run(self):
        try:
            ...
            for case in CASES:
                ...
                cid = f"{case}-{VARIANT}"
                argv = [relative(sys.executable), "-B", "-m", "src.q3_yuanzhifang.pipeline_stages",
                ...
                if not detail.get("guard") or detail.get("selected") != VARIANT:
                    raise Stopped("pipeline guard failed; no E0 on fallback")
                for problem in (2, 3):
                    ...
                    self.save()
            self.info.update(status="ok", stop_reason="one guarded pipeline construction and two external E0s completed")
