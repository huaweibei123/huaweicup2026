...
from join_benchmark import SOLVER_COMMIT, OUT, VARIANT
...
def export(folder):
    ...
    if m["status"] != "ok" and not any(call["status"] != "ok" for call in calls):
        ...
        work.append(dict(evaluation_id=case + "-join-preflight", case_id=case, problem="P3",
                         call=dict(call_id=case + "-join-preflight", kind="preflight", status="failed",
    ...
    for e in work:
        ...
        row.update(attempt_id=m["run_id"] + "-" + e["evaluation_id"], revision=1,
                   ...
                   notes=["Independent 069/071 k5 join-aware DAG development pilot, two cold constructions and four E0s at most.",
        ...
        receipt = dict(schema="q3-join-attempt-v1", solver_commit=SOLVER_COMMIT,
        ...
        p["runner"] = dict(source=source(m["identity"]["runner_commit"],
                                         "src/q3_yuanzhifang/join_benchmark.py", "main"),
    ...
    feed = folder / f"board-feed-{stamp}-join-list.json"
    if feed.exists():
