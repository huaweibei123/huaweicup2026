...
from gap_generalization_benchmark import SOLVER_COMMIT, OUT, VARIANT
...
def export(folder):
    ...
    if m.get("schema") != "q3-gap-generalization-v1" or m.get("status") == "running":
        raise ValueError("requires an ended generalization batch")
    ...
    if m["status"] != "ok" and not any(call["status"] != "ok" for call in calls):
        case = m.get("active_case_id", "071")
        work.append(dict(evaluation_id=case + "-gap-generalization-preflight", case_id=case, problem="P3",
                         call=dict(call_id=case + "-gap-generalization-preflight", kind="preflight", status="failed",
    ...
    for e in work:
        ...
        row.update(attempt_id=m["run_id"] + "-" + e["evaluation_id"], revision=1,
                   run_id=m["run_id"], algorithm_id="q3-dag-join-gap-list", algorithm_name="持久管线日历的空隙插入联合分配",
                   ...
                   parameters=dict(cores=5, requested_cores=5, active_cores=c["active_cores"] if c else None,
                                   deterministic=True, candidate_policy="one chain/JOIN list construction with persistent per-pipe earliest-gap reservation",
                                   guard="original M/V with nonnegative integer cycles; chain-DAG real eligible dependencies match; fork and join exist",
                   ...
                   notes=["Independent 071/005/086 k5 generalization of the unchanged gap implementation, three cold constructions and six E0s at most; 069 is not rerun.",
                          ...
                          "Three public generalization graphs; this is not a full100 result, blind review or independent acceptance."])
        ...
        receipt = dict(schema="q3-gap-attempt-v1", solver_commit=SOLVER_COMMIT,
        ...
        p = row["provenance"]
        p["solver"].update(source=source(SOLVER_COMMIT, "src/q3_yuanzhifang/gap_list.py", "main"),
                           method="Condense maximal serial chains and choose ready chains by remaining path. "
                                  "For each arithmetic core placement, reserve each operation at its earliest fitting "
                                  "per-pipe idle interval in a persistent augmented AVL calendar. Price the last "
                                  "predecessor with its released join over at most k^2 placements, commit only the "
                                  "chosen versions, then order singleton priorities by model start. No internal "
                                  "E0, parameter sweep or official COPY/cache/capacity simulation. Local earliest "
                                  "placement does not prove globally optimal or nondegrading official Makespan.",
                           references=["https://github.com/huaweibei123/huaweicup2026/blob/" + SOLVER_COMMIT +
                                       "/docs/a/q3-yuanzhifang/GAP_LIST.md"],
                           upstream=[source(SOLVER_COMMIT, "src/q3_yuanzhifang/gap_calendar.py", "earliest"),
                                     source("071d538ddae05ceda519d3b1b0844e987da908d5",
        ...
        p["runner"] = dict(source=source(m["identity"]["runner_commit"],
                                         "src/q3_yuanzhifang/gap_generalization_benchmark.py", "main"),
        ...
        p["measurement"].update(started_at=solver["started_at"] if solver else m["started_at"],
                                ...
                                budget=dict(wall_seconds=240, candidate_limit=3, stop_reason=m["stop_reason"]),
    ...
    feed = folder / f"board-feed-{stamp}-gap-generalization.json"
    if feed.exists():
    ...
    write_json(folder / "export-receipt.json", dict(exported_at=utc(), feed=artifact(feed), records=len(rows),
               ...
               note="Only template structure reused; all graph/plan/solver/result identities from gap-list receipts. Zero new calls."))
