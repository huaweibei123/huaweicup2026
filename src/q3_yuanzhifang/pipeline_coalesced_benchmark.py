"""Frozen coalesced pipeline: 044 k4, one cold construction and at most two E0s."""
...
SOLVER_COMMIT = "6e620b5e005a281003d516ce070228934e8a03af"
HELPER_COMMIT = "39e9c8d8a78e384855ffbfbf41a2dec4d4a7e5a0"
OUT = "results/a/q3-yuanzhifang/pipeline-coalesced-20260924"
CASES = ("044",)
VARIANT = "pipeline_coalesced"
...
def control_reference(case, identity, graph_dir):
    ...
    return dict(case_id=case, cores=4, original_commit=CONTROL_COMMIT, package_commit=CONTROL_COMMIT,
        ...
        interpretation="Fixed same-core-ownership pipeline control; no control rerun or latest-best claim.")
...
def preflight(graph_dir):
    ...
    for name in ("src/q3_yuanzhifang/pipeline_coalesced.py", "src/q3_yuanzhifang/pipeline_stages.py",
                 "src/q3_yuanzhifang/active_stages.py", "src/q3_yuanzhifang/construct.py",
                 "src/q3_yuanzhifang/baseline.py", "docs/a/q3-yuanzhifang/PIPELINE_COALESCED.md"):
        if Path(name).read_bytes() != git("show", SOLVER_COMMIT + ":" + name):
            raise ValueError("coalesced pipeline source/spec differs from frozen commit: " + name)
    ...
    for name in ("src/q3_yuanzhifang/pipeline_coalesced_benchmark.py", "src/q3_yuanzhifang/pipeline_coalesced_export.py",
                 "docs/a/q3-yuanzhifang/PIPELINE_COALESCED_MEASUREMENT.md"):
        if Path(name).read_bytes() != git("show", head + ":" + name):
            raise ValueError("pipeline-coalesced runner/exporter must be committed: " + name)
...
class PipelineCoalescedPilot(Pilot):
    def __init__(self, args):
        super().__init__(args)
        (self.out / ".gitattributes").write_bytes(b"# Preserve exact process output and receipt bytes.\n* -text\n")
        self.info.update(schema="q3-pipeline-coalesced-v1", run_id="yuanzhifang-q3-pipeline-coalesced-20260924",
                         budget=BUDGET,
                         ...
                         memory_checks=[],
                         offline_costs="No training, precomputed plan or real-graph construction in this preparation. "
                         "Parent reports three synthetic tests: forward-stage quotient/core preservation, guard fallback, "
                         "and frozen official priority-bucket semantics on a supplied synthetic raw sequence. "
                         "These are development costs and do not establish actual Step1/COPY order. "
                         "The cold solver includes input reading, guarded chain indexing, one O(k L^2) contiguous "
                         "pipeline partition, stage coalescing, official static quotient validation, output and exit. "
    ...
    def invoke(self, kind, call_id, argv, folder):
        ...
        if elapsed >= BUDGET["dispatch_until_seconds"]:
            raise Stopped("pipeline-coalesced dispatch cutoff reached; no new call")
        if sum(call["kind"] == kind for call in self.calls) >= limit:
            raise Stopped("pipeline-coalesced call cap reached; no new call")
        memory = memory_gate()
    ...
    def run(self):
        try:
            ...
            for case in CASES:
                ...
                cid = f"{case}-{VARIANT}"
                argv = [relative(sys.executable), "-B", "-m", "src.q3_yuanzhifang.pipeline_coalesced",
                ...
                if not detail.get("guard") or detail.get("selected") != VARIANT:
                    raise Stopped("pipeline-coalesced guard failed; no E0 on fallback")
                ...
                mismatch = sorted(op for op in set(old_owners) | set(new_owners)
                                  if old_owners.get(op) != new_owners.get(op))
                c["historical_control_comparison"] = dict(bytes_equal=plan.read_bytes() == old_plan,
                    ...
                    core_ownership_equal=not mismatch, operations_checked=len(new_owners),
                    ownership_mismatch_ops=mismatch,
                    ...
                    new_subgraphs=len(set(value["node_to_subgraph"].values())),
                    old_subgraphs=len(set(old_value["node_to_subgraph"].values())),
                    quotient_static_validation=dict(passed=True, scope="inside the successful cold solver before plan write",
                    ...
                    note="Every submitted op checked against the frozen control; no second construction or control evaluation. "
                         "Static quotient validity does not establish memory feasibility, M/V FIFO, COPY order or quality.")
                self.save()
                if mismatch:
                    raise Stopped("core ownership differs from the fixed pipeline control; no E0")
                if c["historical_control_comparison"]["full_two_field_json_equal"]:
                    raise Stopped("coalescing produced an unchanged full plan; preserve cold result, no duplicate E0")
                for problem in (2, 3):
                    ...
                    self.save()
            self.info.update(status="ok", stop_reason="one guarded coalesced pipeline construction and two external E0s completed")
...
def main():
    ...
    return PipelineCoalescedPilot(args).run()
