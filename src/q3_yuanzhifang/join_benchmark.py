"""Frozen 069/071 k5 join-aware DAG pilot: two cold constructions, four external E0s."""
...
OUT = "results/a/q3-yuanzhifang/join-list-20260924"
CASES = ("069", "071")
...
def preflight(graph_dir):
    ...
    for name in ("src/q3_yuanzhifang/join_benchmark.py", "src/q3_yuanzhifang/join_export.py"):
...
class JoinPilot(Pilot):
    def __init__(self, args):
        super().__init__(args)
        (self.out / ".gitattributes").write_bytes(b"# Preserve exact process output and receipt bytes.\n* -text\n")
        self.info.update(schema="q3-join-list-v1", run_id="yuanzhifang-q3-join-list-20260924",
                         budget=BUDGET,
                         argv=[relative(sys.executable), "-B", relative(__file__),
                               "--graph-dir", args.graph_dir.as_posix(), "--output", args.output.as_posix()],
                         offline_costs="No training, compilation or precomputed plans. Parent reports two synthetic mechanism "
                         "tests before freeze. Prior DAG results and four-result static pipe/COPY diagnosis informed "
                         "development; these remain separately preserved development costs. Per-input chain condensation "
                         "and at most k^2 arithmetic placements per released join occur inside the cold solver timing. "
                         "uv sync --locked preceded all batches; environment preparation wall unrecorded.")
        ...
        try:
            ...
            for case in CASES:
                ...
                for problem in (2, 3):
                    ...
                    self.save()
            self.info.update(status="ok", stop_reason="two guarded join-aware DAG constructions and four external E0s completed")
