"""Export sealed pipeline-family batch artifacts; this module never evaluates plans."""
...
from pipeline_family_benchmark import SOLVER_COMMIT, OUT, VARIANT, CASES, TEMPLATE
...
def export(folder):
    ...
    if m.get("schema") != "q3-pipeline-family-v1" or m.get("status") not in ("ok", "stopped"):
    ...
    for e in work:
        ...
        row.update(attempt_id=m["run_id"] + "-" + e["evaluation_id"], revision=1,
            ...
            notes=["Six public structural-family k5 cases, up to six cold constructions and twelve external E0s.",
                   "One fresh construction is shared by each P2/P3 pair; count unique calls in sealed ledger.",
        ...
        p["solver"].update(source=source(SOLVER_COMMIT,
            ...
            references=["https://github.com/huaweibei123/huaweicup2026/blob/" +
                        m["identity"]["runner_commit"] + "/docs/a/q3-yuanzhifang/PIPELINE_FAMILY.md"],
        ...
        p["runner"] = dict(source=source(m["identity"]["runner_commit"],
            "src/q3_yuanzhifang/pipeline_family_benchmark.py", "main"),
        ...
        if success and problem == "P3":
            paired = next((x for x in m["evaluations"] if x["case_id"] == case and x["problem"] == "P2"), None)
            if paired:
