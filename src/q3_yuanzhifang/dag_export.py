...
from dag_benchmark import SOLVER_COMMIT, OUT, VARIANT
...
def export(folder):
    ...
    if m["status"] != "ok" and not any(call["status"] != "ok" for call in calls):
        ...
        work.append(dict(evaluation_id=case + "-dag-preflight", case_id=case, problem="P3",
                         call=dict(call_id=case + "-dag-preflight", kind="preflight", status="failed",
    ...
    for e in work:
        ...
        row.update(attempt_id=m["run_id"] + "-" + e["evaluation_id"], revision=1,
                   run_id=m["run_id"], algorithm_id="q3-dag-chain-list", algorithm_name="串行链凝聚与释放时间感知分核",
                   ...
                   parameters=dict(cores=5, requested_cores=5, active_cores=c["active_cores"] if c else None,
                                   deterministic=True, candidate_policy="one communication-aware chain-DAG list schedule",
                   ...
                   notes=["Independent 069/071 k5 DAG development pilot, two cold constructions and four E0s at most.",
                          ...
                          "Two public development graphs; this is not a full100 result, blind review or independent acceptance."])
        ...
        receipt = dict(schema="q3-dag-attempt-v1", solver_commit=SOLVER_COMMIT,
        ...
        p = row["provenance"]
        p["solver"].update(source=source(SOLVER_COMMIT, "src/q3_yuanzhifang/dag_list.py", "main"),
                           method="Guard edge agreement and fork/join structure. Condense maximal serial chains, then "
                                  "assign ready chains by remaining-path priority to the core with earliest modeled finish, "
                                  "using per-pipe clocks and static all-miss cross-edge transfer estimates. Build one plan and "
                                  "validate its structure. No E0, capacity or shared COPY-pool simulation inside the model; "
                                  "the model is not an official duration or a general lower bound.",
        ...
        p["runner"] = dict(source=source(m["identity"]["runner_commit"],
                                         "src/q3_yuanzhifang/dag_benchmark.py", "main"),
        ...
        p["measurement"].update(started_at=solver["started_at"] if solver else m["started_at"],
                                ...
                                budget=dict(wall_seconds=180, candidate_limit=2, stop_reason=m["stop_reason"]),
    ...
    feed = folder / f"board-feed-{stamp}-dag-list.json"
    if feed.exists():
    ...
    write_json(folder / "export-receipt.json", dict(exported_at=utc(), feed=artifact(feed), records=len(rows),
               ...
               note="Only template structure reused; all graph/plan/solver/result identities from DAG receipts. Zero new calls."))
