"""Export sealed 067 wave-mode artifacts; no constructor or evaluator calls."""
...
from wave_benchmark import SOLVER_COMMIT, OUT, MODES, CASES, TEMPLATE
...
def export(folder):
    ...
    if m.get("schema") != "q3-wave-v1" or m.get("status") not in ("ok", "stopped"):
        raise ValueError("requires an ended wave comparison batch")
    ...
    for failure_index, failure in enumerate(m.get("failures", []), 1):
        ...
        attempted = [x for x in calls if x["call_id"].startswith(f"{case}-wave_{failure['mode']}")]
        ...
        if last is None or last["status"] == "ok":
            ...
            construction_id = f"{case}-wave_{failure['mode']}"
    ...
    for e in work:
        ...
        if success and problem == "P3":
            ...
            if (audit_state is None or not audit_state.get("passed")) and not any(f["case_id"] == case and f["mode"] == mode
        ...
        row.update(attempt_id=m["run_id"] + "-" + e["evaluation_id"], revision=1,
            run_id=m["run_id"], algorithm_id="q3-wave-capacity", algorithm_name="完整作业容量波次构造",
            variant=f"wave_{mode}", solver_commit=SOLVER_COMMIT, status="ok" if success else call["status"],
            ...
            parameters=dict(cores=5, requested_cores=5,
                ...
                mode=mode, candidate_policy="fixed full/capacity comparison; one direct wave construction per mode, no E0 selection",
                guard="complete homogeneous jobs and private tensor signatures, unique common consumer, positive M/V, jobs>=cores",
            ...
            notes=["067 k5 research comparison: one independent cold plan per prescribed mode, up to four external E0s.",
                   "P2/P3 within each mode share its one construction; new calls counted once in sealed ledger.",
                   "One worker; full and capacity are independent fixed modes, not an online best-of-two solver.",
        ...
        if success and problem == "P3":
            ...
            row["notes"].append("P3 post-E0 shared-read/cross-core audit " +
        ...
        receipt = dict(schema="q3-wave-attempt-v1", run_id=m["run_id"],
            ...
            note="P2/P3 share a single cold construction. Full batch counts are in the sealed ledger.")
        destination = folder / case / f"wave_{mode}" / (problem if success else "failed-" + call["call_id"])
        ...
        p["solver"].update(source=source(SOLVER_COMMIT,
            "src/q3_yuanzhifang/wave_capacity.py", "main"),
            method="Strict homogeneous complete-job guard; evenly distribute jobs across five cores. "
                   "Full uses one wave per core, capacity derives one bmax from W/A/F static headroom and balanced waves. "
                   "Singleton plan and no internal E0; model footprint and shared-read bound are conditional, not official score bounds.",
            references=["https://github.com/huaweibei123/huaweicup2026/blob/" +
                        m["identity"]["runner_commit"] + "/docs/a/q3-yuanzhifang/WAVE_MEASUREMENT.md"],
        ...
        rows.append(row)
        summary.append(dict(problem=problem, case_id=case, cores=5, variant=f"wave_{mode}",
                            status=row["status"], **row["metrics"]))
    stamp = utc().replace("-", "").replace(":", "").split(".")[0] + "Z"
    feed_path = folder / f"board-feed-{stamp}-wave.json"
    if feed_path.exists():
