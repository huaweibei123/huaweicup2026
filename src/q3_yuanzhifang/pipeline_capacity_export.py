"""Export preserved capacity-pipeline pilot bytes; never launch an evaluator."""
...
from pipeline_capacity_benchmark import SOLVER_COMMIT, OUT, VARIANT
...
def export(folder):
    ...
    if m.get("schema") != "q3-pipeline-capacity-v1" or m.get("status") == "running":
        raise ValueError("requires an ended capacity pipeline batch")
    ...
    if m["status"] != "ok" and not any(call["status"] != "ok" for call in calls):
        ...
        work.append(dict(evaluation_id=case + "-pipeline-capacity-preflight", case_id=case, problem="P3",
                         call=dict(call_id=case + "-pipeline-capacity-preflight", kind="preflight", status="failed",
    ...
    for e in work:
        ...
        row.update(attempt_id=m["run_id"] + "-" + e["evaluation_id"], revision=1,
                   run_id=m["run_id"], algorithm_id="q3-pipeline-capacity", algorithm_name="容量约束的共同输入连续流水阶段DP",
                   ...
                   parameters=dict(cores=4, requested_cores=4, active_cores=c["active_cores"] if c else None,
                                   deterministic=True, candidate_policy="one O(k L^2) contiguous cold-setup partition constrained by modeled L1/UB shared residence plus maximum one-operation noncommon footprint",
                                   guard="homogeneous serial jobs; positive integer M/V cycles; each shared input consumed at one position; supported L1/UB tensors; feasible capacity split; jobs>=cores and length<=512",
                   ...
                   notes=["Independent 044 k4 capacity pipeline pilot: at most one cold construction and two E0s, no control rerun.",
        ...
        receipt = dict(schema="q3-pipeline-capacity-attempt-v1", solver_commit=SOLVER_COMMIT,
        ...
        p = row["provenance"]
        p["solver"].update(source=source(SOLVER_COMMIT, "src/q3_yuanzhifang/pipeline_capacity.py", "main"),
                           method="Guard homogeneous positive M/V serial chains with each shared input consumed "
                                  "at one position. Price each shared tensor once as ceil(bytes/bandwidth). "
                                  "Solve an O(k L^2), O(k L) cold-setup stage DP with explicit L1/UB constraints: "
                                  "stage shared-input residence plus largest single-operation noncommon footprint "
                                  "must fit configured capacity. Submit one contiguous singleton plan. "
                                  "Prefetch and overlapping lifetimes remain outside this working-set model; "
                                  "it does not certify zero spills or bound official E0 Makespan. No internal E0.",
                           references=["https://github.com/huaweibei123/huaweicup2026/blob/" + SOLVER_COMMIT +
                                       "/docs/a/q3-yuanzhifang/PIPELINE_CAPACITY.md"],
                           upstream=[source("6bae8dfa317bc71226068344b59dd65d2612c32b",
                                            "src/q3_yuanzhifang/pipeline_stages.py", "build"),
                                     source("f14e8f0ae38033e83d9f497301595b7568a55cb6",
                                            "src/q3_yuanzhifang/pipeline_setup.py", "partition"),
        ...
        p["runner"] = dict(source=source(m["identity"]["runner_commit"],
                                         "src/q3_yuanzhifang/pipeline_capacity_benchmark.py", "main"),
    ...
    feed = folder / f"board-feed-{stamp}-pipeline-capacity.json"
    if feed.exists():
    ...
    write_json(folder / "alias-reuse.json", dict(schema="q3-pipeline-capacity-alias-v1", aliases=aliases,
    ...
    write_json(folder / "export-receipt.json", dict(exported_at=utc(), feed=artifact(feed), records=len(rows),
               ...
               note="Only template structure reused; all graph/plan/solver/result identities from pipeline-capacity receipts. Zero new calls."))
