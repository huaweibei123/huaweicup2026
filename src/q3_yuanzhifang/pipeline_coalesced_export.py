"""Export only preserved coalesced pipeline pilot bytes; never launch a solver or evaluator."""
...
from pipeline_coalesced_benchmark import SOLVER_COMMIT, OUT, VARIANT
...
def export(folder):
    ...
    if m.get("schema") != "q3-pipeline-coalesced-v1" or m.get("status") == "running":
        raise ValueError("requires an ended coalesced pipeline batch")
    ...
    if m["status"] != "ok" and not any(call["status"] != "ok" for call in calls):
        ...
        work.append(dict(evaluation_id=case + "-pipeline-coalesced-preflight", case_id=case, problem="P3",
                         call=dict(call_id=case + "-pipeline-coalesced-preflight", kind="preflight", status="failed",
    ...
    for e in work:
        ...
        row.update(attempt_id=m["run_id"] + "-" + e["evaluation_id"], revision=1,
                   run_id=m["run_id"], algorithm_id="q3-pipeline-coalesced", algorithm_name="流水分核保持与阶段子图合并",
                   ...
                   parameters=dict(cores=4, requested_cores=4, active_cores=c["active_cores"] if c else None,
                                   deterministic=True, candidate_policy="one contiguous pipeline partition, then one subgraph per nonempty stage with unchanged op-to-core ownership",
                                   guard="homogeneous serial jobs with identical common-input signatures; jobs>=cores and chain length<=512",
                   ...
                   notes=["Independent 044 k4 coalesced pipeline pilot: at most one cold construction and two E0s, no control rerun.",
        ...
        receipt = dict(schema="q3-pipeline-coalesced-attempt-v1", solver_commit=SOLVER_COMMIT,
        ...
        p = row["provenance"]
        p["solver"].update(source=source(SOLVER_COMMIT, "src/q3_yuanzhifang/pipeline_coalesced.py", "main"),
                           method="Apply the guarded homogeneous-chain contiguous pipeline partition once, with "
                                  "O(k L^2) split complexity and L<=512. Retain every original operation's core, "
                                  "then merge each nonempty stage into one subgraph. Official Step1 chooses "
                                  "within-stage order. Static derive checks quotient legality; no internal E0, "
                                  "COPY/cache/capacity simulation or candidate sweep. This deliberately changes "
                                  "priority buckets and cannot inherit the singleton plan's M/V FIFO, no-spill "
                                  "observation, model bound or quality. Post-cold ownership check uses the fixed original plan.",
                           references=["https://github.com/huaweibei123/huaweicup2026/blob/" + SOLVER_COMMIT +
                                       "/docs/a/q3-yuanzhifang/PIPELINE_COALESCED.md"],
        ...
        p["runner"] = dict(source=source(m["identity"]["runner_commit"],
                                         "src/q3_yuanzhifang/pipeline_coalesced_benchmark.py", "main"),
    ...
    feed = folder / f"board-feed-{stamp}-pipeline-coalesced.json"
    if feed.exists():
    ...
    write_json(folder / "export-receipt.json", dict(exported_at=utc(), feed=artifact(feed), records=len(rows),
               ...
               note="Only template structure reused; all graph/plan/solver/result identities from pipeline-coalesced receipts. Zero new calls."))
