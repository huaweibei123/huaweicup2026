"""Frozen 067 k5 full/capacity wave comparison; no online selection."""
...
SOLVER_COMMIT = "2cf325fb717077cd902830e7f01c920d27cbf8ec"
HELPER_COMMIT = "39e9c8d8a78e384855ffbfbf41a2dec4d4a7e5a0"
RUNTIME_HELPER_COMMIT = "60477a382514cb6e97bfd8ea1bb560fda1594c06"
OUT = "results/a/q3-yuanzhifang/wave-20260925"
CASES = ("067",)
MODES = ("full", "capacity")
BUDGET = dict(workers=1, cold_solver_limit=2, E0_limit=4, E1_limit=0,
              E2_limit=0, per_call_seconds=60, batch_seconds=300,
              dispatch_until_seconds=240, retries=0)
TEMPLATE = "results/a/q3-yuanzhifang/pilot-20260924/board-feed-20260924T141108Z-unique-plans.json"
DEPENDENCIES = {
    "src/q3_yuanzhifang/wave_capacity.py": SOLVER_COMMIT,
    "docs/a/q3-yuanzhifang/WAVE_CAPACITY.md": SOLVER_COMMIT,
...
def preflight(graph_dir):
    ...
    for name in ("src/q3_yuanzhifang/wave_benchmark.py",
                 "src/q3_yuanzhifang/wave_export.py",
                 "docs/a/q3-yuanzhifang/WAVE_MEASUREMENT.md"):
...
def audit_p3(graph, result, detail):
    ...
    bound = detail["conditional_shared_read_bound_bytes"]
    cross = result.get("cross_core_transfers")
    stats = result.get("cache_stats", {})
    expected_coverage = {(core, tid) for core in range(detail["active_cores"]) for tid in shared}
    checks = dict(
        shared_identity_bytes=sum(tensors[t]["size"] for t in shared) == detail["shared_weight_bytes"],
        cache_accounting=all_read_bytes == stats.get("hit_bytes", -1) + stats.get("miss_bytes", -1),
        all_core_shared_coverage=covered == expected_coverage,
        no_cross_core_transfer=isinstance(cross, list) and not cross,
        conditional_read_bound=measured <= bound,
    )
    audit = dict(shared_tensor_count=len(shared), shared_read_bytes=measured,
                 conditional_bound_bytes=bound, by_tensor_id=bytes_by_id,
...
class WavePilot(Pilot):
    def __init__(self, args):
        super().__init__(args)
        (self.out / ".gitattributes").write_bytes(b"# Preserve exact process/receipt bytes.\n* -text\n")
        self.lock = threading.RLock()
        self.stop = threading.Event()
        self.queue = deque((case, mode) for case in CASES for mode in MODES)
        self.info.update(schema="q3-wave-v1", run_id="yuanzhifang-q3-wave-20260925",
                         ...
                         "Each fresh Python solver call includes graph read/indexing, structure guard, fixed-mode wave construction, validation and plan write. "
                         "Modes are prescribed independently; no online E0 selection. Prior pipeline 067 P2/P3 at c514edf0 is historical only.")
    ...
    def case(self, case, mode):
        cid = f"{case}-wave_{mode}"
        folder = self.out / case / f"wave_{mode}"
        graph = self.args.graph_dir / f"case_{case}.json"
        plan = folder / f"case_{case}_multicore_res.json"
        python = relative(sys.executable)
        solver_argv = [python, "-B", "-m", "src.q3_yuanzhifang.wave_capacity", graph.as_posix(),
                       "--cores", "5", "--config", CONFIG.as_posix(), "--mode", mode,
        ...
        construction = dict(construction_id=cid, case_id=case, variant=f"wave_{mode}", mode=mode,
        ...
        with self.lock:
            ...
            self.save()
        if not detail.get("guard") or detail.get("selected") != "wave_capacity" or detail.get("mode") != mode:
            raise Stopped("wave structural guard/mode failed; no E0 on fallback: " + cid)
        owners = {}
        ...
        if any(len({owners[value["node_to_subgraph"][str(u)]] for u in job}) != 1
               for job in index.components):
            raise Stopped("complete job split across cores")
        for problem in (2, 3):
            ...
            evaluation = dict(evaluation_id=eval_call["call_id"], construction_id=cid, case_id=case,
                variant=f"wave_{mode}", problem=f"P{problem}", makespan_cycles=result["makespan"],
            ...
            if problem == 3:
                ...
                if not evaluation["p3_audit"]["passed"]:
                    raise Stopped("P3 shared-read/cross-core audit failed: " + json.dumps(evaluation["p3_audit"]))
        ...
        while True:
            with self.lock:
                if self.stop.is_set() or not self.queue:
                    return
                case, mode = self.queue.popleft()
                self.info["active_case_ids"] = [f"{case}-{mode}"]
                self.save()
            try:
                self.case(case, mode)
                ...
                with self.lock:
                    self.stop.set()
                    self.info.setdefault("failures", []).append(dict(case_id=case, mode=mode, type=type(exc).__name__, reason=str(exc)))
                    self.save()
            finally:
                with self.lock:
                    self.info["active_case_ids"].remove(f"{case}-{mode}")
                    self.save()
    ...
    def run(self):
        ...
        try:
            ...
            self.save()
            threads = [threading.Thread(target=self.worker, name="wave-worker")]
            for thread in threads:
                thread.start()
            for thread in threads:
                thread.join()
            if self.info.get("failures") or self.queue or len(self.constructions) != 2 or len(self.evaluations) != 4:
                self.info.update(status="stopped", stop_reason=self.info.get("failures", [{}])[0].get(
                    "reason", "batch ended before all prescribed calls"))
            else:
                self.info.update(status="ok", stop_reason="two guarded wave plans and four external E0s completed")
...
def main():
    ...
    if args.check_only:
        ...
        print(json.dumps(dict(verified_files=len(identity["verified_files"]), solver_commit=SOLVER_COMMIT,
            ...
            planned_calls=dict(solver=2, E0=4, E1=0, E2=0), calls_executed=0)))
        return 0
    if args.concurrent_work is None:
        ap.error("actual run requires --concurrent-work")
    return WavePilot(args).run()
