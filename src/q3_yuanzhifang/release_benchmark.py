"""Frozen 069 k5 release-order pilot: one cold construction, at most two external E0s."""
...
SOLVER_COMMIT = "9c8bd47a53f3d68f4deefd460dbc8a361092f808"
HELPER_COMMIT = "39e9c8d8a78e384855ffbfbf41a2dec4d4a7e5a0"
OUT = "results/a/q3-yuanzhifang/release-order-20260924"
CASES = ("069",)
VARIANT = "dag_join_release_order"
...
CONTROL_COMMIT = "ef80a88ed04b26e70465d56a54c373b0dcc5d00f"
CONTROL_MANIFEST = "results/a/q3-yuanzhifang/join-list-20260924/manifest.json"
CONTROL_FEED = "results/a/q3-yuanzhifang/join-list-20260924/board-feed-20260924T160841Z-join-list.json"
RUNTIME_HELPER_COMMIT = "60477a382514cb6e97bfd8ea1bb560fda1594c06"


def fixed_control_artifact(ref):
    path = ROOT / ref["path"]
    raw = path.read_bytes()
    if raw != git("show", CONTROL_COMMIT + ":" + ref["path"]) or digest(raw) != ref["sha256"]:
        raise ValueError("fixed JOIN control artifact differs: " + ref["path"])
    if path.suffix == ".gz":
...
def control_reference(identity, graph_dir):
    old = json.loads(fixed_control_artifact(artifact(CONTROL_MANIFEST)))
    feed = json.loads(fixed_control_artifact(artifact(CONTROL_FEED)))
    ...
    for e in old["evaluations"]:
        ...
        if row["identity"]["plan_sha256"] != c["plan"]["sha256"] or row["artifacts"]["result"]["sha256"] != e["artifacts"]["result"]["sha256"]:
            raise ValueError("JOIN control feed points to different bytes")
    ...
    if set(evaluations) != {"P2", "P3"}:
        raise ValueError("JOIN control requires both frozen P2/P3 results")
    return dict(commit=CONTROL_COMMIT, manifest=artifact(CONTROL_MANIFEST), feed=artifact(CONTROL_FEED),
...
def preflight(graph_dir):
    ...
    for name in ("src/q3_yuanzhifang/release_order.py", "src/q3_yuanzhifang/join_list.py", "src/q3_yuanzhifang/dag_list.py",
        ...
        if Path(name).read_bytes() != git("show", SOLVER_COMMIT + ":" + name):
            raise ValueError("release-order source/spec differs from frozen commit: " + name)
    ...
    for name in ("src/q3_yuanzhifang/release_benchmark.py", "src/q3_yuanzhifang/release_export.py"):
        if Path(name).read_bytes() != git("show", head + ":" + name):
            raise ValueError("release-order runner/exporter must be committed: " + name)
    ...
    result["baseline_refs"] = {case: baseline(case, dict(
        ...
        for case in CASES}
    result["control"] = control_reference(result, graph_dir)
    return result


class ReleasePilot(Pilot):
    def __init__(self, args):
        super().__init__(args)
        (self.out / ".gitattributes").write_bytes(b"# Preserve exact process output and receipt bytes.\n* -text\n")
        self.info.update(schema="q3-release-order-v1", run_id="yuanzhifang-q3-release-order-20260924",
                         budget=BUDGET,
                         argv=[relative(sys.executable), "-B", relative(__file__),
                               "--graph-dir", args.graph_dir.as_posix(), "--output", args.output.as_posix()],
                         offline_costs="No training, compilation or precomputed candidate plan. Two synthetic release-order "
                         "correctness tests and 18 passing suite tests are development work reported by the parent. "
                         "Prior JOIN result/trace diagnosis informed this mechanism; prior costs remain separately recorded. "
                         "The cold solver includes JOIN construction, augmented-DAG earliest starts, stable cross-pipe "
                         "sorting and validation. uv sync --locked preceded this batch; preparation wall unrecorded.")
    ...
    def invoke(self, kind, call_id, argv, folder):
        ...
        if elapsed >= BUDGET["dispatch_until_seconds"]:
            raise Stopped("release-order dispatch cutoff reached; no new call")
        if sum(call["kind"] == kind for call in self.calls) >= limit:
            raise Stopped("release-order call cap reached; no new call")
    ...
    def run(self):
        try:
            ...
            for case in CASES:
                ...
                cid = f"{case}-{VARIANT}"
                argv = [relative(sys.executable), "-B", "-m", "src.q3_yuanzhifang.release_order",
                ...
                if not detail.get("guard") or detail.get("selected") != VARIANT:
                    raise Stopped("release-order guard failed; no E0 on fallback")
                control = self.info["identity"]["control"]
                old_plan = fixed_control_artifact(control["plan"])
                c["control_comparison"] = dict(bytes_equal=plan.read_bytes() == old_plan,
                    full_two_field_json_equal=value == json.loads(old_plan), control_commit=CONTROL_COMMIT,
                    control_plan=control["plan"], rule="Complete parsed mapping and ordered per-core schedules; no hash-only or proxy comparison.")
                if c["control_comparison"]["full_two_field_json_equal"]:
                    c["alias_of"] = dict(commit=CONTROL_COMMIT, run_id=control["run_id"],
                                         construction_id=control["construction_id"])
                ...
                for problem in (2, 3):
                    ...
                    self.save()
            reason = ("one cold construction; identical JOIN plan reused with zero new E0s"
                      if any(c["alias_of"] for c in self.constructions)
                      else "one guarded release-order construction and two external E0s completed")
...
def main():
    ...
    if args.check_only:
        ...
        print(json.dumps(dict(verified_files=len(identity["verified_files"]), solver_commit=SOLVER_COMMIT,
                              ...
                              control_commit=CONTROL_COMMIT, control_problems=list(identity["control"]["evaluations"]))))
        return 0
    return ReleasePilot(args).run()
