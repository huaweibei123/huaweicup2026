"""Second P2 batch: certified-bound pruning with fixed first-batch comparison.
...
SOLVER = '405024df0532b72e916306399278d5ad6c2413c7'
FIRST_COMMIT = '5e5d688ca473a80b7d72b26b397ce568ac70f534'
FIRST_RUN = Path('results/a/q2-nikolastarx/feedback-20260924/run')
REPO = 'huaweibei123/huaweicup2026'
SESSION = 'nikolastarx/s-8ee33b891eb94c529bf5be94bb5d8894'
PREFIX = Path('results/a/q2-nikolastarx/pruning-20260924')
...
def verify_inputs(runner_commit):
    ...
    assert protocol['prune_bounds'] is True and protocol['comparison_data_commit'] == FIRST_COMMIT
    assert protocol['comparison_run'] == FIRST_RUN.as_posix()
...
def execute(output, runner_commit):
    ...
    batch = {'started_at': utc(), 'status': 'running', 'solver_commit': SOLVER,
             ...
             'command': ['.venv/bin/python', '-B', '-m', 'src.q2_nikolastarx.evaluate_pruned',
    ...
    try:
        for case in protocol['cases']:
            ...
            solver_argv = ['.venv/bin/python', '-B', '-m', 'src.q2_nikolastarx.solve', str(graph),
                           ...
                           '--wall', str(protocol['solver_wall_seconds']), '--prune-bounds']
            ...
            if row['solver']['status'] != 'ok' or ledger.get('status') != 'ok':
                ...
                if row['final']['status'] == 'ok':
                    ...
                    previous_plan = git_bytes(FIRST_COMMIT, FIRST_RUN / case / 'plan.json')
                    previous_result = gzip.decompress(git_bytes(
                        FIRST_COMMIT, FIRST_RUN / case / 'final/result.json.gz'))
                    row['first_batch_comparison'] = {
                        'commit': FIRST_COMMIT,
                        'plan_bytes_equal': (folder / 'plan.json').read_bytes() == previous_plan,
                        'full_result_bytes_equal': (final / 'result.json').read_bytes() == previous_result,
                        'prior_plan_sha256': digest(previous_plan),
                        'prior_raw_result_sha256': digest(previous_result),
                        'candidate_plan_bytes_equal': {
                            name: (folder / 'online' / name / 'plan.json').read_bytes() ==
                            git_bytes(FIRST_COMMIT, FIRST_RUN / case / 'online' / name / 'plan.json')
                            for name in METHODS}}
...
def export(output):
    ...
    records, summary = [], []
    before = json.loads(git_bytes(FIRST_COMMIT, FIRST_RUN.parent / 'board-before.json'))
    previous = {c['case_id']: c['best']['metrics']['makespan_cycles'] for c in before['cells']}
    run_id = 'nikolastarx-q2-structural-pruned-' + batch['started_at'].replace('-', '').replace(':', '').split('.')[0]
    for row in batch['cases']:
        ...
        provenance = {
            ...
            'solver': {'source': source(SOLVER, 'src/q2_nikolastarx/solve.py', 'main'),
                       'authors': ['NikolaStarx'],
                       'method': 'Same from-graph four proposals and strict E0 incumbent selection; skip an already constructed candidate only if its certified assigned-core pipe-work lower bound cannot strictly improve the incumbent.',
                       'references': [f'https://github.com/{REPO}/blob/{SOLVER}/docs/a/q2-nikolastarx/PRUNING.md'],
            ...
            'runner': {'source': source(batch['runner_commit'], 'src/q2_nikolastarx/evaluate_pruned.py', 'execute'),
        ...
        record = {'attempt_id': f'{run_id}-p2-{case}-k4', 'revision': 1, 'run_id': run_id,
                  ...
                  'variant': 'e0-protected-four-constructors-bound-pruned', 'solver_commit': SOLVER,
                  'parameters': {'protocol': protocol, 'selected': ledger.get('selected'),
                                 ...
                                 'sample_status': 'same six development cases; runtime pruning check, not new holdout evidence',
                                 'first_batch_comparison': row.get('first_batch_comparison'),
                  ...
                  'artifacts': artifacts, 'runtime_id': 'nikolastarx-q2-pruning-macos27-arm64-py31213',
                  ...
                  'notes': ['Independent evaluator worker shares parent development context; not a blind scientific review.',
                            ...
                            'All 24 candidate plans remain in run/online. Bound-pruned entries carry certificates and have no invented E0 result or duration. Compressed official JSON is lossless; raw hashes are in batch.compression.',
        ...
        summary.append({'case': case, 'status': row['status'], 'candidates': [
            ...
            'first_batch_comparison': row.get('first_batch_comparison'),
            'bound_pruned': [{k: a.get(k) for k in ('name', 'assigned_pipe_lower_bound_cycles', 'incumbent_name', 'incumbent_makespan_cycles', 'prune_reason')}
                             for a in attempts if a['status'] == 'bound_pruned']})
