"""P2 component/pipe constructors; priorities are NOT official start times.

Adapted from team Q3 construct.py at a4e7ee13310d693ec4fb5cc236669ceb3b172d1f.
Resource-word/affine originate in archived Pro2; added pipe-ready fallback and
P2 applicability diagnostics. See docs/a/q2-nikolastarx/FEEDBACK_METHOD.md.
"""
...
class Index:
    ...
    def pipe_ready(self, nodes):
        """List schedule of the compute DAG with per-pipe heaps.

        Each ready operation has a fixed dependency release. Pipe availability
        only increases, so a per-pipe release heap and four-way head comparison
        avoid rescoring every ready vertex after every dispatch. A dispatch
        appends a priority, not an official event or a proof about COPY/spill.
        """
        node_set = set(nodes)
        degree = {u: len(self.pred[u] & node_set) for u in nodes}
        release = dict.fromkeys(nodes, 0)
        tail = {}
        for u in reversed(self.order):
            if u in node_set:
                tail[u] = self.duration(u) + max(
                    (tail[v] for v in self.succ[u] if v in node_set), default=0)
        queues = {}
        available = {}

        def push(u):
            pipe = self.ops[u]['pipe']
            available.setdefault(pipe, 0)
            heapq.heappush(queues.setdefault(pipe, []), (release[u], -tail[u], u))
        ...
        while any(queues.values()):
            ...
            for pipe, queue in queues.items():
                if queue:
                    rel, neg_tail, u = queue[0]
                    choices.append((max(available[pipe], rel), neg_tail, u, pipe))
            start, _, u, pipe = min(choices)
            heapq.heappop(queues[pipe])
            end = start + self.duration(u)
            available[pipe] = end
            sequence.append(u)
            for v in self.succ[u]:
                if v in node_set:
                    release[v] = max(release[v], end)
                    degree[v] -= 1
                    if degree[v] == 0:
                        push(v)
    ...
    def sequences(self, cores, strategy):
        ...
        if strategy == "guarded_reentry":
            try:
                self.word_descriptor()
                strategy = "resource_word"
        ...
        for jobs in groups:
            ...
            if strategy == "component":
                seq = nodes
            elif strategy == "pipe_ready":
                seq = self.pipe_ready(nodes)
        ...
        return sequences, {**detail, 'selected_strategy': strategy}
...
def main():
    ...
    parser.add_argument("--strategy", choices=("component", "affine_eighth", "resource_word", "pipe_ready", "guarded_reentry"),
                        default="component")
