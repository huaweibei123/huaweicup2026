...
OUTPUT_LOCK_GUARD = threading.Lock()
HELD_OUTPUT_LOCKS = set()
HARNESS_SHA = None
CONTROL_DIR = ".control"
FATAL_STOP_NAME = "fatal-stop.json"
OUTPUT_LOCK_NAME = "controller.lock"
...
class OutputDirectoryBusy(RuntimeError):
    """Another controller owns the output directory or left an active run."""
...
def fatal_stop_path(out):
    return Path(out) / CONTROL_DIR / FATAL_STOP_NAME


def check_running(roots=None):
    if STOP.is_set():
        raise FatalRunStop("benchmark stopped after a fatal worker error")
    if roots and roots.get("OUTPUT") is not None:
        marker = fatal_stop_path(roots["OUTPUT"])
        if marker.exists():
            try:
                created_at = read(marker).get("created_at", "unknown time")
            except Exception as error:
                raise FatalRunStop(f"unreadable persistent fatal-stop marker: {marker}") from error
            raise FatalRunStop(
                f"persistent fatal-stop marker from {created_at}; confirm cleanup before removing {marker}"
            )
...
@contextmanager
def output_directory_lock(out):
    """Hold one controller lock for this output directory until the run ends."""
    out = Path(out).resolve()
    control = out / CONTROL_DIR
    control.mkdir(parents=True, exist_ok=True)
    lock_path = control / OUTPUT_LOCK_NAME
    key = os.path.normcase(str(lock_path))
    with OUTPUT_LOCK_GUARD:
        if key in HELD_OUTPUT_LOCKS:
            raise OutputDirectoryBusy(f"output directory is already locked: {out}")
        HELD_OUTPUT_LOCKS.add(key)
    handle = None
    acquired = False
    try:
        ...
        try:
            ...
            raise OutputDirectoryBusy(f"another controller owns output directory: {out}") from error
        acquired = True
        yield lock_path
    finally:
        try:
            ...
            if handle is not None:
                handle.close()
            with OUTPUT_LOCK_GUARD:
                HELD_OUTPUT_LOCKS.discard(key)


def ensure_output_startable(out):
    """Fail closed on a prior fatal cleanup or an unresolved controller run."""
    out = Path(out)
    check_running({"OUTPUT": out})
    invocation_dir = out / "invocations"
    if invocation_dir.exists():
        for path in sorted(invocation_dir.glob("*.json")):
            try:
                status = read(path).get("status")
            except Exception as error:
                raise OutputDirectoryBusy(f"cannot verify prior invocation: {path}") from error
            if status == "running":
                raise OutputDirectoryBusy(f"unresolved running invocation blocks resume: {path}")
...
def persist_fatal_stop(roots, folder, receipt):
    """Record the first unconfirmed cleanup so future controllers fail closed."""
    out = roots.get("OUTPUT")
    if out is None:
        raise RuntimeError("OUTPUT root is required to persist a fatal cleanup stop")
    marker = fatal_stop_path(out)
    if not marker.exists():
        save(marker, {
            "status": "fatal_cleanup",
            "created_at": utc(),
            "harness_sha256": HARNESS_SHA,
            "pid": receipt.get("pid"),
            "process_receipt": portable(Path(folder) / "process.json", roots),
            "cleanup_error": receipt.get("cleanup_error"),
            "clearance": "Remove only after independently confirming the recorded process tree is gone.",
        })
    return marker
            ...
            try:
                ...
                try:
                    ...
                    with LAUNCH_LOCK:
                        STOP.set()
                        try:
                            marker = persist_fatal_stop(roots, folder, receipt)
                            receipt["fatal_stop_marker"] = portable(marker, roots)
                        except Exception as persistence_error:
                            receipt["fatal_stop_persistence_error"] = portable(
                                f"{type(persistence_error).__name__}: {persistence_error}", roots
                            )
...
def pin_protocol(out, protocol, revision_note):
    ...
    if not protocol_path.exists():
        existing = [path for path in out.iterdir() if path.name != CONTROL_DIR]
        if existing:
            raise ValueError("existing benchmark artifacts have no original protocol; use a new run directory")
