"""Frozen one-cell P1 development batch; one process at a time, no retry.
...
RESULT_ROOT = ROOT / "results/a/q1-bounded-probe-20260924"
...
RUNNER_PATH = "src/q1_benchmarks/bounded_probe_e0.py"
...
def run(batch):
    ...
    meta = {"run_id": batch.name, "solver_commit": SOLVER, "runner_commit": head,
            ...
            "concurrency_context": "P2 and Q3 scoring reported stopped; no exclusive-host reservation or controlled timing claim",
    ...
    with tempfile.TemporaryDirectory(prefix="q1-tree-input-") as tmp:
        ...
        for case, cores in CELLS:
            ...
            try:
                r["calls"]["solver"] = 1
                r["solver"] = process([sys.executable, "-B", SOLVER_PATH, graph, "--output", plan, "--cores", cores, "--packet-factor", "4", "--trigger-ops", "4096", "--chunk-ops", "1024", "--diagnostics", diagnostics], folder, "solver", 30, inputs)
...
def export(batch):
    ...
    for case, cores in CELLS:
        ...
        record = {"attempt_id": f"nikolastarx-{batch.name}-P1-{case}-k{cores}-r0", "revision": 1, "run_id": batch.name,
            ...
            "solver_commit": SOLVER, "parameters": {"cores": cores, "packet_factor": 4, "trigger_ops": 4096, "chunk_ops": 1024, "base_selected": r.get("diagnostics", {}).get("base", {}).get("selected"), "constructor_timeout_seconds": 30, "evaluation_timeout_seconds": 60, "candidate_limit": 1,
            ...
            "provenance": {"producer_session": SESSION, "task_url": TASK,
                "solver": {"source": source(SOLVER, SOLVER_PATH, "main"), "authors": ["NikolaStarx"],
                    "method": "Apply fixed frontier factor4, then split only Tasks above4096 compute ops along internal independent components, decreasing-size first-fit chunks of1024; indivisible large components remain intact; no scoring or search",
            ...
            "notes": ["One predeclared independent Task-chunk mechanism test on case014, new algorithm and budget; no online search or full-100 validation.",
                      "P2 and Q3 scoring reported stopped; no exclusive-host reservation or controlled timing claim.",
