...
RESULT_ROOT = ROOT / "results/a/q1-tree-fine-20260924"
...
RUNNER_PATH = "src/q1_benchmarks/tree_fine_e0.py"
...
def run(batch):
    ...
    with tempfile.TemporaryDirectory(prefix="q1-tree-input-") as tmp:
        ...
        for case, cores in CELLS:
            ...
            try:
                r["calls"]["solver"] = 1
                r["solver"] = process([sys.executable, "-B", SOLVER_PATH, graph, "--output", plan, "--cores", cores, "--packet-factor", "4", "--diagnostics", diagnostics], folder, "solver", 30, inputs)
...
def export(batch):
    ...
    for case, cores in CELLS:
        ...
        record = {"attempt_id": f"nikolastarx-{batch.name}-P1-{case}-k{cores}-r0", "revision": 1, "run_id": batch.name,
            ...
            "solver_commit": SOLVER, "parameters": {"cores": cores, "packet_factor": 4, "selected": r.get("diagnostics", {}).get("selected"), "constructor_timeout_seconds": 30, "evaluation_timeout_seconds": 60, "candidate_limit": 1,
            ...
            "notes": ["One predeclared packet-factor=4 mechanism ablation; no online parameter search, full-100 validation or independent algorithm acceptance.",
