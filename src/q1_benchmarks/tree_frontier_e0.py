"""Frozen four-cell P1 development batch; one process at a time, no retry.

`run RUN_ID` starts at most 4 constructors and 4 independent official E0 calls.
...
RESULT_ROOT = ROOT / "results/a/q1-tree-frontier-20260924"
SOLVER = "40c3c093d15ac6bd57f1f0b06472c8a09e1e7d79"
...
CELLS = [(c, 4) for c in ("002", "008", "010", "051")]
SOLVER_PATH = "src/q1/tree_frontier.py"
RUNNER_PATH = "src/q1_benchmarks/tree_frontier_e0.py"
...
def run(batch):
    ...
    meta = {"run_id": batch.name, "solver_commit": SOLVER, "runner_commit": head,
            ...
            "started_at": utc(), "finished_at": None, "cells": CELLS, "maximum_calls": {"solver": 4, "E0": 4, "E1": 0, "E2": 0},
    ...
    meta.update(finished_at=utc(), status="stopped" if stopped else "complete", stop_reason=stopped or "all four frozen cells complete")
...
def export(batch):
    ...
    for case, cores in CELLS:
        ...
        record = {"attempt_id": f"nikolastarx-{batch.name}-P1-{case}-k{cores}-r0", "revision": 1, "run_id": batch.name,
            ...
            "solver_commit": SOLVER, "parameters": {"cores": cores, "selected": r.get("diagnostics", {}).get("selected"), "constructor_timeout_seconds": 30, "evaluation_timeout_seconds": 60, "candidate_limit": 1,
            ...
            "notes": ["Four predeclared development cells; not full-100 validation or independent algorithm acceptance.",
