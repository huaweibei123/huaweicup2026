"""Frozen two-cell P1 sink-peel development batch; one process at a time, no retry.

`run RUN_ID` starts at most 2 constructor and 2 independent official E0 calls.
...
RESULT_ROOT = ROOT / "results/a/q1-sink-probe-20260924"
SOLVER = "d89a6cbf1e292f2abc36b4fef89590d16ccb1ab2"
...
RUNNER_PATH = "src/q1_benchmarks/sink_probe_e0.py"
...
def verify():
    ...
    for path, commit in ((SOLVER_PATH, SOLVER), (CLI_PATH, head), ("src/q1/bounded_tasks.py", SOLVER), ("src/q1/tree_frontier.py", SOLVER), ("src/q1/component_pack.py", SOLVER), (RUNNER_PATH, head)):
...
def run(batch):
    ...
    meta = {"run_id": batch.name, "solver_commit": SOLVER, "runner_commit": head,
            ...
            "started_at": utc(), "finished_at": None, "cells": CELLS, "maximum_calls": {"solver": 2, "E0": 2, "E1": 0, "E2": 0},
            ...
            "stop_policy": "one active cell; stop at first unexpected failure; no retry; constructor 30s, E0 60s, total180s including input preparation",
    ...
    with tempfile.TemporaryDirectory(prefix="q1-tree-input-") as tmp:
        ...
        for case, cores in CELLS:
            ...
            if time.monotonic() >= deadline and not stopped:
                stopped = "Batch180s wall deadline exceeded"
            if stopped:
            ...
            try:
                r["calls"]["solver"] = 1
                r["solver"] = process([sys.executable, "-B", CLI_PATH, graph, "--output", plan, "--cores", cores, "--max-rounds", "64", "--max-sinks", "64", "--diagnostics", diagnostics], folder, "solver", min(30, max(.001, deadline - time.monotonic())), inputs)
    ...
    meta.update(finished_at=utc(), status="stopped" if stopped else "complete", stop_reason=stopped or "two frozen cells complete")
...
def export(batch):
    ...
    for case, cores in CELLS:
        ...
        record = {"attempt_id": f"nikolastarx-{batch.name}-P1-{case}-k{cores}-r0", "revision": 1, "run_id": batch.name,
            ...
            "solver_commit": SOLVER, "parameters": {"cores": cores, "max_rounds": 64, "max_sinks": 64, "base_packet_factor": 4, "base_trigger_ops": 4096, "base_chunk_ops": 1024, "batch_timeout_seconds": 180, "selected": r.get("diagnostics", {}).get("selected"), "constructor_timeout_seconds": 30, "evaluation_timeout_seconds": 60, "candidate_limit": 1,
            ...
            "provenance": {"producer_session": SESSION, "task_url": TASK,
                "solver": {"source": source(SOLVER, SOLVER_PATH, "construct"), "authors": ["NikolaStarx"],
                    "method": "Use bounded04 fallback, peel sink-exclusive suffix packets from the active DAG, reverse waves and per-wave pipe-work LPT packing; max64 rounds/sinks; no scoring or search",
                    "references": [f"https://github.com/{REPO}/blob/{SOLVER}/docs/a/Q1_SINK_PEEL.md"], "upstream": [source(SOLVER, "src/q1/bounded_tasks.py", "construct"), source(meta["runner_commit"], CLI_PATH, "main")],
            ...
            "notes": ["Two predeclared sink-exclusive suffix mechanism tests on048/071k4; no online search or full-100 validation; base construct called once inside each solver call and included in wall time.",
