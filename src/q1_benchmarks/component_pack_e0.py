"""Frozen eight-cell P1 development batch; one process at a time, no retry.

`run RUN_ID` starts at most 8 constructors and 8 independent official E0 calls.
...
RESULT_ROOT = ROOT / "results/a/q1-component-pack-20260924"
SOLVER = "2cf3951af50be1e35ff78acb91629f2d0207bd82"
...
CELLS = [(c, 4) for c in ("002", "020", "044", "045", "080", "097")] + [("020", 1), ("044", 1)]
SOLVER_PATH = "src/q1/component_pack.py"
RUNNER_PATH = "src/q1_benchmarks/component_pack_e0.py"
...
def verify():
    ...
    for path, commit in ((SOLVER_PATH, SOLVER), (RUNNER_PATH, head)):
...
def run(batch):
    ...
    meta = {"run_id": batch.name, "solver_commit": SOLVER, "runner_commit": head,
            ...
            "started_at": utc(), "finished_at": None, "cells": CELLS, "maximum_calls": {"solver": 8, "E0": 8, "E1": 0, "E2": 0},
            ...
            "concurrency_context": "Other P2/Q3 sessions may each run one worker concurrently; no claim of exclusive-host timing",
    ...
    stopped = None
    with tempfile.TemporaryDirectory(prefix="q1-component-input-") as tmp:
        inputs = Path(tmp)
        t0 = time.perf_counter()
        ...
        for case, cores in CELLS:
            ...
            try:
                r["calls"]["solver"] = 1
                r["solver"] = process([sys.executable, "-B", SOLVER_PATH, graph, "--output", plan, "--cores", cores, "--diagnostics", diagnostics], folder, "solver", 30, inputs)
                ...
                r["evaluation"] = process([sys.executable, "-B", OFFICIAL / "code/multicore_cut_evaluate_problem_1.py", graph,
                    ...
                    "--trace-output", folder / "trace.json", "--log-output", folder / "official.log"], folder, "E0", 60, inputs)
    ...
    meta.update(finished_at=utc(), status="stopped" if stopped else "complete", stop_reason=stopped or "all eight frozen cells complete")
...
def export(batch):
    ...
    for case, cores in CELLS:
        ...
        record = {"attempt_id": f"nikolastarx-{batch.name}-P1-{case}-k{cores}-r0", "revision": 1, "run_id": batch.name,
            "algorithm_id": "q1-component-pack", "algorithm_name": "P1 complete-component pipe-load packing", "variant": "pipe-lpt-one-task-per-core",
            "solver_commit": SOLVER, "parameters": {"cores": cores, "constructor_timeout_seconds": 30, "evaluation_timeout_seconds": 60, "candidate_limit": 1,
            ...
            "provenance": {"producer_session": SESSION, "task_url": TASK,
                "solver": {"source": source(SOLVER, SOLVER_PATH, "main"), "authors": ["NikolaStarx"],
                    "method": "COPY-contracted weak components; per-pipe LPT placement; one Task per used core; original op-map order retained; no scoring or search",
                    "references": [f"https://github.com/{REPO}/blob/{SOLVER}/docs/a/Q1_COMPONENT_PACK.md"], "upstream": [],
            ...
            "notes": ["Eight predeclared development cells; not full-100 validation or independent algorithm acceptance.",
                      "Other P2/Q3 sessions may each run one worker concurrently; observed wall times do not claim exclusive-host controlled performance.",
        ...
        m = r["makespan_cycles"]
        comparisons.append({"case": case, "cores": cores, "status": r["status"], **metrics,
