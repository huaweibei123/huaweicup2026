...
def _build_scene_a_tasks_indexed(graph_json, plan, bandwidth, capacity, *, runtime=_runtime):
    """Equivalent task builder using one graph-wide boundary indexing pass."""
    plan_view = runtime.derive_multicore_plan(graph_json, plan)
    runtime.validate_task_order(plan_view)
    ...
    producers, consumers, direct_edges = runtime._original_tensor_views(graph_json)
    ...
    for task_id in plan_view["subgraph_ids"]:
        ...
        task_graph_copy_traffic += runtime._copy_traffic_bytes(graph)
        seq = runtime.step1_schedule(graph)
        result2 = runtime.step2_spill_insertion(graph, seq, capacity=capacity)
        spill_copy_traffic += sum(
            spill["size"] * (1 + int(spill["spill_out_copies_data"]))
            for spill in result2["spill_records"]
        )
        ext_graph = runtime._build_extended_graph(graph, result2)
        prepared = runtime.prepare_step3_execution(
    ...
    original_copy_traffic = runtime._copy_traffic_bytes(graph_json)
