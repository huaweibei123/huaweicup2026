"""Internal prepared-task bridge to the explicitly supplied native backend.

Adapted from research/a/native-replay-20260924/replay_api.py at
03f02e79de4b4bd6f55241385664b154f4332454, source SHA-256
6f3f9aaf278059351e87a84163f582880dcd43245f52d179a3fd40d00383998d.
Packing, candidate validation, and replay buffers are preserved, with additional
integer-width checks before ctypes/NumPy narrowing and explicit internal errors.
Library discovery/loading belongs to the caller; importing this module does not
load a dynamic library. No library or prepared-task cache is maintained here.

pack_tasks accepts ACTUAL Step3 prepared tasks, or explicitly synthetic fixtures.
Compiled is the prototype's mutable container, not a production immutable handle
or an official validation boundary. Official graph/plan/Step1--3 checks remain
the caller's responsibility. Returned timing arrays are not full official JSON.
Only native status 6 is an unsupported environment; other nonzero statuses are
execution failures and must not be silently converted into a successful fallback.
"""
...
class NativeExecutionError(RuntimeError):
    """A nonzero native execution status other than unsupported rounding mode."""

    def __init__(self, status, message=None):
        self.status = status
        super().__init__(message or f'native replay failed with status={status}')

@dataclass
class Compiled:
    """Prototype container; array flags do not make the whole object immutable."""
...
def pack_tasks(tasks, runtime, bandwidth):
    ...
    if len(ix)!=len(keys):
        raise NativeExecutionError(None, 'duplicate prepared-task keys')
    if len(keys)>=2**31 or len(tids)>=2**29:
        raise Unsupported('32-bit index domain exceeded')
    ...
    for t in tids:
        ...
        for p,order in task['pipe_ops'].items():
            if p not in PIPES: raise NativeExecutionError(None, 'unknown prepared-task pipe')
    ...
    if len(succ)>2**31-1 or len(tp)>2**31-1 or any(x>2**31-1 for x in off+tp_off):
        raise Unsupported('32-bit adjacency offset domain exceeded')
...
def score(comp, orders, *, library, same_wait=100, cross_wait=1000, max_iter=1_000_000, audit=False, project_once=False):
    """Costs include candidate validation, fresh buffers, ctypes and native replay.

    The caller supplies an explicitly loaded library with the Input/Output replay
    signature configured. Returns dense op/task timings, not full official JSON.
    """
    ...
    if any(x>2**63-1 for x in (same_wait,cross_wait,max_iter)):
        raise Unsupported('signed 64-bit wait/max_iter domain exceeded; use E1')
    ...
    status=library.replay(ct.byref(inp),ct.byref(out))
    if status == 6:
        raise Unsupported('native status=6; round-to-nearest environment required; use E1')
    if status:
        raise NativeExecutionError(status)
