...
\begin{document}
\hypersetup{pdfsubject={Paper review v8}}
\maketitle
\begin{center}\small 审阅稿 v8\end{center}
\begin{abstract}
面向多核神经网络处理器的计算图调度需求，本文研究子图划分、核心分配与执行顺序的协同优化。评测以最小化仿真完成时间（Makespan）为首要目标，兼顾外部主存（DDR）数据搬运量，并将端到端求解耗时作为算法效率指标。赛题三问对应不同的芯片存储与执行规则；本文依据图拓扑结构构造少量候选方案，在适用条件成立的分支引入理论下界辅助剪枝，并由模拟器在受限预算内比较完整执行时间择优。算法在全部 100 张基准图与 1 至 5 核心（三题共 1500 种测试组合）上均生成了合法方案。

\textbf{针对问题一：} 在子图独立封装为任务、跨任务数据必须经外部主存中转的场景下，本文通过连通分量与拓扑前沿生成\textbf{少量结构化候选并实施多级细化}；通过\textbf{桥树分解}识别可迁移的独立计算分支在核心间重分配；基于\textbf{拓扑高度波次}严格保证执行时序无环，消除跨核循环等待。5 核心场景下，100 张基准图相对官方单核基准的逐用例\textbf{平均加速比达到 4.0553}。

\textbf{针对问题二：} 在同一核心内子图合并为单一任务、允许私有缓存跨子图复用数据的场景下，本文协同优化计算分配与数据流转。针对片上缓存容量受限问题，利用流水线空闲区间紧凑编排计算；建立\textbf{超图连接度模型}作为划分引起的数据跨核心复制量代理（区别于最终额外 DDR 搬运量）；在二核心局部调整中设计\textbf{受流水线工作量上限保护的最小割方法}，防止算力向单核倾斜。5 核心场景下，100 张基准图相对官方单核基准的逐用例\textbf{平均加速比达到 4.5498}。

\textbf{针对问题三：} 在同一核心内子图合并并引入共享只读缓存的场景下，本文协同优化算子顺序与缓存利用。针对多分支树状归约结构，在子树不交错且内部开销固定的受限模型下，证明\textbf{按子树内部峰值与保留输出之差（\(h_i - r_i\)）降序排列}可使模型内临时结果存储峰值达到最小；构建\textbf{基于硬件流水线必要不等式的时间下界}快速剪枝，在至多 3 次在线评测预算内按 Makespan 严格改善选优。5 核心场景下，100 张基准图相对官方单核基准的逐用例\textbf{平均加速比达到 4.7576}；针对同一方案开展同配置成对仿真，开启只读缓存相比关闭缓存的\textbf{逐用例成对平均加速比达到 1.0829}，平均字节命中率为 \textbf{31.36\%}。

在共享多核主机环境下，三道题目各 500 种测试组合的端到端平均求解墙钟时间分别为 \textbf{P1 2.825 秒}、\textbf{P2 4.029 秒}与 \textbf{P3 4.401 秒}。测试所报告的方案完成时间为离散事件仿真时钟周期，求解耗时为通用计算机上的物理墙钟时间，数据反映基准仿真推进结果，而非物理芯片板级实测。
\keywords{计算图切分；多核调度；微架构仿真；超图最小割；树状存储排序；理论下界}
\end{abstract}
\section{问题重述与分析}
...
\caption{计算操作的分组、分配与调度方案表示}\label{tab:01-problem-1}\small
\begin{tabular}{@{}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.3333}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.3333}}@{}}
\caption{主要符号}\label{tab:02-assumptions-1}\\
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
符号
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
为了在合理的时间内产出高质量方案，本文在方法论层面对三个问题的求解策略进行了通用流程归纳：
...
\item
  \textbf{条件支持的下界评估与剪枝}：在具备确定性下界推导的特定算法分支中（如问题三中的流水线计算下界），根据必要时序约束计算完成时间下界，排除在 Makespan 指标上绝无可能超越当前最优解的候选方案；
\item
  \textbf{受限预算下的离散事件模拟选优}：对通过筛选的少量优质候选方案，调用离散事件评测程序获取模拟性能指标，自适应输出 Makespan 最小的方案。
\end{enumerate}

图 \ref{fig:framework} 概括了上述五步求解流程的通用方法逻辑。需要明确的是，上述流程属于对各题求解思想的抽象归纳，在软件实现上，三个问题对应三个独立且已固化的求解器程序，其入口脚本、候选生成逻辑与在线评价工具均不相同（问题一入口为 \texttt{\seqsplit{src.q1.branch\_refine}}，在线调用至多 10 次轻量评估器 E1；问题二入口为 \texttt{\seqsplit{src.q2\_nikolastarx.adaptive\_hypergap\_guarded}}，在线调用至多 3 次原生评估器 E2；问题三入口为 \texttt{\seqsplit{src.q3.forest\_solve}}，各阶段共享至多 3 次官方仿真器 E0）。贯穿全篇的核心设计准则体现为三点：\textbf{以结构化构造配合拓扑无环检查}、\textbf{以图特征约束有限候选数量}，以及\textbf{在紧凑的在线评测预算内以仿真结果闭环选优}。具体基于标号偏序的无环性证明留待后文命题展开；理论下界剪枝仅在具备严格证明条件的分支中启用，不声称三道题目统一实施了硬性下界剪枝。
...
\subsection{相关调度与编译优化研究对比}

多处理器任务调度与深度学习编译优化领域的研究为本文提供了重要的理论参照，但在问题设定与优化自由度上存在明确区别：

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{深度学习编译优化与搜索空间控制}：张洪滨等提出的 AutoConfig 机制\cite{ref13}通过综合静态特征与动态运行开销来缩减自动调优搜索空间；高伟等在 TVM 框架下\cite{ref14}利用预定义规则与代价模型裁剪算子调度空间。上述工作体现了在编译优化中兼顾方案质量与搜索耗时的平衡理念。与端到端代码生成系统不同，本赛题的提交自由度严格限定于官方接口约定的算子划分、核心分配与执行顺序三个维度，不涉及算子内部循环变换或底层微内核代码生成；
\item
  \textbf{经典多处理器与异构计算调度}：在异构 DAG 调度领域，HEFT 算法\cite{ref15}通过静态计算与通信成本预估建立任务优先级与最早完成时间排程，是经典启发式基准。然而，HEFT 依赖静态确定的传输延迟模型，无法直接适应本题中多核心共享 DDR 时的动态总线争用、片上私有缓存容量限制以及复杂的事件驱动推进机制；
\item
  \textbf{理想化车间模型与理论边界}：在运筹调度领域，Drobouchevitch 与 Strusevich 针对两机重入车间模型\cite{ref16}证明了特定工件流向下的多项式时间 \(4/3\) 近似界。该类经典理论有助于理解反复进出执行资源的排队时序边界，但其理想化假设无法覆盖本题包含多类硬件执行流水线（Cube/Vector/MTE）、共享只读 Cache 及离散事件仲裁的实际执行环境，其理论近似比不能直接迁移至本题；
\item
  \textbf{神经网络处理器调度与存储约束}：针对 NPU 的调度研究（如 LATTICE\cite{ref17}）表明，执行次序的调整与片上存储规划存在密切的相互影响。在问题设定上，本文的决策空间严格限定于算子划分、分核指派与执行次序三个维度，不能直接复用针对底层物理存储块的联合规划方法；各求解器中采用的流水线工作量上限或树模型启发式亦不能代替片上缓存的容量证明，方案最终的合法性与完成时间仍由本赛题的官方离散事件评测器统一核定。
\end{enumerate}
...
L(P) \ge U,
\end{equation} 则必然有 \(M_q(P) \ge U\)，该候选在完成时间维度绝无可能严格超越当前已知最优解 \(P_0\)。需要指出的是，该判定仅排除在 Makespan 维度不可能严格改善的方案，并不等同于过滤了所有劣质解；若在相同周期时允许通过次要目标（减少额外搬运量 \(D_q(P)\)）选优，则在 \(L(P) = U\) 的边界情形下，下界无法直接排除该候选产生更小搬运量的可能性。在实际算法实现中，全局理论下界并未作为三个问题统一执行的硬性搜索剪枝器，而是在具备确定下界推导的特定算法分支（如问题三中的流水线计算下界）中作为辅助判据；各题主要通过轻量级结构校验与有限高质量候选策略来规避无效搜索。

若 \(L\) 为全局下界，\(\mathrm{OPT} = \min_{P \in \mathcal{F}_q(G, K)} M_q(P)\) 为当前用例在给定核心数下的未知数学最优值，而 \(U\) 为算法所得最佳方案的实际 Makespan，三者在量纲上均为 cycle 且满足 \(L \le \mathrm{OPT} \le U\)。当 \(L > 0\) 时，同量纲相除得到无量纲的最优性相对间隙上界：

\begin{equation}
\frac{U - \mathrm{OPT}}{\mathrm{OPT}} \le \frac{U - L}{L} = \frac{U}{L} - 1.
\end{equation}

比值 \(U/L\) 在数学上给出了算法当前方案偏离未知理论最优解的保守相对误差上限 \(\frac{U}{L} - 1\)。比值越接近 1，表明理论上可证明的最大相对误差越小；需要强调的是，该比值仅反映相对于特定松弛下界的可证明误差上限，并不等同于当前方案距离真实未知最优解的精确绝对距离。
...
\tightlist
\item
  \textbf{拓扑解析与无环校验}：设待调度算子数为 \(|V|\)，直接与间接数据依赖边数为 \(|E_V|\)，生成的子图数量为 \(|\mathcal{S}|\)。在扩展依赖图 \(D^+(P)\) 边集已显式构建的前提下，基于 Kahn 算法的有向无环性判定可在 \(O(|V| + |E_V| + |\mathcal{S}|)\) 的线性时间内完成。需要指出的是，该线性界仅适用于显式图的拓扑排序本身，并不包含此前跨边界 COPY 路径的提取投影、确定性字典序排序以及各候选方案的具体构造耗时；
\item
  \textbf{结构候选构造与软预算约束}：结构化构造主要包含连通分量遍历、前沿划分与支路重分配检查。各算法针对深层结构分析设定了工程规模与时间上限（例如问题一中限制任务总数不超过 320，并引入 2.0 秒的结构分析软预算检测点，若超时则主动退出深层探索并安全回退）。此类工程护栏旨在规避极端复杂拓扑下的退化搜索，并不构成整体端到端求解时间的渐近复杂度保证；
\item
  \textbf{离散事件模拟评测开销}：单次离散事件评测耗时 \(T_j\) 取决于评测内核在推进离散事件时的队列操作与硬件模拟规模。本文各求解器将单用例的在线评测调用次数 \(b\) 严格限制在固定常数上限内（问题一至多 10 次，问题二与问题三至多 3 次），避免了评测次数随图规模无限制累加。结合前置结构校验与分支守卫，实测端到端物理求解耗时（具体数值见第 7 章）稳定在工程实用范围之内；赛题第 5 页脚注 1 所提的 5～10 分钟属于避免暴力迭代的效率建议，并非算法运行耗时的下限指标。
\end{enumerate}
...
\tightlist
\item
  \textbf{动态分组长度细化与拓扑保形顺序（\texttt{\seqsplit{structural\_refine.py}}）}：在基础候选选优结果的基础上，一方面对链状流水结构进行分组长度调整（至多产生 1 次在线评测）；另一方面尝试 \texttt{paced} 与 \texttt{\seqsplit{root-heavy-fused}} 两种拓扑保形执行次序（至多产生 2 次在线评测；输入错峰候选的单例时间线见附录D图\ref{fig:p1-paced-diagnostic}），从中优选出性能最佳的父级方案 \(P_{\mathrm{parent}}\)；
\item
  \textbf{分支援助重分配（\texttt{\seqsplit{branch\_aid.py}} \& \texttt{\seqsplit{branch\_refine.py}}）}：针对父级方案中各核心在同一拓扑阶段负载失衡的情形，通过桥树分析识别可转移的独立计算分支，将其重分配至同波次已有任务的轻载援助核心（至多产生 1 次在线评测）。
\end{itemize}
...
\caption{问题一多候选协同生成与评测选优算法}\label{alg:p1-unified}
\small
...
# 至多发起 1 次在线 E1 评测（全流程评测总预算 6 + 1 + 2 + 1 <= 10 次）
try:
...
return parent_plan
\end{lstlisting}
\textbf{源码对应：} \texttt{834d8c9}，\texttt{\seqsplit{src/q1/branch\_refine.py::solve}} 与 \texttt{unified.py}。伪代码在此对字典键完整性校验、诊断字段返回（如 \texttt{\seqsplit{stop\_reason}}、评测上下界记录）及细粒度耗时统计进行了抽象简化；完整保留了从父方案中提取 \texttt{parent{[}"selected\_objective"{]}}、从评测结果中通过 \texttt{\seqsplit{response\_refine.objective(score)}} 解析目标向量并按严格字典序择优、以及构造/校验/评测全过程的异常捕获回退机制。全流程在线 E1 评测预算由各阶段共享（\(6 + 1 + 2 + 1 \le 10\) 次），离线通过官方 E0 独立复核。
...
\caption{典型用例 case\_026（5 Cores）独立分支重分配前后的切分结构映射。展示原始划分中 Core 0 上的超大任务解构为保留在 Core 0 上的独立支路 Y（算子 710、712、720）与汇合计算 J（算子 724、726），以及重分配至 Core 1 的导出独立支路 X（算子 714、716、722）。图中黑实线表示计算依赖，橙虚线表示跨子图数据依赖，点线端口表示连接至局部裁剪窗口外部的算子依赖（非孤立无依赖节点）。图示仅对应特定用例局部的 8 节点子图结构，不代表构造器处理的一般计算图全局结构。}
\label{fig:p1-cuts}
\end{figure}

\FloatBarrier
\begin{algorithm}[!htbp]
\caption{分支援助重分配的波次分支选择与分配算法}\label{alg:p1-branch-aid}
\small


\begin{lstlisting}[style=paperpseudolong]
...
\{\small \textbf{源码对应：} \texttt{834d8c9}，\texttt{\seqsplit{src/q1/branch\_aid.py::construct}}（第 359--438 行）。上述局部算法聚焦单个波次内的分支选择与分配，\texttt{pieces} 为跨波次累积更新的映射字典。前置条件假定算子包单汇点合法性检查与同高度无跨核计算边已由外围断言验证（不满足时外围抛出 \texttt{\seqsplit{UnsupportedStructure}}）；容量上限 \texttt{\seqsplit{max(320,\ n\_tasks)}} 仅为构造器内部工程规模限制而非官方系统硬上限；外围波次遍历（\texttt{\seqsplit{\_wave\_domain}}）、调度全序重组与异常处理见完整源码。
...
\item
  \textbf{流水线负载统计与角色划分}：统计波次 \(W_h\) 内各核心在各个实际出现的执行流水线上的有效计算工作量 \(\operatorname{work}(c)[p] = \sum_{v \in T_{h, c}, \operatorname{pipe}(v) = p} \max(1, \operatorname{cycles}_v)\)，确定该波次全系统工作量最大的主导流水线 \(p^*\)。在主导流水线 \(p^*\) 上，满足 \(K \times \operatorname{work}(c)[p^*] > \operatorname{total}[p^*]\)（即本核工作量严格高于全波次平均工作量）的核心被判定为\textbf{捐赠核心候选（donor candidates）}；满足 \(K \times \operatorname{work}(c)[p^*] < \operatorname{total}[p^*]\) 且在波次 \(W_h\) 处分配有原有独立任务 \(T_{\mathrm{orig}}\) 的核心被判定为\textbf{援助核心候选（helper candidates）}。捐赠核心集与援助核心集基于主导流水线负载严格角色互斥，在该波次完全空闲（未分配任务）的核心不进入援助核心集合；
\item
...
\begin{table}[!htbp]\centering
\caption{全量实验评测与结果分析}\label{tab:04-p1-1}\small
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=.84\textheight,keepaspectratio]{figures/team-v8/p1-timeline-full.pdf}
\caption{用例 case\_026 在 5 核配置下重分配前后的完整执行时间线对比。基于官方模拟事件绘制，(a) 原始方案 Makespan 为 44,114 cycles，额外数据搬运量为 211,680 B；(b) 分支援助重分配后 Makespan 为 42,014 cycles，额外数据搬运量为 375,532 B。各核心 Task 行表示任务执行区间，M、V、MTE2、MTE3 分别表示四类硬件流水线的已记录操作区间，图中浅色区域标出 5,200--9,400 cycles 局部观察窗口。}
\label{fig:p1-overlap}
\end{figure}

在局部放大图（图 \ref{fig:p1-overlap-zoom}）中，展示了 5,200--9,400 cycles 关键时间窗内核心 0 与核心 1 的流水线推进细节：图内 \texttt{C0:T0/SG0} 表示核心 0 上执行的任务 0（对应子图 0，即保留分支 \(Y\)）；重分配后在核心 1 上优先执行的任务 \texttt{C1:T6/SG6} 对应导出的独立分支 \(X\)（包含算子 714、716、722），其在向量流水线上密集执行，随后核心 1 推进执行其原有任务 \texttt{C1:T1/SG1}；核心 0 上则在完成保留支路 \(Y\) 后，紧接着执行汇合任务 \texttt{C0:T5/SG5}（子图 5，对应 \(J\)）。在两组时间线中，执行资源对应行的空白仅表示该时段没有记录到流水线占用事件，不能据此主观判定其原因是等待外部内存、片上存储或其他硬件资源。

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=.84\textheight,keepaspectratio]{figures/team-v8/p1-timeline-zoom.pdf}
\caption{用例 case\_026 在 5,200--9,400 cycles 窗口内的局部微架构流水线时序放大图（对应图 \ref{fig:p1-overlap} 浅色区域）。展示核心 0 与核心 1 在该时段内的任务切换与各流水线操作事件分布。图中执行资源对应行的空白仅表示该时段未记录到流水线占用事件，不推定具体的外部等待原因。}
\label{fig:p1-overlap-zoom}
\end{figure}
...
若数据生成算子与数据消费算子被分配至不同的计算核心，则数据依然必须通过跨核心通信链路传递：由源核心执行 \texttt{COPY\_OUT} 将数据写入 DDR，再由目标核心执行 \texttt{COPY\_IN} 将数据读入本地缓存。在硬件时序上，目标核心的读取操作必须在源核心写出操作彻底完成 500 cycles 之后方可启动。在此执行模型下，即使在方案提交文件（\texttt{\seqsplit{node\_to\_subgraph}}）中将每个计算算子单独划分为一个子图，也不会在硬件底层为每个算子生成孤立的物理 Task，子图序列仅用于指示底层编译器在各核心内部排定算子的先后执行次序。
...
\tightlist
\item
  \textbf{缓存复用收益}：当某一数据张量的生成算子与全部消费算子均被分配至同一计算核心、且不存在其他跨核消费者时，可免去该张量对应的跨核 DDR 读写搬运与 500 cycles 的跨核同步等待；若仍有部分消费者位于远端核心或存在外部主存写出，则相应的边界传输依然发生；
\item
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=.84\textheight,keepaspectratio]{figures/team-v8/p2-lifetimes.pdf}
\caption{单核心张量生命周期与片上私有缓存容量占用手算示例。上部展示 9 个物理张量在闭区间 \([f(t), \ell(t)]\) 内的驻留条带（如 \(T_6 = [5, 6]\) 与 \(U_3 = [4, 6]\)）；中部与下部展示基于模仿 Step 2 静态编译操作序号下的 L1 Buffer（容量 512 KiB，步骤 2 达峰值 512 KiB）与 Unified Buffer（容量 128 KiB，步骤 4 达峰值 72 KiB）存储占用阶梯曲线，说明输入与输出张量在操作边界共同驻留对局部容量峰值的影响。图中无官方 spill 事件记录且未绘制溢出事件，横轴为静态操作序号而非微架构物理执行周期。}
\label{fig:p2-lifetimes}
\end{figure}
...
设输入计算图的所有操作集合为 \(O\)。定义待调度的操作集合为 \begin{equation}
V = \{u \in O \mid \operatorname{op}(u) \notin \{\text{COPY\_IN}, \text{COPY\_OUT}\}\},
\end{equation} 即排除跨边界核外搬运操作 \texttt{COPY\_IN} 与 \texttt{COPY\_OUT} 之后的全部待调度操作集合（其中计算算子与子图内部搬运操作均可能包含在 \(V\) 内）。
...
\tightlist
\item
  顶点集 \(V\) 完整包含上述除核外搬运操作外的全部原始待调度操作；
\item
  在原始计算图 \(O\) 中，每个物理张量至多由一个原始操作生成（即 \(|\operatorname{producers}(t)| \le 1\)）；严格拒绝逻辑张量别名（\texttt{\seqsplit{logical\_tid}}），张量大小 \(b_t\) 为非负整数；
\item
  直接操作间依赖边（direct op-op edges）亦纳入模型统一处理。
\end{enumerate}

\subsubsection{超边构成与权值定义}

对于计算图中的每个物理张量 \(t\)，定义其在待调度操作集合 \(V\) 内的生成操作集合与消费操作集合为： \begin{equation}
p_V(t) = \operatorname{producers}(t) \cap V, \qquad c_V(t) = \operatorname{consumers}(t) \cap V.
\end{equation} 由于原始图中张量生成者至多一个，故集合 \(p_V(t)\) 要么为空集，要么包含唯一的生成操作。
...
\tightlist
\item
  \textbf{无关联张量略去}：若 \(p_V(t) = \emptyset\) 且 \(c_V(t) = \emptyset\)，该张量在待调度操作集 \(V\) 内既无生成者亦无消费者（例如纯粹在外部搬运操作之间流转的张量），在超图模型中直接略去，不建立对应超边；
\item
  \textbf{模型外部输入张量}：若 \(p_V(t) = \emptyset\) 且 \(c_V(t) \ne \emptyset\)（例如由外部 DDR 读入或由图内 \texttt{COPY\_IN} 载入但生成操作不在 \(V\) 中的张量），该张量在待调度操作层面上构成外部输入。其首次载入的基础开销 \(b_t\) 计入全局常数项 \(C\)；其超边引脚集合仅包含读取该输入的待调度操作子集 \(c_V(t)\)。每增加一个需要该输入的核心，需额外从 DDR 搬入 \(b_t\) 字节，故超边权重赋值为 \(w_e = b_t\)；
\item
  \textbf{模型内部生产张量}：若 \(p_V(t) \ne \emptyset\)（记唯一的生成操作为 \(u \in V\)），其超边引脚集合包含生成操作及全部消费操作 \(c_V(t) \cup \{u\}\)。依官方冻结评测编译语义，跨核心访问时涉及源核心写出外存与目标核心读入本地的一对操作，故超边权重赋值为 \(w_e = 2b_t\)（硬件常数严格依据源码，不作未经证实的广播假设）；若该张量同时被外部 \texttt{COPY\_OUT} 操作消费，或者在计算图内无后继待调度消费操作（\(c_V(t) = \emptyset\)），表明其结果必须写出至外部主存，其写出外存的固定基础开销 \(b_t\) 累加至常数项 \(C\)；
\item
  \textbf{直接操作间依赖边}：对原始图中源节点与目标节点均属于 \(V\) 的直接操作依赖边 \(e = (u, v)\)，构建引脚集合为 \(\{u, v\}\)、权重为 \(w_e = 2 \times \operatorname{data\_size}(e)\) 的超边；
\item
  引脚集合为空或权重为零的超边被直接丢弃，不加入超边集 \(\mathcal{E}\)。
\end{enumerate}

\subsubsection{预 Step 2 原始复制字节数公式}

给定待调度操作到计算核心的指派映射 \(a: V \to \{0, 1, \dots, K-1\}\)，定义 \(\lambda_e(a)\) 为超边 \(e\) 中全部操作所跨越的不同核心数量（无量纲整数）。在官方评测程序第二阶段（Step 2）插入缓存溢出换入换出之前，方案的\textbf{预 Step 2 原始复制字节数}（pre-Step2 original COPY bytes）精确满足：

\begin{equation}
D_{\mathrm{pre2}}(a) = C + \sum_{e \in \mathcal{E}} w_e \bigl(\lambda_e(a) - 1\bigr).
\end{equation}

其中，\(D_{\mathrm{pre2}}\)、\(C\) 与 \(w_e\) 的单位均为字节（B）。当一条超边的所有相关操作均指派给同一核心时，\(\lambda_e(a) = 1\)，对求和式的贡献为 0；每向一个新核心延伸，均线性增加 \(w_e\) 字节。该指标精确度量了 Step 2 插入缓存溢出换入换出前重建方案的 COPY 总字节数（包括基础输入输出常数项 \(C\) 和分核指派引起的跨核边界项），并不等同于官方离散仿真中扣除原始图 COPY 量后的额外 DDR 搬运量（\texttt{\seqsplit{added\_copy\_bytes}}），亦不代表 Makespan。
...
\item
  \textbf{锚定弧（anchors）}：若某计算链单元 \(u\) 被强制锁定在核心 \(a\)，则添加容量为 \(M_\infty\) 的源点指向 \(u\) 的弧；若锁定在核心 \(b\)，则添加 \(u\) 指向汇点的 \(M_\infty\) 弧。
\end{enumerate}
...
\tightlist
\item
  \textbf{例 A（固定核心集合为空，\(F_e = \emptyset\)）}：超边连接局部两条可移动链 \(u, v\)，超边权重为 \(w\)。基准偏移为 \(\text{offset} = -w\)。当给定 \(u, v\) 同归属于某一核心时（例如均分在核心 \(a\) 或均分在核心 \(b\)），辅助节点在较优侧时的对应割容量为 \(c_{\mathrm{cut}} = w\)，加上偏移后该分配下的局部复制代价为 \(\operatorname{Cost} = -w + w = 0\)；若 \(u, v\) 分跨两个核心，两侧辅助节点的容量边均被割断，对应割容量为 \(c_{\mathrm{cut}} = 2w\)，局部复制代价为 \(\operatorname{Cost} = -w + 2w = w\)；
\item
  \textbf{例 B（已固定触及核心 \(a\) 与另一核心 \(c\)，\(F_e = \{a, c\}\)）}：由于核心 \(a \in F_e\)，系统不再为核心 \(a\) 建立容量边，仅当有链移入新增核心 \(b\) 时才产生新增复制，基准偏移为 \(\text{offset} = (|F_e| - 1)w = +w\)。若给定链全部保留在核心 \(a\)，对应割容量为 \(c_{\mathrm{cut}} = 0\)，局部代价为 \(\operatorname{Cost} = w + 0 = w\)；若有链被分配至核心 \(b\)，割断指向核心 \(b\) 的容量边（权重 \(w\)），对应割容量为 \(c_{\mathrm{cut}} = w\)，局部代价为 \(\operatorname{Cost} = w + w = 2w\)。 对任意给定的局部链分配，辅助网络在最优辅助节点放置下的割容量 \(c_{\mathrm{cut}}\) 加上偏移均严格等于超图连接代价；而在网络未加额外锚定时求得的最小割容量（即最大流值 \(f_{\max}\)），则对应无约束局部最优分配，满足总体最优成本 \(\operatorname{Cost}^* = \text{offset} + f_{\max}\)。
\end{enumerate}

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=.84\textheight,keepaspectratio]{figures/team-v8/p2-local-cut.pdf}
\caption{二核心局部超图割辅助网络构建与工作量保护流程。展示超边在固定核心集合为空集与非空集合时的有向辅助网络映射，以及结合流水线工作量上限保护的迭代锚定求解机制。}
\label{fig:p2-local-cut}
\end{figure}

需要强调的是，辅助网络中的局部超图割仅针对预 Step 2 原始复制字节数 \(D_{\mathrm{pre2}}\) 进行直接建模，并以 cycles 为单位对各计算流水线施加工作量上限保护，它并不保证片上私有缓存不发生溢出（spill 为 0）。同时，\texttt{\seqsplit{load\_guarded\_cut}} 本身返回的是满足工作量上限且复制代价不增加（非增，\(\operatorname{Cost} \le \operatorname{Cost}_0\)）的分配；而在外层细化流程 \texttt{\seqsplit{gap\_hyperrefine}} 中，触发重排生成完整 \(P_2\) 候选的门槛则严格要求全图 \(D_{\mathrm{pre2}}\) 产生严格下降（\(\text{after} < \text{before}\)）。此外，局部割容量的下降并不证明完整多核调度方案的 Makespan 必定缩短，各候选方案的综合性能仍须依赖官方离散事件仿真器裁决。
...
\FloatBarrier
\begin{algorithm}[!htbp]
\caption{受工作量保护的二核心局部超图割算法}\label{alg:p2-load-guarded-cut}
\small
...
\end{lstlisting}
\textbf{源码对应：} \texttt{c66559a}，\texttt{\seqsplit{src/q2\_nikolastarx/binary\_hypercut.py::load\_guarded\_cut}}。为突出核心流程，上述算法省略了输入合法性（源码第 198--240 行，包含相同 Pipe 集合、非负整数及初始分配完全满足 caps 等约束）的展开校验与诊断字段组装，返回 \texttt{\seqsplit{(labels,\ cost)}} 为 \texttt{\seqsplit{GuardedCutResult}} 对象的简化表示。
...
\begin{proof} 初始分配始终完全满足工作量上限约束。由于每次锚定均与初始分配一致，初始分配在每一次增加锚定约束后的辅助网络中始终构成一个合法的割。根据最小割的最优性，求得的最小割代价必不高于初始分配的代价值。由初始分配完全满足各流水线工作量上限且算子工作量非负，若某一核心在某流水线上的总工作量超过上限，则该核心在该流水线上的工作量必然严格大于初始值，从而必然存在自其他核心迁入该核心且在该流水线上具有正工作量的流动链；又因已锚定单元只能留在其初始分配的核心，不可能作为迁入者，故待选的迁入链中必存在尚未被锚定的流动单元。由于局部自由链的总数为 \(w\)（\(w \le 16\)），若每次迭代均发生工作量超限，每次至少有一条未锚定的流动链被永久锁定回原位置。在最坏情况下，经过至多 \(w\) 次锁定后，全部流动链均被重置回初始分配，此时工作量超限自然消除。因此，算法必在至多 \(w + 1\) 次求解内终止，且终止解的局部复制字节数不高于原方案。 \end{proof}

需要明确指出的是，命题 5.1 证明的仅是在给定的局部二核心区域及锚定启发过程下预 Step 2 复制量不增加且流水线工作量不超限。局部受保护割最多执行 \(w + 1\) 次最大流求解（\(w \le 16\)），并非在线离散仿真评测预算（后者全局冻结为至多 3 次评分请求）。流水线工作量上限是通过迭代式的启发式锚定进行防护，而非单次带约束规划的全局最优解；该机制\textbf{并不保证}片上私有缓存不发生溢出（spill），\textbf{并不保证}全局 Makespan 一定缩短，亦非全图范围的多核全局最优解。算法仍须依赖最终的离散事件仿真来进行综合裁决。
...
\tightlist
\item
  \textbf{基础方案 P0 保底}：P0 由自适应装箱生成并始终保留，当后继候选不支持、构造异常或评测不可用时立即作为安全回退方案输出；
\item
  \textbf{空闲区间方案 P1}：尝试构造 P1，即使 P1 与 P0 方案相同，算法仍以原始 P1 为基准启动局部超图调整；若 P1 构造失败，则直接返回保底方案 P0；
\item
  \textbf{局部改进方案 P2 的门槛控制}：以 P1 为基础尝试局部超图调整，仅当调整前后的预 Step 2 原始复制字节数（pre-Step 2 original COPY bytes）均为有效非负整数且满足严格下降（\(\Delta D_{\mathrm{pre2}} < 0\)）时，方调用重定时模块尝试生成完整的 P2 方案（\texttt{hypergap}）；若局部调整不支持、重排失败或字节数未严格减少，则跳过 P2 构造，保留已有方案；若发生非预期错误或字节数证据无效，则直接返回 P0；
\item
  \textbf{按序去重与受限评测}：完整方案按 P0 \(\to\) P1 \(\to\) P2 的构造顺序进行去重。若去重后仅有 1 份有效方案（即仅剩 P0），直接输出 P0 而不发起任何在线评测；若存在 2 份或 3 份互不相同的完整方案，按构造顺序逐份向在线评分接口提交请求（总调用次数严格不超过 3 次）；
\item
  \textbf{字典序决选与安全回退}：评分接口返回原生评测结果，算法依据目标向量 \((M_2,\allowbreak \text{added\_copy\_bytes})\) 按完成时间优先、额外 COPY 字节数次之的严格字典序选优；平局时优先保留构造序列靠前的方案。若在线评分抛出异常或返回结果无效，控制器捕获后安全回退并输出保底方案 P0。
\end{enumerate}

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=.84\textheight,keepaspectratio]{figures/team-v8/p2-three-plans-a.pdf}
\par\vspace{4pt}
\includegraphics[width=\linewidth,height=.84\textheight,keepaspectratio]{figures/team-v8/p2-three-plans-b.pdf}
\caption{问题二三种候选方案的构造分支与评测选优决策流。展示基础方案 P0、空闲区间方案 P1 与局部超图调整方案 P2 的构造分支条件、按构造顺序去重机制以及至多 3 次在线评分的字典序择优规则。}
\label{fig:p2-three-plans}
\end{figure}

\FloatBarrier
\begin{algorithm}[!htbp]
\caption{问题二自适应超图细化与多核调度算法}\label{alg:p2-adaptive-hypergap}
\small
...
\end{lstlisting}
\textbf{源码对应：} \texttt{c66559a}，\texttt{\seqsplit{src/q2\_nikolastarx/adaptive\_hypergap\_guarded.py::build}}。
...
\tightlist
\item
  \textbf{预 Step 2 原始复制字节数}（pre-Step 2 original COPY bytes，\(D_{\mathrm{pre2}}\)）：是在候选构造阶段由超图连接度模型所度量的通信量，反映 Step 2 插入缓存溢出换入换出前重建方案的 COPY 总字节数（包括固定输入输出项和分核指派引起项），作为决定是否调用重排模块尝试生成 \(P_2\) 的内部触发门槛，并不等同于最终完成时间的缩短；
\item
  \textbf{额外 COPY 字节数}（\texttt{\seqsplit{added\_copy\_bytes}}）：是在完整方案经官方离散事件仿真评测后，记录的调度 COPY 总量扣除原始图 COPY 量后的额外数据搬运量，作为多目标选优中的第二排序键。
\end{enumerate}

评分函数仅在评测返回结果为字典映射（\texttt{Mapping}）、状态为正常（\texttt{status\ ==\ \textquotesingle{}ok\textquotesingle{}}）且 Makespan 与额外 COPY 字节数均为有效非负整数时采纳该得分向量。全量实验中原生仿真器 E2 正常返回原生评测结果（未触发回退）属于实测数据事实，并非代码中的前置校验谓词。图中的``至多 3 次''指在线求解器内部的评分调用上限，独立的外部 E0 复评作为离线验证单独报告，不计入在线求解调用账。
...
\begin{table}[!htbp]\centering
\caption{全量实验评测与结果分析}\label{tab:05-p2-1}\small
...
\item
  \textbf{通信流量与多核权衡}：全量 500 个用例的额外 DDR 搬运总量从基准版本的 3,043,860,416 字节上升至 4,159,738,560 字节（增幅为 36.66\%）。深入分析 500 个用例的性能变动分布：共有 55 个用例实现了执行时间与搬运量双重下降；199 个用例执行时间缩短但搬运量有所增加；14 个用例执行时间缩短而搬运量持平；其余 232 个用例两项指标均保持不变。该分布反映了算法以主目标（Makespan）优先的选优取向：在版本迭代中观察到了执行时间下降与额外搬运量增加相伴随的成对数值变动，此处仅客观记录前后版本对比的数据呈现，不作未经独立消融隔离的因果机制归因；
\item
...
{\def\LTcaptype{table} % do not increment counter
\begin{table}[!htbp]\centering
\caption{减少搬运量反而增加完成时间的反例（C04）}\label{tab:05-p2-2}\small
\begin{tabular}{@{}
...
尤其在 \texttt{case\_019}、\texttt{case\_039} 与 \texttt{case\_075} 中，C04 方案的边界 COPY 字节数低于本文主算法，但其执行完成时间却出现了大幅退化（在 \texttt{case\_019} 中变慢达 75.50\%）。

图 \ref{fig:p2-counterexample} 与图 \ref{fig:p2-counterexample-zoom} 分别给出了 \texttt{case\_019}（5 核心场景）在官方离散事件仿真下的全景与局部时序对比。数据显示，C04 方案的边界 COPY 字节数低于主算法（额外搬运量减少 73.20\%），且无片上容量不足引发的 Step 2 缓存换入换出（spill 为 0），但其最终执行完成时间增加了 75.50\%（Makespan 由 16,247 cycles 增至 28,514 cycles）。图中各核心横道分别对应各个核心的 Task 与 Pipe 活跃区间（例如 C0:T0 与 C3:T3 分别表示 Core 0 与 Core 3 上排定的具体 Task 任务段）。在全景时序中，主算法于 16,247 cycles 即完成了全图计算，而 C04 方案持续运行至 28,514 cycles。在 13,800 至 17,400 cycles 的局部放大窗口中，C04 方案在各个核心的 Task 之间存在未记录流水线操作的间隔。需要严格说明的是，图中未着色的空白区域仅代表仿真器在此时间段内未调度对应的流水线事件，客观反映对应流水线未记录操作占用，此处不作未经微架构事件级归因分析的机理臆测。

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=.84\textheight,keepaspectratio]{figures/team-v8/p2-timeline-full.pdf}
\caption{case\_019（5 核心）主算法与 C04 对照方案全景执行时序对比。展示两方案在全流程时间尺度（0 至 28,514 cycles）下各核心 Task 与 Pipe 活跃区间；C04 方案无容量不足引起的缓存换入换出（spill 为 0），但完成时间（28,514 cycles）劣于主算法（16,247 cycles）。}
\label{fig:p2-counterexample}
\end{figure}

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=.84\textheight,keepaspectratio]{figures/team-v8/p2-timeline-zoom.pdf}
\caption{case\_019（5 核心）两方案在关键局部时间窗（13,800 至 17,400 cycles）的时序放大对比。展示主算法与 C04 在 Core 0（C0:T0）及 Core 3（C3:T3）等任务上的流水线事件分布。图中未着色空白区域仅客观反映对应流水线未记录操作占用，不作未经事件级归因分析的机理臆测。}
\label{fig:p2-counterexample-zoom}
\end{figure}
...
\FloatBarrier
\begin{algorithm}[!htbp]
\caption{归约森林核心排序与DFS局部步骤}\label{alg:p3-forest-memory-order}
\small


\begin{lstlisting}[style=paperpseudo]
# 输入：拓扑索引 index、分核归属 assignment，以及由前处理与 _original_tree 获得的 records、component_for、tensors、output_tensors
# 输出：各核心的算子执行序列 sequences（单操作子图映射与合法性校验见 construct 外围代码）
peak, retained, child_order = {}, {}, {}
# 沿 index.order 自底向上递推：汇聚子节点按 peak - retained 降序排序，相同按操作 ID 升序破平
...
# 依排定子树次序进行 DFS 后序遍历（子节点逆序入栈使出栈与子树顺序一致）
postorders = {}
for record in records:
    out, stack = [], [(record["root"], False)]
    while stack:
        u, visited = stack.pop()
        if visited:
            out.append(u)
            ...
            stack.append((u, True))
            stack.extend((c, False) for c in reversed(child_order[u]))
...
for component_ids in assignment:
    ...
    sequences.append([u for cid in ordered_ids for u in postorders[min(index.components[cid])]])
return sequences
\end{lstlisting}
\textbf{源码对应：} \texttt{311322b}，\texttt{\seqsplit{src/q3/forest\_memory\_order.py::construct}}（第 153--192 行）。前置结构守卫检查、单操作子图映射与合法性校验（\texttt{\seqsplit{derive\_multicore\_plan(index.graph,\ plan)}}）等外围逻辑见完整源码。
...
在问题三的算法体系中，顶层求解器 \texttt{\seqsplit{forest\_solve}} 嵌套调用一系列前序求解模块，其调用链关系为：
...
\item
  \textbf{\texttt{\seqsplit{pipeline\_solve}}}：调用 \texttt{\seqsplit{calendar\_solve}} 获得基准方案后，针对具有共享输入特征的多核流水线结构，调用 \texttt{\seqsplit{shared\_pipeline.construct}} 构造候选方案。该模块要求非空连通分量必须为相邻拓扑位置均存在直接依赖的有向链，且每条链均读取相同的外部输入张量；各链对应位置的流水线类型、计算时长以及共同输入读取签名完全一致；且链长（位置数）不超过 512、分量数不小于核心数 \(K\)。算法提取代表链的计算时长序列，利用动态规划将其切分为 \(\min(K, \text{位置数})\) 个连续段，最小化各段工作量和的最大瓶颈，进而按阶段构建核心执行序列。该连续极小化极大划分仅作为平衡各核心计算时长的代理目标，并不直接等同于官方最终的 Makespan；
\item
...
\item
  \textbf{评测执行与异常处理}：若通过全部前置检验，在剩余预算内调用官方 E0 进行评测。若评测抛出数据校验异常（\texttt{\seqsplit{EvaluationValidationError}}），模块捕获后记录 \texttt{rejected} 并安全保留当前最优解；若评测成功且最终 Makespan 严格小于当前最优解（\(M < U\)），方接受该方案并替换为新的最优解。\textbf{在问题三的决策逻辑中，仅以 Makespan 严格改善为采纳标准，不使用 DDR 搬运量进行平局决选（tie-break）}。
\end{enumerate}
...
\tightlist
\item
  \textbf{树状排序的触发与适用限定}：排序机制仅作用于满足特定条件的归约子树结构（多棵互不交错的归约树且至少包含一处汇聚汇合节点、每个算子具有确定尺寸的单一输出、非根算子输出仅被唯一父算子消费）。对各子树计算指标差值 \(h_i - r_i\)（其中 \(h_i\) 为子树内部结果的存储峰值字节数，\(r_i\) 为子树完成后保留的输出字节数），按该差值降序排列处理同一父算子的子树，并在同值时按操作编号稳定决胜，随后执行父算子。在非交错树模型下，该规则使单棵树内部结果的存储峰值达到最小；需要强调的是，该性质仅是对该局部排序指标的理论保证，并不改变既定的分核归属，亦不直接推出官方评测 Makespan 缩短或缓存命中率提升；
\item
  \textbf{完整候选的评测决策链}：在求解流程中，所有候选方案共享至多 3 次在线 E0 评测预算。基础方案与注意力感知候选先行生成并确立暂定最优解（当前最优方案 \(\mathrm{winner}\)，指迄今已完成评测的候选方案中具有最小官方 Makespan 的方案，并非全局理论最优解）；针对新构造的树状排序方案，系统依次实施严格的前置检验：若已用评测次数达到 3 次，或无法构造合法的树状排序方案，或与当前最优方案编码完全相同，则直接跳过评测；
\item
  \textbf{理论下界过滤与严格改善}：若该方案满足下界构建条件且计算所得下界 \(L(\text{proposal}) \ge U\)，则断定其完成时间绝不可能严格超越当前最优值，直接作为 \texttt{\seqsplit{bound\_pruned}} 剪除；若由于拓扑或算子类型导致下界不支持（\texttt{\seqsplit{UnsupportedBound}}），算法记录 \texttt{\seqsplit{bound\_unavailable}} 并继续推进评测，绝不将无法证明的下界当作剪枝条件；仅当通过全部前置检验时，方调用官方 E0 进行最终评测，且仅在 Makespan 严格缩短时采纳该方案。
\end{enumerate}

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=.84\textheight,keepaspectratio]{figures/team-v8/p3-forest-decision.pdf}
\caption{树状分支排序准则与完整候选方案的评测决策流程。展示基于指标差值 \(h_i - r_i\) 降序排定非交错子树顺序与单树内部存储峰值最小化范围，以及共享至多 3 次在线 E0 评测预算下的前置检查、去重、下界剪枝与严格 Makespan 改善择优决策链。}
\label{fig:p3-forest-decision}
\end{figure}

\FloatBarrier
\begin{algorithm}[!htbp]
\caption{问题三归约森林与多候选协同求解算法}\label{alg:p3-forest-solve}
\small


\begin{lstlisting}[style=paperpseudo]
# 输入：拓扑索引 index，核心数 cores，官方评测器 evaluate，保存回调 save，硬件跨核延迟 delay
# 输出：当前最优调度方案 winner[0]，实际评测次数 calls（已省略诊断追踪与落盘外围）
winner, calls, _, _ = witness_solve.evaluate_candidates(index, cores, evaluate, save)
if calls >= 3:
    return winner[0], calls  # 共享评测预算耗尽，跳过后续构造
# 尝试构造归约森林内存优化候选方案
try:
    proposal, _ = forest_memory_order.construct(index, cores)
except UnsupportedStructure:
    return winner[0], calls  # 结构超出守卫支持，跳过并保留当前最优解
# 方案编码去重检查：避免对相同方案重复发起评测
if encoded(proposal) == encoded(winner[0]):
    return winner[0], calls
# 依据硬件依赖与流水线 FIFO 必要不等式求解完成时间理论下界
try:
    ...
    if lower >= winner[1]["makespan"]:
        return winner[0], calls  # 完成时间下界剪枝
except UnsupportedBound:
    pass  # 超出下界模型支持时继续推进评测，绝不误剪
# 在剩余预算内发起官方仿真评测
calls += 1
try:
    result = evaluate(proposal)
except EvaluationValidationError:
    return winner[0], calls  # 评测校验异常安全回退
# 仅当官方 Makespan 严格更小时采纳新方案，平局不以 DDR 搬运量破平
if result["makespan"] < winner[1]["makespan"]:
    winner = (proposal, result, "forest_memory_order")
return winner[0], calls
\end{lstlisting}
\textbf{源码对应：} \texttt{311322b}，\texttt{\seqsplit{src/q3/forest\_solve.py::evaluate\_candidates}}。为突出核心决策与评测控制逻辑，省略了落盘（\texttt{save}）与诊断追踪字典（\texttt{records}, \texttt{selection}）的外围组装。跨核心搬运延迟 \texttt{delay} 依照官方硬件配置固定为 500 cycles。
...
\end{equation}
\item
  \textbf{深度优先后序遍历与方案拼接}：生成各分量 DFS 序列与拼接核心序列的基础遍历耗时为 \(O(V)\)；对分配至各个核心的分量，按照各分量子树最小算子编号进行确定性升序排序的开销另计为 \(O\left(\sum_{k=0}^{K-1} C_k \log C_k\right) \le O(C \log C)\)（其中 \(C_k\) 为分配至核心 \(k\) 的连通分量数，\(\sum_{k=0}^{K-1} C_k = C \le V\)）；
\item
  \textbf{必要时序图构建与最长路径求解}：若候选方案满足单操作 M/V 算子条件，从原图中提取直接算子依赖边并进行端点排序去重耗时 \(O(E_o \log E_o)\)（\(E_o\) 为原图算子间依赖边数）；在构建好的包含 \(V\) 个节点与 \(E_{\mathrm{bound}}\) 条约束弧的有向无环增广图上，利用拓扑动态规划求解最长路径的时间复杂度为 \(O(V + E_{\mathrm{bound}})\)；
\item
...
{\def\LTcaptype{table} % do not increment counter
\begin{table}[!htbp]\centering
\caption{启用和关闭共享缓存的配对实验}\label{tab:06-p3-2}\small
\begin{tabular}{@{}
...
\item
  \textbf{指标口径的精确区分}：表中第一、二列的分子基准 \(B\) 均严格采用官方单核基准方案的 Makespan。第三列为逐用例计算 \(M_2/M_3\) 后的算术平均值（\(1.0829\)），该数值在数学上并不等于两列均值的直接相除（\(\frac{4.7576}{4.4763} \approx 1.0628\)）；
\item
...
对这 3 张被拒绝用例的深层数据复核揭示了一个关键反例现象：这三份被拒绝的候选方案在实际执行中，\textbf{其实际执行时间 \(M_2\) 与 \(M_3\) 均有缩短}，具体数值如下表所示：

{\def\LTcaptype{table} % do not increment counter
\begin{table}[!htbp]\centering
\caption{附加比值选择条件的反例与消融（R9F）}\label{tab:06-p3-3}\small
\begin{tabular}{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1579}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2105}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2105}}
...
在实验数据整理、统计口径核对与结果分析过程中，本研究采用了人工智能工具提供辅助支持。所使用的工具为 GPT-6 Astra（API 型号：\texttt{gpt-6-astra}，由 OpenAI 开发，版本发布日期为 2026 年 9 月 3 日）。文中全部测试用例的实测数值均严格源自前述已冻结的数据资产库与官方离散事件仿真记录，AI 输出仅用于辅助数据整理、统计指标核对与分析讨论，不作为生成新测试数值或替代离散事件仿真的依据。
...
\tightlist
\item
  在子图（a）中，每个数据点对应一张基准计算图。横轴为图中非 COPY 计算操作的总数量（采用对数刻度），纵轴为图中全部张量的大小总和（以 MiB 为单位，采用对数刻度）；
\item
  在子图（b）中，对每张计算图分别汇总矩阵类（\texttt{PIPE\_M}）与向量类（\texttt{PIPE\_V}）计算操作在题面模型中的标称计算周期数，纵坐标为静态矩阵标称时长占矩阵与向量标称计算时长之和的比值。横轴按该比值从小到大升序排列 100 张计算图，每张图对应一个位置。需要特别强调的是，该无量纲占比描述的是计算图的静态工作量构成，不能等同于调度后的实际物理执行时间或全局 Makespan 的占比。测试所用的全部 100 张图在研发阶段均作为已知基准参与过特征分析，本文不主观臆测具体的神经网络模型名称，亦不将其虚称为完全未见的盲测集。
\end{enumerate}

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=.84\textheight,keepaspectratio]{figures/team-v8/dataset.pdf}
\caption{官方 100 张原始计算图的规模与操作标称时长构成。（a）计算图操作数量与张量大小分布；（b）矩阵类操作标称计算时长占矩阵与向量标称计算时长之和的比例分布。}
\label{fig:dataset}
\end{figure}
...
{\def\LTcaptype{table} % do not increment counter
\begin{table}[!htbp]\centering
\caption{三问主口径加速比综合对比}\label{tab:07-experiments-1}\small
\begin{tabular}{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1579}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2105}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2105}}
...
\item
  ...
  \item
    问题三进一步引入 1 MiB 的共享只读 Cache。依据第 6 章所述的缓存资格与 FIFO 规则，符合条件的 \texttt{COPY\_IN} 操作若发生缓存命中，可直接从 Cache 读取数据，从而减少相应的 DDR 访存读取开销；
  \end{itemize}
\item
  \textbf{指标统计口径的严谨区分}：
...
{\def\LTcaptype{table} % do not increment counter
\begin{table}[!htbp]\centering
\caption{求解效率与在线评测调用统计}\label{tab:07-experiments-2}\small
\begin{tabular}{@{}
...
\tightlist
\item
  \textbf{调用频次分布}：在三道题目各 500 种测试组合的求解过程中，在线评测调用次数平均每用例发起约 2 次（均严格在设定的上限以内，P1 至多 10 次，P2 与 P3 至多 3 次）。该频次客观记录了各算法在候选生成与前置筛选后实际发起的评测规模，不将调用次数受控直接作为各项具体守卫或剪枝机制各自有效性的孤立因果证明；
\item
  \textbf{端到端求解时间}：在所有 1500 次运行中，实际端到端求解物理耗时的最大值为 51.335 秒。赛题第 5 页脚注 1 提出的 5～10 分钟属于避免暴力迭代搜索的工程效率建议，并非程序最低运行时间要求或硬性淘汰阈值，亦非独占硬件下的性能判定。由于测试运行于共享主机且各题工作进程数不同，所记录的耗时反映工程实际测得的求解时间，不作为独占硬件条件下的算力性能基准。
\end{enumerate}
...
\subsection{版本对比、机制消融与限定反例}

为了检验各项机制的实际影响并核实设计假设，实验区分宏观版本演进对比与受控消融/反例检验两类不同证据。在多处理器系统性能分析中，单纯统计局部算子耗时并不直接对应全局完成时间的因果改善\cite{ref12}；若多个机制同时演变，不能将整版本的宏观收益直接归因于单一孤立机制：

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{问题一版本演进对比}：在问题一中，包含分支重排与前沿细化的新版本相比前序基准，在 5 核心场景下的平均加速比提升约 0.729\%，全图额外 DDR 搬运总量微增约 0.389\%。这一数据客观呈现了完整版本在提升平均加速比的同时伴随额外搬运量小幅增加的数值变动；由于该版本同时包含结构细化与拓扑次序探索等多项复合改动，此处仅记录整版本对比表现，不作单一机制的因果分摊；
\item
  \textbf{问题二版本演进对比}：与前一完整版本相比，P2 求解器在全量 500 个测试组合中有 268 个用例缩短了 Makespan，232 个用例持平，零退化。与此同时，500 个用例的额外 DDR 搬运总量增加了 36.66\%。该分布客观记录了版本迭代中执行时间缩短与额外搬运量增加相伴随的成对数值变动，反映了整体方案以 Makespan 优先的选优结果，不将其作为单一超图割或工作量保护机制的独立因果归因；
\item
  \textbf{C04 对照策略的限定反例}：针对前期研发策略 C04 在 6 个预选 5 核心用例上的对比显示，C04 方案虽然经官方 Step 2 编译确认未发生任何片上缓存换入换出（spill 为 0），且跨核心边界搬运字节数低于主算法，但其最终 Makespan 在这 6 个用例上全部变慢（在 \texttt{case\_019} 上变慢 75.50\%）。该限定反例否定了``边界搬运更少且无换出换入则完成时间必然缩短''的假设，表明单纯优化静态搬运量并不能保证执行时间下降；
\item
  \textbf{问题三 Forest 方案收益统计}：与前序基线方案相比，P3 Forest 求解器在全量 500 个测试组合中，在 24 个用例上缩短了 Makespan，其余 476 个用例持平，零退化；在全网格额外 DDR 搬运量与溢出搬运量上，各有 18 个用例减少，479 个用例持平，3 个用例增加，两项统计总量各净减少 \(413{,}450{,}240\) 字节。此处记录用例计数与总量变化，不外推为全用例普适改善；
\item
  \textbf{R9F 复合比值准则的消融对比}：在 5 核心下对候选方案附加 \(G_{\mathrm{new}} \ge G_{\mathrm{old}}\)（即 \(M_2/M_3\) 不下降）等三重准则的 R9F 对照实验中，有 3 个候选用例（\texttt{case\_064}、\texttt{case\_068}、\texttt{case\_088}）在实际执行中无 Cache 耗时 \(M_2\) 与开启 Cache 耗时 \(M_3\) 均下降，但因无 Cache 耗时的相对降幅更大导致比值数值下降，从而被复合比值准则拒绝。该消融对比表明，在多场景多目标评估中，人为构造的复合比值指标可能对主优化目标的选优产生干扰。
\end{enumerate}
...
由于全局下界 \(L\) 适用于全体合法解，任意合法执行的物理完成时间必然满足 \(L \le \mathrm{OPT} \le U\)。因此，在 \(L > 0\) 时，相对偏差比值 \(\frac{U}{L} - 1\) 给出了算法当前方案与理论最优解之间相对误差的\textbf{保守数学上限}。需要明确的是，此处全局下界 \(L\) 针对于全体可行方案空间，区别于第 6 章中仅对特定已排定候选方案成立的条件时序下界 \(L(P)\)，亦不使用附录 B.3 针对局部区间推导的新残余等待公式。
...
\tightlist
\item
  \textbf{高精度达标分布}：在 1 至 5 核心下，分别有 81、62、32、33 与 23 张基准计算图严格满足 \(U \le 1.05 L\)。即在当前声明的硬件条件下，这些测试用例输出方案的 Makespan 与理论最优解的相对误差不超过 5\%；
\item
  \textbf{多核松弛间隙演变}：在 5 核心场景下，全量计算图的平均下界加速比 \(\operatorname{mean}(B/L)\) 约为 6.108，高于算法实际达到的平均加速比 4.550。这一差距反映出静态全局下界模型松弛了片上存储容量竞争、总线读写仲裁以及跨核心复杂通信等动态瓶颈，因此 6.108 属于松弛数学边界，并不代表在真实硬件约束下已证实可达的加速比。
\end{enumerate}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=.84\textheight,keepaspectratio]{figures/team-v8/bound-gap.pdf}
\caption{同一输入、配置与合法解空间下的平均加速比及理论界对比。展示当前可行方案平均加速比 \(\operatorname{mean}(B/U)\) 与全局下界平均加速比 \(\operatorname{mean}(B/L)\) 随核心数演进的数学界限（阴影区域表示松弛理论间隙，非可实现加速比）。}
\label{fig:bound-gap}
\end{figure}
...
\tightlist
\item
  \textbf{样本关联性与统计范围}：测试所用的全部 100 张官方计算图在前期研发中均作为基准参与过特征分析。由于同一张计算图在 1 至 5 核心下的拓扑结构完全一致，不同核心数下的测试结果存在高度的内在关联性，不可将其视为 500 个独立同分布的统计样本进行假设检验，全篇 1500 种测试组合亦非统计独立样本；
\item
  \textbf{结构自适应与通用性}：算法内部的分支选择依据图的无向连通分量数量、算子出度以及流水线工作量等拓扑结构特征自动决策，避免了按用例编号显式查表，但其在完全未见的外部工业模型计算图上的表现仍有待进一步实测验证；
\item
  \textbf{仿真模型与物理硬件差异}：方案的最终质量指标（Makespan 与额外数据搬运量）完全由官方离散事件仿真器（\texttt{\seqsplit{schedule\_step2.py}} 与 \texttt{\seqsplit{schedule\_step3.py}}）推进时钟得出，端到端求解耗时由通用计算机的物理墙钟秒数记录。官方离散事件仿真模型提供了统一可比的基准评测环境，但本研究尚未在真实的神经网络处理器（NPU）物理芯片上部署运行并开展板级实测，仿真时钟周期不直接等价于已在真机上验证的物理性能收益。
\end{enumerate}
...
\FloatBarrier
\clearpage\begin{thebibliography}{17}
\raggedright
...
\bibitem{ref13} 张洪滨, 周旭林, 邢明杰, 等. AutoConfig: 面向深度学习编译优化的自动配置机制{[}J{]}. 软件学报, 2024, 35(6): 2668--2686. DOI: 10.13328/j.cnki.jos.007102.
\bibitem{ref14} 高伟, 李帅龙, 茆琳, 等. 一种基于TVM的算子生成加速策略{[}J{]}. 计算机工程, 2024, 50(8): 353--362. DOI: 10.19678/j.issn.1000-3428.0068182.
\bibitem{ref15} TOPCUOGLU H, HARIRI S, WU M Y. Performance-effective and low-complexity task scheduling for heterogeneous computing{[}J{]}. IEEE Transactions on Parallel and Distributed Systems, 2002, 13(3): 260--274. DOI: 10.1109/71.993206.
\bibitem{ref16} DROBOUCHEVITCH I G, STRUSEVICH V A. A heuristic algorithm for two-machine re-entrant shop scheduling{[}J{]}. Annals of Operations Research, 1999, 86: 417--439. DOI: 10.1023/A:1018927407164.
\bibitem{ref17} LIU R, PEI M, DING F, et al.~LATTICE: Constraint-Directed Scheduling, Memory Planning, and Pipeline Refinement for NPUs{[}EB/OL{]}. arXiv:2607.17422v3, 2026-08-04 {[}2026-09-26{]}. https://arxiv.org/abs/2607.17422v3. DOI: 10.48550/arXiv.2607.17422.
...
\end{equation} 进而通过观测恢复函数导出两者的观测完全一致： \begin{equation}
\begin{aligned}
o(S(a_{k+1}(w(P)))) &= o(S(a_{k+1}(w(Q)))) \\
&= \Phi(a_{k+1}(w(P))) \\
&= \Phi(a_{k+1}(w(Q))).
\end{aligned}
\end{equation} 由数学归纳法可知，等价性结论对任意有限长度的可执行动作序列均严格成立。 \end{proof}
...
\subsection{算法实现与正文伪代码映射}

各问题的算法主实现、评测上限及正文伪代码与源码的映射关系如表 \ref{tab:appendix-1} 与表 \ref{tab:appendix-2} 所示。

{\def\LTcaptype{table} % do not increment counter
\begin{table}[H]\centering
\caption{问题 1～3 的固定实现入口与在线评测上限。}\label{tab:appendix-1}\small
\begin{tabular}{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}
...
{\def\LTcaptype{table} % do not increment counter
\begin{table}[H]\centering
\caption{正文各算法伪代码与程序源码对应映射关系。}\label{tab:appendix-2}\small
\begin{tabular}{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}
...
算法 \ref{alg:p1-unified} & P1 统一细化流程 & \texttt{\seqsplit{src/q1/branch\_refine.py::solve}} & P1，见上一表 \\
算法 \ref{alg:p1-branch-aid} & 局部支路重分配候选构造 & \texttt{\seqsplit{src/q1/branch\_aid.py::construct}} (359--438 行) & P1，见上一表 \\
算法 \ref{alg:p2-load-guarded-cut} & 二核心负载约束超图割 & \texttt{\seqsplit{src/q2\_nikolastarx/binary\_hypercut.py::load\_guarded\_cut}} & P2，见上一表 \\
算法 \ref{alg:p2-adaptive-hypergap} & 自适应保护超图分核 & \texttt{\seqsplit{src/q2\_nikolastarx/adaptive\_hypergap\_guarded.py::build}} & P2，见上一表 \\
算法 \ref{alg:p3-forest-memory-order} & 森林拓扑指标递推与遍历序列构造 & \texttt{\seqsplit{src/q3/forest\_memory\_order.py::construct}} (153--192 行) & P3，见上一表 \\
算法 \ref{alg:p3-forest-solve} & 森林候选池受限在线评估 & \texttt{\seqsplit{src/q3/forest\_solve.py::evaluate\_candidates}} & P3，见上一表 \\
\bottomrule\end{tabular}
\end{table}
}

需要强调的是，正文中给出的各伪代码块系为阐述核心控制流与关键数学决策而提取的算法抽象，在实现细节上省略了输入校验展开、日志追踪与数据结构映射封装等外围代码；求解器的完整行为、输入输出接口与环境依赖完全以表 \ref{tab:appendix-1} 和表 \ref{tab:appendix-2} 标定的固定提交为准，附录 E 中的导出源码不作任意改动或替代。

官方代码哈希为 \texttt{\seqsplit{de11a83db8d7c47ed328b15a7df71d613a833b16cd23ee9fe877999578a1ace0}}，官方硬件配置哈希为 \texttt{\seqsplit{dcd10de54b23f8366428fb24e828812b1da9549e6eae4a3c3f38604fe5ae77b9}}。已有实验结果可在保留相关 Git 对象的仓库中，通过运行 \texttt{\seqsplit{python3\ paper/manuscript-v1/scripts/export\_tables.py}} 重新导出。该脚本仅读取冻结数据，不调用求解器或评价器。

\subsection{P2 全局下界证书来源与复核索引}

第 7 章中与 P2 全量方案 \(U\) 比较所采用的全局下界证书 \(L(G, K)\)，源自固定提交 \texttt{\seqsplit{00d311ed0eea0fd86f9840df406956041a7c192a}} 中的文档 \texttt{\seqsplit{docs/a/q2-nikolastarx/OPTIMALITY\_BOUNDS.md}} 与生成脚本 \texttt{\seqsplit{src/q2\_nikolastarx/global\_bounds.py}}。该证书文件的 SHA-256 校验和为 \texttt{\seqsplit{fe25f7b7737dbd3d841284bc5939242982613f64e0cffa9bd3d25df1f4a0c96b}}，归档于数据资产 \texttt{\seqsplit{results/a/q2-nikolastarx/goal-20260924/global-bounds.json}}。

在固定复核脚本 \texttt{\seqsplit{results/a/q2-nikolastarx/hypergap-full500-audit-20260925/audit.py}} 中，该证书直接关联至新方案 \(U\) 的逐用例评估数据。该下界覆盖 100 张计算图在 1～5 核下的完整 500 个评测点，是针对任意合法无死锁多核划分可行域 \(\mathcal{F}_2(G, K)\) 建立的全局理论下界；其与第 6 章针对固定完整多核候选方案 \(P\) 所建立的 FIFO 与依赖必要不等式下界 \(L(P)\) 具有不同内涵，而附录 B.3 则属于未产生本次统计的新离线理论补充推导（不构成单核方案证书），三者概念严格分离，第 7 章的数值界限比较仅采用上述全局证书 \(L(G, K)\)。

\FloatBarrier
\section{图中各类数据的含义与诊断示例}

\subsection{图中符号与数据来源定义}
...
\tightlist
\item
  \textbf{计算图片段}：图 \ref{fig:p1-cuts} 中以 \texttt{case\_026} 为例展示了原图局部的 8 个 \texttt{ADD} 算子，图中完整保留原始节点数字编号与各张量的传输字节数；黑色实线箭头表示算子间的数据与控制依赖，橙色虚线表示跨子图通信数据流；边界引出的点线端口用于连接局部裁剪区域外的依赖算子（表明其在原图中具有前驱或后继，并非孤立节点）。
\item
  \textbf{执行时间线与状态}：图 \ref{fig:p1-overlap}、图 \ref{fig:p2-counterexample} 等展示的真实执行时间线严格绑定相同输入、方案、核心数量和官方配置，各时间区间均由官方微架构模拟器记录的真实底层事件直接导出。时间线中的空白区间仅代表对应流水线在当前周期未记录到活跃操作占位，并不等价于计算核心整体休眠或 Task 停止，只有在存在仿真事件证据时才对等待原因进行解释。
\item
  \textbf{理论与教学示例}：第 5 章中的手算张量生命周期图与第 6 章中的树状示例明确注明为教学与数学原理示例，横轴为离散处理步骤，纵轴为内部张量占用的理论字节数，用于辅助阐明状态递推与定序准则，不代表官方模拟器的实际执行时刻。
\item
  \textbf{Cache 行为标注}：Cache 相关图件中严格区分了发起请求时刻的查询动作与 DDR 传输完成后的写入插入时刻。
\end{enumerate}

\subsection{P1 执行步调诊断性时间线分析}

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=.84\textheight,keepaspectratio]{figures/team-v8/p1-paced-diagnostic.png}
\caption{算例 case\_051（3核）control 对照组（291,222 cycles，额外 DDR 搬运量 9,045,304 B）与 paced 步调组（278,618 cycles，额外 DDR 搬运量 9,045,350 B）的执行时间线对比。图内呈现彩色流水线操作与灰色 Task 执行区间两层结构；Core 1--3 对应官方 Core 0--2；``归约尾''标注最终汇合 Task 的起始时刻。}
\label{fig:p1-paced-diagnostic}
\end{figure}

图 \ref{fig:p1-paced-diagnostic} 展示了针对算例 \texttt{case\_051}（3 核）进行的一组微架构诊断性对比（对应图件来源提交 \texttt{\seqsplit{bc2a42c277b9d70c51f3fc6974275354df33ee26}}，数据来源提交 \texttt{\seqsplit{059056ce1ee999e634049e5e5baf72566832effe}}），呈现了在相同链-核映射下，不同子图分组方案（control 与 paced 候选）在彩色流水线操作与灰色 Task 执行区间两层结构中的微架构事件分布。
...
按照代码清单说明（\texttt{\seqsplit{appendix-code/manifest.json}}），本文从附录 C 标定的三个固定 Git 提交中导出了 7 个核心算法模块，共计 1464 行原始 Python 代码。所选模块涵盖了各问最具代表性的核心算法逻辑：问题一的分支构造与重分配精化（\texttt{\seqsplit{p1/branch\_refine.py}} 与 \texttt{\seqsplit{p1/branch\_aid.py}}，计 677 行）；问题二的二核心超图割构建、代价计算与自适应保护选择（\texttt{\seqsplit{p2/adaptive\_hypergap\_guarded.py}}、\texttt{\seqsplit{p2/hypergraph\_cost.py}} 与 \texttt{\seqsplit{p2/binary\_hypercut.py}}，计 505 行）；以及问题三的森林树定序与自适应求解（\texttt{\seqsplit{p3/forest\_solve.py}} 与 \texttt{\seqsplit{p3/forest\_memory\_order.py}}，计 282 行）。上述 1464 行统计严格对应原始算法逻辑代码行数，不计入后续添加的 AI 使用声明注释。

各算法模块在编程实现过程中主要借助了由 OpenAI 开发的 GPT-6 Astra（API 型号：\texttt{gpt-6-astra}，版本发布日期：2026 年 9 月 3 日）提供辅助编程支持。代码归档体系对两类源码文件进行了明确区分：位于 \texttt{\seqsplit{appendix-code/p1}}、\texttt{p2}、\texttt{p3} 目录下的为逐字原始副本（7 个模块共 1464 行），其内容和字节哈希与附录 C 标定的三个固定历史提交严格一致；位于 \texttt{\seqsplit{appendix-code/declared/p1}}、\texttt{p2}、\texttt{p3} 目录下的为论文展示版本（7 个模块共 1520 行，包含 1464 行原始代码与 56 行头部规范 AI 辅助编程声明注释，声明``本程序及代码是在人工智能工具辅助下完成的''，并标明工具名称、型号、开发机构及版本发布日期）。展示版本除头部声明外，原程序逻辑部分与原始副本保持逐字一致且抽象语法树（AST）完全等价；因新增声明注释，整份展示文件的物理校验哈希必然不同，清单文件对两套副本的校验和与行数分别独立建档。模块间的具体依赖关系、执行入口脚本以及微架构仿真运行环境均依托附录 C 中记录的代码版本；此处展示的代码切片旨在呈现关键数学与调度决策的程序表达，并不代表脱离代码仓即可独立运行的自包含整包。
\Needspace{14\baselineskip}
\subsection{P1 branch\_refine.py}
\lstinputlisting[style=gmcmappendix,language=Python,belowskip=0.4em]{source/appendix-code/declared/p1/branch_refine.py}
\Needspace{14\baselineskip}
\subsection{P1 branch\_aid.py}
\lstinputlisting[style=gmcmappendix,language=Python,belowskip=0.4em]{source/appendix-code/declared/p1/branch_aid.py}
\Needspace{14\baselineskip}
\subsection{P2 adaptive\_hypergap\_guarded.py}
\lstinputlisting[style=gmcmappendix,language=Python,belowskip=0.4em]{source/appendix-code/declared/p2/adaptive_hypergap_guarded.py}
\Needspace{14\baselineskip}
\subsection{P2 hypergraph\_cost.py}
\lstinputlisting[style=gmcmappendix,language=Python,belowskip=0.4em]{source/appendix-code/declared/p2/hypergraph_cost.py}
\Needspace{14\baselineskip}
\subsection{P2 binary\_hypercut.py}
\lstinputlisting[style=gmcmappendix,language=Python,belowskip=0.4em]{source/appendix-code/declared/p2/binary_hypercut.py}
\Needspace{14\baselineskip}
\subsection{P3 forest\_solve.py}
\lstinputlisting[style=gmcmappendix,language=Python,belowskip=0.4em]{source/appendix-code/declared/p3/forest_solve.py}
\Needspace{14\baselineskip}
\subsection{P3 forest\_memory\_order.py}
\lstinputlisting[style=gmcmappendix,language=Python,belowskip=0.4em]{source/appendix-code/declared/p3/forest_memory_order.py}
