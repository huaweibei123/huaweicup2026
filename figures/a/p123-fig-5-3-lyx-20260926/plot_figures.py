...
def load_records(problem: str) -> list[dict[str, Any]]:
    ...
    elif problem in {"P1", "P2"}:
        # The published full500 feed retains per-case makespans while the
        # large normalized input JSON remains intentionally gitignored.
        records = [
            {
                "case_id": row["case_id"],
                "cores": row["cores"],
                "status": "ok",
                "metrics": {"makespan_cycles": row["makespan"]},
                "attempt_id": row.get("attempt_id", ""),
                "revision": row.get("revision", 0),
            }
            for row in load_per_case()[problem]
        ]
    else:
        raise FileNotFoundError(f"missing input feed: {source}")
    if len(records) != 500:
...
def make_fig53() -> Path:
    ...
    manifest = base_manifest("fig-5-3", "P2 multicore speedup", out, "ready_for_human_review", 100, [PER_CASE], ["Only q2-adaptive-budget has complete 100-case coverage in this source batch.", "The k=1 plot anchor is fixed to 1.0; observed k=1 ratios remain in the CSV.", "The published per_case.json is the reproducible plot feed; the normalized P2 input is a local ignored source.", "SVG/PDF were generated but not opened in a desktop reader in this session."])
    manifest["source"]["commit"] = "5f4b7036ef7d05fcc88242742ad9ac0ab75b2b5a"
    manifest["source"]["sha256"] = {"results/a/p123-full500-lyx-20260925/per_case.json": "122da1335cf8f70e3db472ff84270cb3fc17690fcb3200607555c9dad7545847"}
    manifest["source"]["working_copy_sha256"] = {"results/a/p123-full500-lyx-20260925/per_case.json": sha256_file(PER_CASE)}
...
def make_fig45() -> Path:
    ...
    axes[2].set_title("Transfer cost (mean)", loc="left")
