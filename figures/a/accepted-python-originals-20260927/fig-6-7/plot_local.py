"""Plot verified P3 forest revision2 data; no experiment code."""
import csv,json
...
p=Path(__file__).resolve().parent;r=list(csv.DictReader((p/'p3_quality_cost_rows.csv').open(encoding='utf-8')));s=list(csv.DictReader((p/'solver_wall_quantiles.csv').open(encoding='utf-8')))
plt.rcParams.update({'font.family':'Microsoft YaHei','font.size':9,'axes.unicode_minus':False,'svg.hashsalt':'fig67-local-format-v1'})
...
fig.text(.16,.973,'图 6-7｜P3 forest 质量、求解耗时与尾部延迟',fontsize=11,weight='bold')
...
fig.text(.16,.072,'Apple M5 Pro / macOS arm64；每分片 worker = 1，全局最多 4。',fontsize=8)
fig.text(.16,.052,'墙钟含在线 E0；无独立外部评价计时；每格单次观测。',fontsize=8)
fig.text(.16,.032,'P95 线性插值；父进程预检另计，不推断冷启动或 Pareto 前沿。',fontsize=8)
for ext in ['svg','png','pdf']:fig.savefig(p/('fig67_p3_quality_cost.'+ext),dpi=300)
with Image.open(p/'fig67_p3_quality_cost.png') as im:im.resize((624,797),Image.Resampling.LANCZOS).save(p/'preview-insert-width.png')
