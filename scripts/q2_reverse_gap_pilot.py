"""One frozen three-cell reverse-gap mechanism probe, never a full benchmark."""
...
def freeze(raw_root, output):
    ...
    subprocess.run(['git', 'diff', '--quiet', 'HEAD', '--', 'src/q2_nikolastarx',
                    'scripts/q2_reverse_gap_pilot.py', 'data/raw/a/official/code'],
                   cwd=ROOT, check=True)
    source = subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip()
    names = subprocess.check_output(['git', 'ls-files', 'src/q2_nikolastarx',
        'scripts/q2_reverse_gap_pilot.py', 'data/raw/a/official/code'], cwd=ROOT, text=True).splitlines()
    ...
    dump(output, {'schema': 'p2-reverse-gap-three-v1', 'created_at': utc(),
                  'source_commit': source, 'source_hashes': hashes,
                  'config_sha256': sha(CONFIG), 'cells': cells,
                  'limits': {'constructor': 3, 'E0': 3, 'E1': 0, 'E2': 0, 'retry': 0,
...
def run(frozen, raw_root, output, python):
    ...
    assert m['schema'] == 'p2-reverse-gap-three-v1'
    assert [r['case'] for r in m['cells']] == list(CASES)
    assert m['limits'] == {'constructor': 3, 'E0': 3, 'E1': 0, 'E2': 0, 'retry': 0,
                          'workers': 1, 'solver_seconds': 30, 'E0_seconds': 30,
                          'batch_seconds': 120, 'rss_bytes': 536870912}
