...
def model_plan(graph,plan,bandwidth,capacity,gate):
    ...
    local_op_counts=[len(g['ops']) for g in local.values()]
    event_budget_ok=(max(local_op_counts,default=0)+1<=100000 and sum(local_op_counts)+len(local)+2<=1000000)
    return {'kind':'middle_model_NOT_E0','official_iteration_budget_sufficient_condition':event_budget_ok,
            'compiled_op_count_without_spill':sum(local_op_counts),'cycles':max(endcore,default=0),'upper_envelope_cycles':max(uppercore,default=0),
...
def choose_and_construct(graph,cores,bandwidth,capacity,gate=100):
    ...
    info={'kind':'candidate_generation_NOT_E0','algorithm':'p1-one-cut-return-rotation-v1',
          ...
          'conservative_dominance_certificate':bool(model['virgin_capacity_certificate'] and model['official_iteration_budget_sufficient_condition']
                                                   and model['upper_envelope_cycles']<baseline_compute_lb),
          'model':model}
    return plan,info
