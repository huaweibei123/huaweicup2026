...
def homogeneous_cut_or_serialize(n:int,ell:int,delta:int,d0:int,cores:int):
    """Universal only under the accompanying intact-chain FIFO-lock theorem.
    min_{0<=s<=n} max(ell*ceil((n-s)/K), d0+delta*s). O(log n).
    """
    if not (n>=0 and ell>=1 and delta>=0 and d0>=0 and 1<=cores<=5):raise ValueError('bad inputs')
    def f(s):return ell*ceildiv(n-s,cores)
    ...
    return {'bound':max(f(s),g(s)),'minimizer_cut_count':s,'N':n,'chain_compute_cycles':ell,
        ...
        'crossing':lo,'checked_counts':candidates}
...
def strict_family_cut_certificate(graph,cores,bandwidth):
    ...
    cert=homogeneous_cut_or_serialize(len(chains),ell,delta,d0,cores)
...
def heterogeneous_dual(lengths,deltas,d0,cores):
    """Exact maximization of a concave dual bound, O(N log N), rational witness.
    Scope: disjoint intact spans and disjoint necessary extra cut services.
    """
    ...
    events.sort();slope=sum((Fraction(e,cores) for e in lengths),Fraction(-d0));value=Fraction(d0)
