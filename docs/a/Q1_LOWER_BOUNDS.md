...
每 Pipe 取 c=K，全局 DDR 取 c=1；空工作集取 0。旧版只使用整组的最小释放/尾部。2026-09-25 新实现继续使用同一批必要 job，增加下面的双阈值窗口；旧结果文件保留原件，不能用新实现的数字覆盖旧记录。

### 双阈值资源窗口

对非空 `S = {i: r_i >= R, q_i >= Q}`，每个必要 job 都须在 `[R, M-Q]` 中取得服务，因此容量守恒给出

```
M >= R + Q + ceil(sum(d_i for i in S) / c).
```

这是图本身的必要 job，不含候选 FIFO、Task 数或内存复用边。对可抢占公平 DDR 仍成立，DDR 容量取 1，不是 K。

`src/q1/resource_windows.py` 将 q 坐标压缩，以 `c*q` 初始化线段树，按 r 降序激活 job，对不大于当前 job 的 q 的前缀加 d。每个 r 组激活完，只查询非空的阈值范围；对空集合查询会制造假界。最大窗口计算为 O(J log J)，空间 O(J)。整数计算不使用浮点近似。

每个资源新增 `resource_window`，含阈值、容量、job 数、入选数、服务和及下界。`verify_resource_window` 用另一遍 O(J) 扫描核对证书，构造调用时立即核验。这个验证证明给定窗口不等式，不证明其为最大窗口，更不自动证明调用方传来的 job 必要；图到 job 的语义仍由前述原始依赖/边界证明承担。

最终 `lower_bound_cycles` 取旧界与各窗口界最大值。反例夹具：两个 M100 位于共同 V100 前缀与尾部之间，另有独立 M1；整组释放/尾部变成0、工作201，关键路径300，而必要 M 子集给出400。测试还用固定种子的350个小对象与独立朴素阈值枚举对照，不进行方案搜索或官方评分。

来源：[Pro 原始研究](../../AI%20chats/P1多Pipe链构造证明/附件/r1-p1_s6607/RESEARCH_NOTE.md)及[独立初审](../../results/a/review/p1-pro-initial-20260924/INDEPENDENT_REVIEW.md)。模块是独立整理的实现，不导入 Pro 原型或其结果。

在 op 邻接已经形成后，最长路为 O(n+m)，资源归约增加 O(J log J) 时间、O(J) 空间。从原二部图展开边还包含每个张量生产者×消费者的成本，官方输入验证与确定性初始化也有排序开销；不把完整 CLI 笼统称为严格线性。代码身份额外保存窗口模块 SHA256。
